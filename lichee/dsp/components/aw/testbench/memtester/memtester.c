...
/* #define HARDCODE_LSPSRAM */
/* #define HARDCODE_HSPSRAM */

#if defined HARDCODE_LSPSRAM
#define HARDCODE_BUFFER 0x8000000 //lpsram
#define HARDCODE_BUFFER_SIZE (4*1024*1024)
#elif defined HARDCODE_HSPSRAM
#define HARDCODE_BUFFER 0xc000000 //hspsram
#define HARDCODE_BUFFER1 0xc000000 //hspsram
#define HARDCODE_BUFFER2 0xc200000 //hspsram
#define HARDCODE_BUFFER3 0xc400000 //hspsram
#define HARDCODE_BUFFER4 0xc600000 //hspsram
#define HARDCODE_BUFFER_SIZE (2*1024*1024)
#endif
...
int cmd_memtest(int argc, char **argv) {
    ...
    optind = 0;
    while ((opt = getopt(argc, argv, "qm:s:t:a:")) != -1) {
        switch (opt) {
            case 'q':
                quiet = 1;
                break;
	    ...
	    case 'a':
		errno = 0;
		buf_num = strtoul(optarg, 0, 10);
		if (errno != 0) {
			fprintf(stderr, "failed to parse address argument");
			usage(argv[0]);
			return (EXIT_FAIL_NONSTARTER);
		}
		break;	
    ...
    if (wantbytes < pagesize) {
        fprintf(stderr, "bytes %zd < pagesize %zd -- memory argument too large?\n",
                wantbytes, pagesize);
        return (EXIT_FAIL_NONSTARTER);
    }
    ...
    while (!buf && wantbytes) {
#ifndef HARDCODE_BUFFER
	buf = (void volatile *) malloc(wantbytes);
	if (!buf) wantbytes -= pagesize;
#else
	switch (buf_num) {
	case 1:
		buf = HARDCODE_BUFFER1;
		break;
	case 2:
		buf = HARDCODE_BUFFER2;
		break;
	case 3:
		buf = HARDCODE_BUFFER3;
		break;
	case 4:
		buf = HARDCODE_BUFFER4;
		break;
	default :
		buf = HARDCODE_BUFFER;
		wantbytes = 8 * 1024 * 1024;
	}
		if (wantbytes > HARDCODE_BUFFER_SIZE && buf_num != 0)
			wantbytes = HARDCODE_BUFFER_SIZE;
#endif
    }
    aligned = buf;
    bufsize = wantbytes;
...
FINSH_FUNCTION_EXPORT_CMD(cmd_memtest, memtester, memory test);
