...
/*  @brief SPI chip select signal polarity*/
typedef enum
{
    HAL_SPI_MASTER_SPOL_LOW = 0, /**< default, Low level effect. */
    HAL_SPI_MASTER_SPOL_HIGH = 4 /**< High level effect. */
}hal_spi_master_spol_t;
...
typedef struct
{
    ...
    cpha; /**< SPI master clock phase setting. 0: Phase 0(Leading edge
            ...
            */
    hal_spi_master_spol_t
    spol;/**< SPI chip select signal polarity. 0:Low 1:High */
    uint32_t sip;
    uint32_t flash;
    bool slave;
...
spi_master_status_t hal_spi_panic_xfer(hal_spi_master_port_t port, hal_spi_master_transfer_t *transfer);
#endif
#endif
