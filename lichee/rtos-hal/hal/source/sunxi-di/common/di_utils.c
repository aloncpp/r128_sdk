...
void *di_malloc(__u32 bytes_num, uintptr_t *phy_addr)
{
	...
	if (bytes_num != 0) {
		address = hal_malloc_coherent(bytes_num);
		if (address) {
		...
		DI_ERR("hal_malloc_coherent fail, size=0x%x\n", bytes_num);
		return NULL;
	}
...
struct di_mapped_buf *di_dma_buf_alloc_map(u32 size)
{
	...
	mbuf->vir_addr = hal_malloc_coherent(mbuf->size_alloced);
	if (mbuf->vir_addr == NULL) {
		DI_ERR("hal_malloc_coherent fail, size=0x%x->0x%x\n",
...
void di_dma_buf_unmap_free(struct di_mapped_buf *mbuf)
{
	DI_DEBUG("%s: addr[%p], size=0x%x\n", __func__,
		mbuf->vir_addr, mbuf->size_alloced);
	hal_free_coherent(mbuf->vir_addr);
	hal_free(mbuf);
}
