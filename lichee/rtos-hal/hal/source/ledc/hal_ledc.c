...
int hal_ledc_trans_data(struct ledc_config *ledc)
{
	...
	if (ledc->length <= SUNXI_LEDC_FIFO_DEPTH) {
		...
		if (size <= LEDC_DMA_BUF_SIZE) {
			memcpy(ledc->align_dma_buf, buf, size);
			buf = ledc->align_dma_buf;
		}

		for(i = 0; i < ledc->length; i++)
			ledc_info("buf data is %d\n", buf[i]);
...
int hal_ledc_init(void)
{
	...
	led->config.align_dma_buf = hal_malloc_coherent(LEDC_DMA_BUF_SIZE);
...
err5:
	hal_free_coherent(led->config.align_dma_buf);
err4:
 ...
 * @led_num: the num of leds on led-strip
