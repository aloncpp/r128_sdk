...
#define AXP_IRQ_TASK_STACK_SIZE 512
...
int axp2585_init_power(struct power_dev *rdev)
{
	...
	/* close weak bat irq */
	hal_power_byte_update(rdev, AXP2585_INTEN1, 0x00, 0xff);
	hal_power_byte_update(rdev, AXP2585_INTEN3, 0x00, 0x08);
	hal_power_byte_update(rdev, AXP2585_INTEN5, 0x00, 0xff);
	hal_power_byte_update(rdev, AXP2585_INTEN6, 0x00, 0xff);
	...
	ret = hal_cfg_get_keyvalue(PMU_NAME, "pmu_irq_pin",
			(int32_t *)&gpio_set,
			sizeof(user_gpio_set_t) >> 2);
	if (ret == 0) {
		power_spy->gpio = (gpio_set[0].port - 1) * 32 + gpio_set[0].port_num;
		power_spy->mul_sel = gpio_set[0].mul_sel;
	} else {
		power_spy->gpio = 0;
		power_spy->mul_sel = 0;
		printf("axp2585 use no irq\n");
	}

	if (power_spy->gpio) {
		if (hal_gpio_to_irq(power_spy->gpio, &power_spy->pmu_irq_no) < 0) {
			pr_err("request axp_irq gpio to irq error\n");
			return -1;
		}
		hal_gpio_pinmux_set_function(power_spy->gpio, power_spy->mul_sel);
		if (hal_gpio_irq_request(power_spy->pmu_irq_no, axp2585_irq_handler,
					IRQ_TYPE_EDGE_FALLING, rdev) < 0) {
	...
	xTaskCreate(axp_irq_task, (signed portCHAR *) "axp_irq_task",
			AXP_IRQ_TASK_STACK_SIZE, (void *)rdev, 31, &task);
