...
static int isp_config_switch_sensor_info(struct isp_dev *isp)
{
	...
	if (global_sensors[sensor_id].sensor_core->sensor_g_switch_format)
		sensor_format = global_sensors[sensor_id].sensor_core->sensor_g_switch_format(sensor_id, vinc->isp_sel);
	else
		return -1;
...
void vin_isp_set_ae_idx(struct isp_dev *isp, unsigned short ev_idx)
{
	struct hw_isp_device *hw_isp = isp->hw_isp;
	struct isp_lib_context *ctx = isp_dev_get_ctx(hw_isp);
	ae_result_t *ae_result = NULL;
	...
	ae_result->sensor_set.ev_set_curr.ev_idx = ev_idx;
...
void vin_isp_set_lightadc_debug(struct isp_dev *isp, unsigned char lightadc_debug_en)
{
	struct hw_isp_device *hw_isp = isp->hw_isp;
	struct isp_lib_context *ctx = isp_dev_get_ctx(hw_isp);
	...
	ctx->lightadc_debug_en = lightadc_debug_en == 0 ? false : true;
}
...
int vin_isp_ae_done(struct isp_dev *isp, int drap_frame_en)
{
	...
	if (!drap_frame_en ||
			...
			//(awb_result->awb_status == AWB_STATUS_DONE || isp_gen->awb_frame_cnt >= 30))) {
		if (drap_frame_en) {
			vin_print("isp%d:hard light adc & HFR! isp_count:%ld, ae_conut:%ld, awb_count:%ld\n",
				isp->id, isp->isp_frame_number, (unsigned long)isp_gen->ae_frame_cnt, (unsigned long)isp_gen->awb_frame_cnt);
		}
		return 0;
	}
#elif defined CONFIG_ISP_ONLY_HARD_LIGHTADC
	if (1) {
