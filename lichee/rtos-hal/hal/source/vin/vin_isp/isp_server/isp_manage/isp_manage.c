...
static void __isp_ctx_cfg_mod(struct isp_lib_context *isp_gen)
{
	...
	//WDR
	if (isp_gen->sensor_info.wdr_mode == ISP_2FCMD_WDR_MODE_T || isp_gen->sensor_info.wdr_mode == ISP_3FCMD_WDR_MODE_T) { /* one ch data input */
		mod_cfg->mode_cfg.wdr_mode = WDR_1FCH;
		isp_gen->ae_settings.ae_mode = AE_NORM;
		mod_cfg->mode_cfg.dg_mode = DG_AFTER_WDR;
	} else if (isp_gen->sensor_info.wdr_mode == ISP_2FDOL_WDR_MODE_T || isp_gen->sensor_info.wdr_mode == ISP_2FCMD1FDOL_WDR_MODE_T
			|| isp_gen->sensor_info.wdr_mode == ISP_1FDOL2FCMD_WDR_MODE_T) {/* two ch data input */
	...
	} else if (isp_gen->sensor_info.wdr_mode == ISP_3FDOL_WDR_MODE_T) { /* three ch data input */
