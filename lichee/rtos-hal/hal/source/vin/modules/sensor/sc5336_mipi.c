...
static int sensor_reg_init(int id, int isp_id)
{
	...
	//sc5336 need hflip/vflip before sensor stream on
	if (id == 0)
	...
	if ((id == 0 && sensor_isp_cfg->sign == 0xAA66AA66) ||
		...
		sensor_write(id, 0x3040, flip_status);
	}

	//sensor chip version
	sensor_read(id, 0x3040, &rdval);
	if (rdval == 0x00) {
		sensor_write(id, 0x3258, 0x0c);
		sensor_write(id, 0x3249, 0x0b);
		sensor_write(id, 0x3934, 0x0a);
		sensor_write(id, 0x3935, 0x00);
		sensor_write(id, 0x3937, 0x75);
		sensor_write(id, 0x393d, 0x07);
		sensor_write(id, 0x393e, 0xff);
	} else if (rdval == 0x03) {
		sensor_write(id, 0x3258, 0x08);
		sensor_write(id, 0x3249, 0x07);
		sensor_write(id, 0x3934, 0x05);
		sensor_write(id, 0x3935, 0x07);
		sensor_write(id, 0x3937, 0x74);
		sensor_write(id, 0x393d, 0x01);
		sensor_write(id, 0x393e, 0xa4);
	}
