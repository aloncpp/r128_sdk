...
/*
 * ISP_GET_CFG_NUM:
 * there is only two part sector, part0 and part1
 * part0 is VIN_SENSOR0_RESERVE_ADDR/ISP1_NORFLASH_SAVE
 * part1 is VIN_SENSOR1_RESERVE_ADDR/ISP1_NORFLASH_SAVE
 * two_linear mode: part0 corresponding isp0, part1 corresponding isp1
 * one_linear and one_wdr mode: part0 corresponding isp0, part1 corresponding isp2
 * two_wdr mode: part0 corresponding isp0, part1 corresponding isp2
 */
#define ISP_GET_CFG_NUM 2

extern struct isp_get_cfg_t isp_get_cfg[ISP_GET_CFG_NUM];
...
struct sensor_fuc_core {
	...
	int (*s_stream)(int id, int isp_id, int enable);
	int (*s_switch)(int id);
	int (*s_exp_gain)(int id, struct sensor_exp_gain *exp_gain);
	struct sensor_format_struct *(*sensor_g_format)(int id, int isp_id);
	struct sensor_format_struct *(*sensor_g_switch_format)(int id, int isp_id);
};
