...
int sunxi_fixed_clk_init(void)
{
    ...
    ret = clk_register_fixed_rate_with_accuracy("rcosc-clk", RCOSC_CLK, NULL, 0, 1400000, 300000000);
    if (ret)
    {
        printf("register clock rcose-clk error\n");
    }

    ret = clk_register_fixed_rate_with_accuracy("rccal-fake-parent", CLK_RCCAL_FAKE_PARENT, NULL, 0, 32768, 0);
    if (ret)
    {
        printf("register clock rc-32k error\n");
    }
#endif
