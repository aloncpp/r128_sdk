...
static struct ccu_sdm_setting pll_audio0_sdm_table[] = {
	{ .rate = 98304000,   .pattern = 0xc001eb85, .m = 21, .n = 86 },
	{ .rate = 1083801600, .pattern = 0xA000A234, .m = 2,  .n = 90 },
};
...
static const char * const gpu_parents[] = {"pll-gpu", "pll-peri0-800m", "pll-peri0-600m", "pll-peri0-400m", "pll-peri0-300m", "pll-peri0-200m" };
...
static SUNXI_CCU_GATE(dma_mbus_gate_sw_clk, "dma-mbus-sw-gate",
		"dcxo24M",
		0x0804, BIT(16), 0);
...
static const char * const csi_parents[] = { "pll-peri0-300m", "pll-peri0-400m", "pll-peri0-480m", "pll-video3-4x", "pll-video3-4x" };
...
static struct clk_hw_onecell_data sun55iw3_hw_clks = {
	.hws    = {
		...
		[CLK_DMA_MBUS_GATE_SW]		= &dma_mbus_gate_sw_clk.common.hw,
...
int sunxi_ccu_init(void)
{
    ...
    {
        val = readl(reg + sun55iw3_pll_regs[i]);
        val |= BIT(29) | BIT(30) | BIT(31);
        writel(val, reg + sun55iw3_pll_regs[i]);
    }
    /*
    ...
    {
        val = readl(reg + sun55iw3_pll_video_regs[i]);
        val &= ~BIT(0);
        writel(val, reg + sun55iw3_pll_video_regs[i]);
    }

    /* Enforce m0 = 0 for Audio0 PLL */
    val = readl(reg + SUN55IW3_PLL_AUDIO0_REG);
    val &= ~BIT(0);
    val &= ~BIT(1);
    writel(val, reg + SUN55IW3_PLL_AUDIO0_REG);
    ...
    {
        val = readl(reg + sun55iw3_usb_clk_regs[i]);
        val &= ~GENMASK(25, 24);
        writel(val, reg + sun55iw3_usb_clk_regs[i]);
    }
