...
int get_xip_part_offset(void)
{
    ...
    int start_sector;
    int sectors;
    char xip_part_name[16] = "rtos-xip";	//part name max len is 16
...
#ifdef CONFIG_FLASHC_GET_XIP_PARTITION_FROM_ENV
    /* get xip part name from env. if get failed, use default name:rtos-xip */
    int flashc_get_xip_part_name_from_env(char *gpt_buf, char *buff);
    flashc_get_xip_part_name_from_env(gpt_buf, xip_part_name);
#endif

    ret = get_part_info_by_name(gpt_buf, xip_part_name, &start_sector, &sectors);
    if (ret) {
        ret = -EIO;
        goto err;
    }
...
extern int nor_panic_erase(unsigned int addr, unsigned int size);
extern int nor_panic_write(unsigned int addr, char *buf, unsigned int size);
int32_t hal_spinor_panic_read_data(uint32_t addr, void *buf, uint32_t cnt)
{
    return nor_panic_read(addr, (void *)buf, cnt);
}

int32_t hal_spinor_panic_program_data(uint32_t addr, const void *buf, uint32_t cnt)
{
    return nor_panic_write(addr, (void *)buf, cnt);
}

int32_t hal_spinor_panic_erase_sector(uint32_t addr, uint32_t size)
{
    return nor_panic_erase(addr, size);
}
#endif
