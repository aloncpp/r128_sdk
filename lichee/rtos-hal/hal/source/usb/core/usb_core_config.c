...
int usb_get_all_config_desc_and_parser(struct usb_device *dev)
{
	...
	buffer = hal_malloc_coherent(USB_DT_CONFIG_SIZE);
#else
...
#if ENABLE_DMA_ALLOC_COHERENT
		bigbuffer = hal_malloc_coherent(length);
#else
...
#if ENABLE_DMA_ALLOC_COHERENT
				hal_free_coherent(dev->rawdescriptors[i]);
#else
