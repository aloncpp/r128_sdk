...
# Created Time:2019年09月02日 星期一 15时51分38秒
#====================================================================*/
...
void download_mult(int client_socket, char *file_name, long content_length)
{
    ...
    int len;
#if 0     /* by danny STAROT on 2022-11-18 */
    char rtos_file_name[256+6];
    strcpy(rtos_file_name, "/data/");
    strcpy(rtos_file_name+6, file_name);
    printf("rtos_file_name: %s ^_^\n\n",rtos_file_name);
#endif
    printf("file_name: %s ^_^\n\n",file_name);

    //创建文件描述符
    int fd = open(file_name, O_CREAT | O_WRONLY | O_TRUNC, S_IRWXG | S_IRWXO | S_IRWXU);
    if (fd < 0)
    {
        printf("文件创建失败!\n");
        return;
    ...
    while (hasrecieve < content_length)
    {
        ...
        if (diff - old >= 1000000)//当一个时间段大于1s=1000000us时, 计算一次速度
        {
...
int cmd_wget(int argc, char const *argv[])
{
    /* 命令行参数: 接收两个参数, 第一个是下载地址, 第二个是文件的保存位置和名字, 下载地址是必须的, 默认下载到当前目录
     * 示例:
	 * wget http://www.baidu.com /data/baidu.html
	 * wget https://ftp.gnu.org/gnu/time/time-1.8.tar.gz /data/time-1.8.tar.gz
     */
...
FINSH_FUNCTION_EXPORT_CMD(cmd_wget, wget, network wget mult test);
