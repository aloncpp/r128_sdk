...
static int hexdump_do(const char *file, int len, int skip, int chr)
{
    ...
    while (len > 0 && (ret = read(fd, buf, MIN(HEXDUMP_BUFSIZE, len)))) {
...
{
    printf("Usage: hexdump [-C] [-n length] [-s skip]\n");
    printf("       hd [-n length] [-s skip]\n");
...
FINSH_FUNCTION_EXPORT_CMD(hexdump_main, hexdump, dump binary data in decimal);
...
FINSH_FUNCTION_EXPORT_CMD(hd_main, hd, the same as 'hexdump -C');
