...
static int aw_upgrade_cmd(int argc, char **argv)
{
	...
	if (argc == 1) {
		 ...
		 * aw_upgrade /dev/rtosB /data/update/update_1.bin       0 512000 0
		 * aw_upgrade /dev/rtosB /data/update/update_2.bin  512000 512000 0
		 * aw_upgrade /dev/rtosB /data/update/update_3.bin 1240000  73728 1
		 */
		file_name = argv[1];
		source_file = argv[2];
		offset = atoi(argv[3]);
		...
		ret = aw_upgrade_slice(file_name, buffer, offset, slice_size, is_last_slice);
	} else {
