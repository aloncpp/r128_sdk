...
void otaprogressBar(long cur_size, long total_size)
{
	...
	if (pgress.FUNC != NULL)
	{
		pgress.FUNC(percent * (pgress.end - pgress.start));
	}
...
void * download(void * socket_d)
{
	...
	while (length < resp.content_length && (len = lwip_read(client_socket, buf, buf_len)) != 0 )
	{
		write(fd, buf, len);
		length += len;
		otaprogressBar(length, resp.content_length);
	}
...
void * download_rtos(void * socket_d)
{
	...
	uint8_t *buf = (uint8_t *) malloc(mem_size * sizeof(uint8_t));
	if (buf == NULL)
	...
	if (length == resp.content_length) {
		printf("\nDownload successful ^_^\n\n");
		//aw_upgrade_end(0); /* just for midea app */
	} else {
...
int update_from_flash(char* path)
{
	...
	if (stat(source_file, &statbuff) < 0) {
		...
		resp.content_length = source_file_size;
		aw_upgrade_end(0);
	}
...
int verify_md5_for_network(unsigned char md5[16])
{
	int ret = 0;
	int source_file_size = resp.content_length;
	...
	{
		printf("verify md5 value failed !\n");
		return -1;
	}
...
int verify_md5_for_flash(char *path)
{
	int ret = 0;
	char target_rtos[16 + 5 + 1]; /* 5 is "/dev/" */
	unsigned char source_md5sum[16] = {0}, dest_md5sum[16] = {0};
	int source, source_file_size;
	struct stat statbuff;
	char* source_file = path;
	...
	int get_rtos_to_upgrade(char *target_rtos);
	get_rtos_to_upgrade(target_rtos);
	printf("para_in:%s , target:%s\n", path, target_rtos);

	int get_md5sum(char *path, unsigned int file_size, unsigned char decrypt[16]);

	ret = get_md5sum(path, source_file_size, source_md5sum);
	if (ret)
	{
		goto md5sum_ota_out;
	}

	ret = get_md5sum(target_rtos, source_file_size, dest_md5sum);
	if (ret)
	{
		goto md5sum_ota_out;
	}

	if (memcmp(source_md5sum, dest_md5sum, 16) != 0)
	{
		printf("verify md5 value failed !\n");
		ret = -1;
		goto md5sum_ota_out;
...
int ota_upgrade(int from, char *path, int start, int end, void (*FUNC)(int))
{
	int ret = 0;
	pgress.start = start;
	pgress.end = end;
	...
	if (from == 1)
	{
		...
		{
			ret = -1;
			printf("path not start with http : %s\n", path);
			goto ota_upgrade_out;
		}
		ret = update_from_network(path);
	}
	else if (from == 0)
	{
		...
		{
			ret = -1;
			printf("path not start with /data : %s\n", path);
			goto ota_upgrade_out;
		}
...
static int cmd_awota(int argc, char **argv)
{
	int ret;
	if (argc == 1) {
...
FINSH_FUNCTION_EXPORT_CMD(cmd_awota, awota, Tina RTOS aw-ota demo);

static int cmd_ota_task(int argc, char **argv)
{
	return ota_task(NULL);
}
FINSH_FUNCTION_EXPORT_CMD(cmd_ota_task, awota_task, Tina RTOS awota debug);
