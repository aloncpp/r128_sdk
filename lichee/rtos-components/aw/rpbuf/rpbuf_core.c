...
__attribute__((constructor)) static void rpbuf_links_lock_create(void)
{
	__rpbuf_links_lock = rpbuf_mutex_create();
}

__attribute__((destructor)) static void rpbuf_links_lock_delete(void)
{
	rpbuf_mutex_delete(__rpbuf_links_lock);
}

#ifdef CONFIG_COMPONENTS_RPBUF_MEASURE_TIME
#define HTIMER_BASE                    (CONFIG_COMPONENTS_RPBUF_MEASURE_TIMER_BASE)
#define HTMR_CTRL_REG                  (HTIMER_BASE + 0x00)
#define HTMR_INIV_VALUE_REG            (HTIMER_BASE + 0x04)
#define HTMR_CUR_VALUE_REG             (HTIMER_BASE + 0x08)
...
uint32_t trace_rpbuf_get_time(void)
{
	/* timer clk is 3MHz */
	return (0xffffffff - *(volatile uint32_t *)(HTMR_CUR_VALUE_REG)) / 3;
}

void put_trace_rpbuf_time(struct rpbuf_buffer *buffer, int idx, uint32_t time)
{
	uint32_t *va;
...
void dump_rpbuf_time(struct rpbuf_buffer *buffer)
{
	uint32_t *va;
	uint32_t time;
	...
	time = va[RPBUF_TRACE_START];
	printf(" %s transmit time:\n", buffer->name);
	printf("\t>start      : 0 us [%u.%u ms]\n", time / 1000, time % 1000);

	time = va[RPBUF_TRACE_FLUSH] - va[RPBUF_TRACE_START];
	printf("\t>flush cache: %u us\n", time);

	time = va[RPBUF_TRACE_NOTIFY] - va[RPBUF_TRACE_START];
	printf("\t>send       : %u us\n", time);

	time = va[RPBUF_TRACE_WAIT_ACK] - va[RPBUF_TRACE_START];
	printf("\t>wait       : %u us\n", time);

	if ((buffer->flags & BUFFER_SYNC_TRANSMIT)) {
		time = va[RPBUF_TRACE_R_NOTIFY] - va[RPBUF_TRACE_START];
		printf("\t\t<recv     : %u us\n", time);

		time = va[RPBUF_TRACE_R_S_CB] - va[RPBUF_TRACE_START];
		printf("\t\t<call cb    : %u us\n", time);

		time = va[RPBUF_TRACE_R_S_NOTIFY] - va[RPBUF_TRACE_START];
		printf("\t\t<send ack   : %u us\n", time);

		time = va[RPBUF_TRACE_R_FINISH] - va[RPBUF_TRACE_START];
		printf("\t\t<finish     : %u us\n", time);
	}

	time = va[RPBUF_TRACE_END] - va[RPBUF_TRACE_START];
	printf("\t>end        : %u us\n", time);

	if ((buffer->flags & BUFFER_SYNC_TRANSMIT)) {
		uint32_t total = va[RPBUF_TRACE_END] - va[RPBUF_TRACE_START];
		uint32_t callback = va[RPBUF_TRACE_R_S_NOTIFY] - va[RPBUF_TRACE_R_S_CB];
		uint32_t flush = va[RPBUF_TRACE_NOTIFY] - va[RPBUF_TRACE_FLUSH];

		printf("\ttotal    : %u us\n", total);
		printf("\tsys      : %u us\n", total - callback - flush);
		printf("\tcb       : %u us\n", callback);
		printf("\tflush    : %u us\n", flush);
	} else {
		uint32_t total = va[RPBUF_TRACE_END] - va[RPBUF_TRACE_START];
...
static struct rpbuf_buffer *rpbuf_alloc_buffer_instance(const char *name, int len)
{
	...
	buffer->len = len;
	hal_waitqueue_head_init(&buffer->wait);
...
{
	hal_waitqueue_head_deinit(&buffer->wait);
	rpbuf_free(buffer);
}
						     ...
						     struct rpbuf_buffer *buffer)
{
	...
	addr = hal_malloc_coherent(buffer->len);
	if (!addr) {
		rpbuf_err("hal_malloc_coherent for len %d failed\n", buffer->len);
		ret = -ENOMEM;
		goto err_out;
	}
...
#ifdef CONFIG_COMPONENTS_RPBUF_MEASURE_TIME
	buffer->len += RPBUF_MEASURE_TIME_RESERVED;
#endif
								...
								struct rpbuf_buffer *buffer)
{
	...
	hal_wait_event(buffer->wait, buffer->state != BUFFER_WAIT);
							...
							void *content)
{
	...
	if (buffer) {
		...
		if (controller->buffers[cont->id]) {
			rpbuf_err("buffer with id %d already exists\n", cont->id);
			ret = -EINVAL;
			goto unlock_controller_lock;
		}
		controller->buffers[cont->id] = buffer;
		buffer->id = cont->id;

		list_del(&buffer->dummy_list);

		rpbuf_info("buffer \"%s\" (id:%d): local_dummy_buffers -> buffers\n",
			  buffer->name, buffer->id);
		is_available = 1;
	} else {
		/*
		...
		if (buffer) {
			...
			rpbuf_info("buffer \"%s\": NULL -> remote_dummy_buffers\n",
				  buffer->name);
		}
	}
							  ...
							  void *content)
{
	...
	if (buffer) {
		...
		rpbuf_info("buffer \"%s\": remote_dummy_buffers -> NULL\n",
			  buffer->name);
		...
		if (buffer) {
			...
			rpbuf_info("buffer \"%s\" (id:%d): buffers -> local_dummy_buffers\n",
				  buffer->name, cont->id);
		} else {
							    ...
							    void *content)
{
	...
	struct rpbuf_buffer *buffer;
	uint32_t timestamp = trace_rpbuf_get_time();
	...
	rpbuf_time_buffer_invalid(buffer);

	put_trace_rpbuf_time(buffer, RPBUF_TRACE_R_NOTIFY, timestamp);
	...
	trace_rpbuf_time(buffer, RPBUF_TRACE_R_FINISH);

	rpbuf_time_buffer_clean(buffer);
...
static int rpbuf_free_buffer_internal(struct rpbuf_buffer *buffer, int do_notify)
{
	...
	if (dummy_buffer) {
		...
		rpbuf_info("buffer \"%s\": local_dummy_buffers -> NULL\n",
			  buffer->name);
	} else {
		/*
		...
		if (dummy_buffer) {
			...
			rpbuf_info("buffer \"%s\" (id:%d): buffers -> remote_dummy_buffers\n",
				  buffer->name, buffer->id);
		}
	}
	...
	if (dummy_buffer) {
		...
		for (i = 0; i < RPBUF_BUFFERS_NUM_MAX; i++) {
			if (!controller->buffers[i]) {
				controller->buffers[i] = buffer;
				break;
			}
		}
		if (i >= RPBUF_BUFFERS_NUM_MAX) {
		...
		rpbuf_info("buffer \"%s\" (id:%d): remote_dummy_buffers -> buffers\n",
			  buffer->name, buffer->id);
		is_available = 1;
	} else {
		/*
		 ...
		 */
		list_add_tail(&buffer->dummy_list, &controller->local_dummy_buffers);
		rpbuf_info("buffer \"%s\": NULL -> local_dummy_buffers\n",
			  buffer->name);
	}
			  ...
			  unsigned int offset, unsigned int data_len)
{
	...
	trace_rpbuf_time(buffer, RPBUF_TRACE_START);
#if 0
	if (offset + data_len > buffer->len) {
	...
	trace_rpbuf_time(buffer, RPBUF_TRACE_FLUSH);

	rpbuf_dcache_clean(buffer->va + offset, data_len);

	trace_rpbuf_time(buffer, RPBUF_TRACE_NOTIFY);
	...
	trace_rpbuf_time(buffer, RPBUF_TRACE_WAIT_ACK);
	...
	trace_rpbuf_time(buffer, RPBUF_TRACE_END);
#ifdef CONFIG_COMPONENTS_RPBUF_DUMP_TIME_INFO
	dump_rpbuf_time(buffer);
#endif
