...
/* need poorting to different project lds */
...
{
	if ((addr >= (unsigned long)__text_start__ && addr <= (unsigned long)__text_end__))
		return 1;
	return 0;
}
...
{
	#if !(CONFIG_KALLSYMS_BASE_RELATIVE != 0)
		return kallsyms_addresses[idx];
	#endif

	/* values are unsigned offsets if --absolute-percpu is not in effect */
	#if !(CONFIG_KALLSYMS_ABSOLUTE_PERCPU != 0)
				    ...
				    unsigned long *offset)
{
	...
	if (!symbol_end) {
		if (all_var)
			...
			symbol_end = (unsigned long)__text_end__;
	}
...
/* This macro allows us to keep printf typechecking */
