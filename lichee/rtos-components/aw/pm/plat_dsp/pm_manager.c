...
extern int pm_dsp_platops_init(void);
int pm_init(int argc, char **argv)
{
	/* creat timer*/
	pm_wakelocks_init();

	/* before arch initialization of hardware
	pm_wakecnt_init();
	pm_wakesrc_init();
	*/
	...
	/* creat mutex
	 * put pm_devops_init() and pm_syscore_init() 
	 * before arch initialization of hardware devices if necessary 
	 */
	//pm_syscore_init();
