...
{
	return HAL_GET_BIT_VAL(PMU->DCDC_STABLE_REF_TIME, BANDGAP_LDO_STABLE_REFTIME_SHIFT, \
			BANDGAP_LDO_STABLE_REFTIME_VMASK);
}
...
{
	HAL_MODIFY_REG(PMU->DCDC_STABLE_REF_TIME, BANDGAP_LDO_STABLE_REFTIME_MASK,
			(val & BANDGAP_LDO_STABLE_REFTIME_VMASK) << BANDGAP_LDO_STABLE_REFTIME_SHIFT);
}
...
void HAL_PMU_SetWakeupIOxDebounce(uint8_t ioIndex, uint8_t val)
{
	...
	if (ioIndex < 8) {
		...
		HAL_MODIFY_REG(PMU->RTC_IO_WUP_DEB_CYCLES1,
		               PMU_WAKEUP_IO8T15_DEDOUNCE_CYCLE_MASK(ioIndex),
		               val << PMU_WAKEUP_IO8T15_DEDOUNCE_CYCLE_SHIFT(ioIndex));
	}
}
