...
#define WAIT_TIME					(MS_TO_OSTICK(1))
...
#define time_is_before_ktime(a)  time_after(hal_tick_get(), a)
...
static int ringbuffer_put(struct _sh_fifo *fifo, void *from, int size)
{
	...
	if (size > LOG_BUF_SIZE) {
		...
		if (fifo->reading) {
			uint64_t timeout;
			timeout = (uint64_t)(hal_tick_get() + WAIT_TIME);
...
int amp_log_put(char ch)
{
	...
	return ringbuffer_put(fifo, &ch, 1);
}
