...
struct remoteproc* rproc_arch_init(struct remoteproc *rproc,
...
void rproc_arch_remove(struct remoteproc *rproc)
{
...
void *rproc_arch_mmap(struct remoteproc *rproc,
...
int rproc_arch_notify(struct remoteproc *rproc, uint32_t id)
{
	int ret;
...
static struct remoteproc_ops rproc_ops = {
	.init       = rproc_arch_init,
	.remove     = rproc_arch_remove,
	.mmap       = rproc_arch_mmap,
	.handle_rsc = NULL,
	.config     = NULL,
	.start      = NULL,
	.stop       = NULL,
	.shutdown   = NULL,
	.notify     = rproc_arch_notify,
	.get_mem    = NULL,
};

struct rproc_global_impls sunxi_rproc_impls[] = {
	{ .ops = &rproc_ops, .priv = &private_data },
};
