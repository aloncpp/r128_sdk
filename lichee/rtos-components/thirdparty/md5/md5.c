...
int cmd_md5sum(int argc, char ** argv)
{
	...
	for (argc--, argv++; argc; argv++, argc--)
	{
		...
		if (!fp)
		{
			printf("md5sum error:can't open %s\n", *argv);
			return -1;
		}
		MD5Init(&context);
...
FINSH_FUNCTION_EXPORT_CMD(cmd_md5sum, md5sum, Calculate md5sum);
...
int get_md5sum(char *path, unsigned int file_size, unsigned char decrypt[16])
{
	...
	if (path)
		printf("get_md5sum open %s\n", path);

	buffer = (unsigned char *)malloc(MD5SUM_BUFFER_SIZE);
	if (buffer == NULL) {
