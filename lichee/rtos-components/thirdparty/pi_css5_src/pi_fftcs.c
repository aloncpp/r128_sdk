...
#endif
    npow = 4;
    elap_time = (double) second() - (double) start_time ;

    do {
        clock_t start_loop_time = second();
        npow *= 2;
        /* ---- e = (a + b) / 2 ---- */
        ...
        loop_time =
            (double) second() - (double) start_loop_time ;
        elap_time += loop_time;
#ifndef QUIET_OUT
...
#if 0
    sprintf(filename, "/data/pi%i.txt", log10_radix * (n - 2));
...
static uint64_t second(void)
{
    extern uint64_t hal_gettime_ns(void);
    return hal_gettime_ns() / 1000 / 1000 / 1000;
}

#include <console.h>
FINSH_FUNCTION_EXPORT_CMD(ipi_fftcs_main, pi, calculate PI);
