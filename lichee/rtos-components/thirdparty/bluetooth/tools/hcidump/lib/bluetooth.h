...
#define htobs(d)  __bswap16(d)      //functions from sys/endian.h
#define htobl(d)  __bswap32(d)
#define htobll(d) __bswap64(d)
#define btohs(d)  __bswap16(d)
#define btohl(d)  __bswap32(d)
#define btohll(d) __bswap64(d)
#else
...
{
	return __bswap16(bt_get_unaligned((const uint64_t *) ptr));
}
...
{
	return __bswap32(bt_get_unaligned((const uint32_t *) ptr));
}
...
{
	return __bswap16(bt_get_unaligned((const uint16_t *) ptr));
}
...
{
	bt_put_unaligned(__bswap64(val), (uint64_t *) ptr);
}
...
{
	bt_put_unaligned(__bswap32(val), (uint32_t *) ptr);
}
...
{
	bt_put_unaligned(__bswap16(val), (uint16_t *) ptr);
}

#elif __BYTE_ORDER == __BIG_ENDIAN
static inline uint64_t bt_get_le64(const void *ptr)
{
	return __bswap64(bt_get_unaligned((const uint64_t *) ptr));
}
