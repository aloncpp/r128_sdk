...
static const char *si2str(uint8_t si)
{
	switch (si & 0x7f) {
	case 0x01:
		return "Discover";
...
static const char *media2str(uint8_t type)
{
	switch (type) {
	case 0:
		return "Audio";
...
static const char *codec2str(uint8_t type, uint8_t codec)
{
	switch (type) {
	case 0:
		switch (codec) {
...
static const char *cat2str(uint8_t cat)
{
	switch (cat) {
	case 1:
...
void avdtp_dump(int level, struct frame *frm)
{
	...
	case 1:
		...
		hdr = get_u8(frm);
#if 0
    //judge signal or media
    if (((hdr >> 6) == 2) && (hdr == 0x80) && (num == 1)) {
        return;
    }
#endif
