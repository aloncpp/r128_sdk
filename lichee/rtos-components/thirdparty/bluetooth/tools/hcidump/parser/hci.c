...
#define LE_EV_NUM 5
static const char *ev_le_meta_str[LE_EV_NUM + 1] = {
	"Unknown",
...
static const char *cmd_linkctl_str[CMD_LINKCTL_NUM + 1] = {
	"Unknown",
	"Inquiry",
	"Inquiry Cancel",
...
static const char *cmd_linkpol_str[CMD_LINKPOL_NUM + 1] = {
	"Unknown",
	"Hold Mode",
	"Unknown",
	"Sniff Mode",
...
static const char *cmd_hostctl_str[CMD_HOSTCTL_NUM + 1] = {
	"Unknown",
	"Set Event Mask",
	"Unknown",
	"Reset",
...
static const char *cmd_info_str[CMD_INFO_NUM + 1] = {
	"Unknown",
...
static const char *cmd_status_str[CMD_STATUS_NUM + 1] = {
	"Unknown",
...
static const char *cmd_testing_str[CMD_TESTING_NUM + 1] = {
	"Unknown",
	"Read Loopback Mode",
	"Write Loopback Mode",
...
static const char *cmd_le_str[CMD_LE_NUM + 1] = {
	"Unknown",
	"LE Set Event Mask",
	"LE Read Buffer Size",
...
{
	bdaddr_t *bdaddr = (bdaddr_t *)frm->ptr;
	char addr[18];
...
{
    inquiry_cp *cp = (inquiry_cp *)frm->ptr;

    p_indent(level, frm);
    printf("[inquiry] lap:0x%2.2x%2.2x%2.2x len:%d num:%d\n",
        cp->lap[2], cp->lap[1], cp->lap[0], cp->length, cp->num_rsp);
}

static inline void periodic_inquiry_dump(int level, struct frame *frm)
{
	periodic_inquiry_cp *cp = (periodic_inquiry_cp *)frm->ptr;
...
{
    create_conn_cp *cp = (create_conn_cp *)frm->ptr;
    uint16_t ptype = btohs(cp->pkt_type);
    uint16_t clkoffset = btohs(cp->clock_offset);
    char addr[18];

    p_ba2str(&cp->bdaddr, addr);
    printf("[create_conn] bdaddr:%s ptype:0x%4.4x rswitch:0x%2.2x clkoffset:0x%4.4x%s\n",
...
{
    disconnect_cp *cp = (disconnect_cp *)frm->ptr;

    p_indent(level, frm);
    printf("[disconnect] handle:%d reason:%s\n", btohs(cp->handle), status2str(cp->reason));

}

static inline void add_sco_dump(int level, struct frame *frm)
{
	add_sco_cp *cp = (add_sco_cp *)frm->ptr;
	uint16_t ptype = btohs(cp->pkt_type);
	char *str;
...
{
    accept_conn_req_cp *cp = (accept_conn_req_cp *)frm->ptr;
    char addr[18];

    p_indent(level, frm);
    p_ba2str(&cp->bdaddr, addr);
	printf("[accept_conn_req] bdaddr:%s role:%s\n", addr, role2str(cp->role));
}

static inline void reject_conn_req_dump(int level, struct frame *frm)
{
	reject_conn_req_cp *cp = (reject_conn_req_cp *)frm->ptr;
	char addr[18];
...
{
	pin_code_reply_cp *cp = (pin_code_reply_cp *)frm->ptr;
	char addr[18], pin[17];
...
{
	link_key_reply_cp *cp = (link_key_reply_cp *)frm->ptr;
	char addr[18];
	int i;
...
{
	user_passkey_reply_cp *cp = (user_passkey_reply_cp *)frm->ptr;
	char addr[18];
...
{
	remote_oob_data_reply_cp *cp = (remote_oob_data_reply_cp *)frm->ptr;
	char addr[18];
	int i;
...
{
	io_capability_reply_cp *cp = (io_capability_reply_cp *)frm->ptr;
	char addr[18];
...
{
	set_conn_encrypt_cp *cp = (set_conn_encrypt_cp *)frm->ptr;
...
{
	remote_name_req_cp *cp = (remote_name_req_cp *)frm->ptr;
	uint16_t clkoffset = btohs(cp->clock_offset);
	char addr[18];
...
{
	master_link_key_cp *cp = (master_link_key_cp *)frm->ptr;
...
{
	read_remote_ext_features_cp *cp = (read_remote_ext_features_cp *)frm->ptr;
...
{
	setup_sync_conn_cp *cp = (setup_sync_conn_cp *)frm->ptr;
...
{
	create_physical_link_cp *cp = (create_physical_link_cp *)frm->ptr;
	int i;
...
{
	create_logical_link_cp *cp = (create_logical_link_cp *)frm->ptr;
	int i;
...
{
	hold_mode_cp *cp = (hold_mode_cp *)frm->ptr;
...
{
    sniff_mode_cp *cp = (sniff_mode_cp *)frm->ptr;

    p_indent(level, frm);
    printf("[sniff_mode] handle:%d max:%d min:%d attempt:%d timeout:%d\n",
...
{
	qos_setup_cp *cp = (qos_setup_cp *)frm->ptr;
...
{
	write_link_policy_cp *cp = (write_link_policy_cp *)frm->ptr;
...
{
	sniff_subrating_cp *cp = (sniff_subrating_cp *)frm->ptr;
...
{
	set_event_mask_cp *cp = (set_event_mask_cp *)frm->ptr;
	int i;
...
{
	set_event_flt_cp *cp = (set_event_flt_cp *)frm->ptr;
...
{
	write_pin_type_cp *cp = (write_pin_type_cp *)frm->ptr;
...
{
	read_stored_link_key_cp *cp = (read_stored_link_key_cp *)frm->ptr;
	char addr[18];
...
{
	change_local_name_cp *cp = (change_local_name_cp *)frm->ptr;
	char name[249];
	int i;
...
{
	write_class_of_dev_cp *cp = (write_class_of_dev_cp *)frm->ptr;
...
{
	write_voice_setting_cp *cp = (write_voice_setting_cp *)frm->ptr;
...
{
	write_current_iac_lap_cp *cp = (write_current_iac_lap_cp *)frm->ptr;
	int i;
...
{
	write_page_timeout_cp *cp = (write_page_timeout_cp *)frm->ptr;
...
{
	write_page_activity_cp *cp = (write_page_activity_cp *)frm->ptr;
...
{
	write_inquiry_scan_type_cp *cp = (write_inquiry_scan_type_cp *)frm->ptr;
...
{
	write_inquiry_mode_cp *cp = (write_inquiry_mode_cp *)frm->ptr;
...
{
	set_afh_classification_cp *cp = (set_afh_classification_cp *)frm->ptr;
	int i;
...
{
	write_link_supervision_timeout_cp *cp = (write_link_supervision_timeout_cp *)frm->ptr;
...
{
	write_ext_inquiry_response_cp *cp = (write_ext_inquiry_response_cp *)frm->ptr;
...
{
	write_inquiry_transmit_power_level_cp *cp = (write_inquiry_transmit_power_level_cp *)frm->ptr;
...
{
	write_default_error_data_reporting_cp *cp = (write_default_error_data_reporting_cp *)frm->ptr;
...
{
	enhanced_flush_cp *cp = (enhanced_flush_cp *)frm->ptr;
...
{
	send_keypress_notify_cp *cp = (send_keypress_notify_cp *)frm->ptr;
	char addr[18];
...
{
	read_transmit_power_level_cp *cp = (read_transmit_power_level_cp *)frm->ptr;
...
{
	read_local_ext_features_cp *cp = (read_local_ext_features_cp *)frm->ptr;
...
{
	read_clock_cp *cp = (read_clock_cp *)frm->ptr;
...
{
	host_buffer_size_cp *cp = (host_buffer_size_cp *)frm->ptr;
...
{
	char addr[18];
	le_create_connection_cp *cp = (le_create_connection_cp *)frm->ptr;
...
{
	int i;
	le_set_event_mask_cp *cp = (le_set_event_mask_cp *)frm->ptr;
...
{
	char addr[18];
	le_set_random_address_cp *cp = (le_set_random_address_cp *)frm->ptr;
...
{
	char addr[18];
	le_set_advertising_parameters_cp *cp = (le_set_advertising_parameters_cp *)frm->ptr;
...
{
	le_set_scan_parameters_cp *cp = (le_set_scan_parameters_cp *)frm->ptr;
...
{
	le_set_scan_enable_cp *cp = (le_set_scan_enable_cp *)frm->ptr;
...
static inline void write_remote_amp_assoc_cmd_dump(int level,
							struct frame *frm)
{
	write_remote_amp_assoc_cp *cp = (write_remote_amp_assoc_cp *)frm->ptr;
...
{
	hci_command_hdr *hdr = (hci_command_hdr *)frm->ptr;
    ...
    case OGF_LINK_CTL:
        ...
        case OCF_CREATE_CONN:
            create_conn_dump(level, frm);
            return;
        case OCF_DISCONNECT:
            disconnect_dump(level, frm);
            return;
        ...
        case OCF_ACCEPT_CONN_REQ:
            accept_conn_req_dump(level, frm);
            return;
...
{
	read_data_block_size_rp *rp = (read_data_block_size_rp *)frm->ptr;
...
{
	read_link_policy_rp *rp = (read_link_policy_rp *)frm->ptr;
...
{
	read_pin_type_rp *rp = (read_pin_type_rp *)frm->ptr;
...
{
	read_stored_link_key_rp *rp = (read_stored_link_key_rp *)frm->ptr;
...
{
	write_stored_link_key_rp *rp = (write_stored_link_key_rp *)frm->ptr;
...
{
	delete_stored_link_key_rp *rp = (delete_stored_link_key_rp *)frm->ptr;
...
{
	read_local_name_rp *rp = (read_local_name_rp *)frm->ptr;
	char name[249];
	int i;
...
{
	read_class_of_dev_rp *rp = (read_class_of_dev_rp *)frm->ptr;
...
{
	read_voice_setting_rp *rp = (read_voice_setting_rp *)frm->ptr;
...
{
	read_current_iac_lap_rp *rp = (read_current_iac_lap_rp *)frm->ptr;
	int i;
...
{
	read_page_timeout_rp *rp = (read_page_timeout_rp *)frm->ptr;
...
{
	read_page_activity_rp *rp = (read_page_activity_rp *)frm->ptr;
...
{
	read_inquiry_scan_type_rp *rp = (read_inquiry_scan_type_rp *)frm->ptr;
...
{
	read_inquiry_mode_rp *rp = (read_inquiry_mode_rp *)frm->ptr;
...
{
	read_link_supervision_timeout_rp *rp = (read_link_supervision_timeout_rp *)frm->ptr;
...
{
	read_transmit_power_level_rp *rp = (read_transmit_power_level_rp *)frm->ptr;
...
{
	read_ext_inquiry_response_rp *rp = (read_ext_inquiry_response_rp *)frm->ptr;
...
{
	read_inquiry_transmit_power_level_rp *rp = (read_inquiry_transmit_power_level_rp *)frm->ptr;
...
{
	read_default_error_data_reporting_rp *rp = (read_default_error_data_reporting_rp *)frm->ptr;
...
{
	read_local_oob_data_rp *rp = (read_local_oob_data_rp *)frm->ptr;
	int i;
...
{
	read_local_version_rp *rp = (read_local_version_rp *)frm->ptr;
...
{
	read_local_commands_rp *rp = (read_local_commands_rp *)frm->ptr;
	int i, max = 0;
...
{
	read_local_features_rp *rp = (read_local_features_rp *)frm->ptr;
	int i;
...
{
	read_local_ext_features_rp *rp = (read_local_ext_features_rp *)frm->ptr;
	int i;
...
{
	read_buffer_size_rp *rp = (read_buffer_size_rp *)frm->ptr;
...
{
	read_link_quality_rp *rp = (read_link_quality_rp *)frm->ptr;
...
{
	read_rssi_rp *rp = (read_rssi_rp *)frm->ptr;
...
{
	read_afh_map_rp *rp = (read_afh_map_rp *)frm->ptr;
	int i;
...
{
	read_clock_rp *rp = (read_clock_rp *)frm->ptr;
...
{
	read_local_amp_info_rp *rp = (read_local_amp_info_rp *)frm->ptr;
...
{
	read_local_amp_assoc_rp *rp = (read_local_amp_assoc_rp *)frm->ptr;
...
{
	write_remote_amp_assoc_rp *rp = (write_remote_amp_assoc_rp *)frm->ptr;
...
{
	le_read_buffer_size_rp *rp = (le_read_buffer_size_rp *)frm->ptr;
...
{
	int i;
	le_read_local_supported_features_rp *rp = (le_read_local_supported_features_rp *)frm->ptr;
...
{
	le_read_advertising_channel_tx_power_rp *rp = (le_read_advertising_channel_tx_power_rp *)frm->ptr;
...
{
	evt_cmd_complete *evt = (evt_cmd_complete *)frm->ptr;
...
{
	evt_cmd_status *evt = (evt_cmd_status *)frm->ptr;
	uint16_t opcode = btohs(evt->opcode);
	uint16_t ogf = cmd_opcode_ogf(opcode);
	uint16_t ocf = cmd_opcode_ocf(opcode);
...
{
	evt_hardware_error *evt = (evt_hardware_error *)frm->ptr;
...
static inline void inq_result_dump(int level, struct frame *frm)
{
	...
	for (i = 0; i < num; i++) {
		inquiry_info *info = (inquiry_info *)frm->ptr;
...
{
	evt_conn_complete *evt = (evt_conn_complete *)frm->ptr;
	char addr[18];

	p_ba2str(&evt->bdaddr, addr);

	p_indent(level, frm);
	printf("[conn_complete] bdaddr:%s handle:0x%02x type:%s encrypt:0x%2.2x status:%s \n",
			addr, btohs(evt->handle), linktype2str(evt->link_type), evt->encr_mode, status2str(evt->status));
}

static inline void conn_request_dump(int level, struct frame *frm)
{
	evt_conn_request *evt = (evt_conn_request *)frm->ptr;
	char addr[18];

	p_ba2str(&evt->bdaddr, addr);

	p_indent(level, frm);
	printf("[conn_request] bdaddr:%s class:0x%2.2x%2.2x%2.2x type:%s\n",
...
{
	evt_disconn_complete *evt = (evt_disconn_complete *)frm->ptr;

	p_indent(level, frm);
	printf("[disconn_complete] status:%s handle:0x%02x reason:%s\n",
				status2str(evt->status), (evt->handle),  status2str(evt->reason));
}

static inline void remote_name_req_complete_dump(int level, struct frame *frm)
{
	evt_remote_name_req_complete *evt = (evt_remote_name_req_complete *)frm->ptr;
...
{
	evt_master_link_key_complete *evt = (evt_master_link_key_complete *)frm->ptr;
...
{
	evt_encrypt_change *evt = (evt_encrypt_change *)frm->ptr;
...
{
	evt_read_remote_features_complete *evt = (evt_read_remote_features_complete *)frm->ptr;
	int i;
...
{
	evt_read_remote_version_complete *evt = (evt_read_remote_version_complete *)frm->ptr;
...
{
	evt_qos_setup_complete *evt = (evt_qos_setup_complete *)frm->ptr;
...
{
	evt_role_change *evt = (evt_role_change *)frm->ptr;
	char addr[18];

	p_ba2str(&evt->bdaddr, addr);

	p_indent(level, frm);
	printf("[role_change] bdaddr:%s role:%s handle:0x%02x status:%s \n",
		addr, role2str(evt->role), frm->handle,
		status2str(evt->status));
}

static inline void mode_change_dump(int level, struct frame *frm)
{
	evt_mode_change *evt = (evt_mode_change *)frm->ptr;
	p_indent(level, frm);
	printf("[mode_change] handle:0x%02x mode:%s status:%s \n",
		frm->handle, mode2str(evt->mode), status2str(evt->status));
}

static inline void pin_code_req_dump(int level, struct frame *frm)
{
	evt_pin_code_req *evt = (evt_pin_code_req *)frm->ptr;
	char addr[18];
...
{
	evt_link_key_notify *evt = (evt_link_key_notify *)frm->ptr;
	char addr[18];
	int i;
...
{
	evt_max_slots_change *evt = (evt_max_slots_change *)frm->ptr;
...
{
	evt_data_buffer_overflow *evt = (evt_data_buffer_overflow *)frm->ptr;
...
{
	evt_read_clock_offset_complete *evt = (evt_read_clock_offset_complete *)frm->ptr;
...
{
	evt_conn_ptype_changed *evt = (evt_conn_ptype_changed *)frm->ptr;
...
{
	evt_pscan_rep_mode_change *evt = (evt_pscan_rep_mode_change *)frm->ptr;
	char addr[18];
...
{
	evt_flow_spec_complete *evt = (evt_flow_spec_complete *)frm->ptr;
...
static inline void inq_result_with_rssi_dump(int level, struct frame *frm)
{
	...
	if (frm->len / num == INQUIRY_INFO_WITH_RSSI_AND_PSCAN_MODE_SIZE) {
		for (i = 0; i < num; i++) {
			inquiry_info_with_rssi_and_pscan_mode *info = (inquiry_info_with_rssi_and_pscan_mode *)frm->ptr;
		...
		for (i = 0; i < num; i++) {
			inquiry_info_with_rssi *info = (inquiry_info_with_rssi *)frm->ptr;
...
{
	evt_read_remote_ext_features_complete *evt = (evt_read_remote_ext_features_complete *)frm->ptr;
	int i;
...
{
	evt_sync_conn_complete *evt = (evt_sync_conn_complete *)frm->ptr;
	char addr[18];
...
{
	evt_sync_conn_changed *evt = (evt_sync_conn_changed *)frm->ptr;
...
{
	evt_sniff_subrating *evt = (evt_sniff_subrating *)frm->ptr;
...
static inline void extended_inq_result_dump(int level, struct frame *frm)
{
	...
	for (i = 0; i < num; i++) {
		extended_inquiry_info *info = (extended_inquiry_info *)frm->ptr;
...
{
	evt_link_supervision_timeout_changed *evt = (evt_link_supervision_timeout_changed *)frm->ptr;
...
{
	evt_user_passkey_notify *evt = (evt_user_passkey_notify *)frm->ptr;
	char addr[18];
...
{
	evt_keypress_notify *evt = (evt_keypress_notify *)frm->ptr;
	char addr[18];
...
{
	evt_remote_host_features_notify *evt = (evt_remote_host_features_notify *)frm->ptr;
	char addr[18];
	int i;
...
{
	evt_le_connection_complete *evt = (evt_le_connection_complete *)frm->ptr;
	char addr[18];
...
static inline void evt_le_conn_update_complete_dump(int level,
							struct frame *frm)
{
	evt_le_connection_update_complete *uevt = (evt_le_connection_update_complete *)frm->ptr;
...
{
	int i;
	evt_le_read_remote_used_features_complete *revt = (evt_le_read_remote_used_features_complete *)frm->ptr;
...
{
	evt_physical_link_complete *evt = (evt_physical_link_complete *)frm->ptr;
...
{
	evt_disconn_physical_link_complete *evt = (evt_disconn_physical_link_complete *)frm->ptr;
...
{
	evt_physical_link_loss_warning *evt = (evt_physical_link_loss_warning *)frm->ptr;
...
{
	evt_physical_link_recovery *evt = (evt_physical_link_recovery *)frm->ptr;
...
{
	evt_logical_link_complete *evt = (evt_logical_link_complete *)frm->ptr;
...
{
	evt_flow_spec_modify_complete *evt = (evt_flow_spec_modify_complete *)frm->ptr;
...
{
	evt_num_completed_blocks *evt = (evt_num_completed_blocks *)frm->ptr;
	int i;
...
{
	hci_acl_hdr *hdr = (hci_acl_hdr *) frm->ptr;
...
{
	hci_sco_hdr *hdr = (hci_sco_hdr *) frm->ptr;
	uint16_t handle = btohs(hdr->handle);
	uint8_t flags = acl_flags(handle);
	int len;
