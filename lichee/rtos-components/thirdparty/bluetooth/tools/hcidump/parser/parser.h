...
#define XRADIODUMP_DBG FALSE
#if 0
#define L2CAP_TRACE(param, ...) {printf(param, ## __VA_ARGS__);}
#define AVDTP_TRACE(param, ...) {printf(param, ## __VA_ARGS__);}
#else
#define L2CAP_TRACE(param, ...) {}
#define AVDTP_TRACE(param, ...) {}
#endif
	...
	if (!parser.state) {
		if (parser.flags & DUMP_TSTAMP) {
			if (parser.flags & DUMP_VERBOSE) {
				...
				printf("%8llu.%06lu ", f->ts.tv_sec, f->ts.tv_usec);
		}
		printf("%c ", (f->in ? '>' : '<'));
		parser.state = 1;
	} else 
