...
static const struct features l2cap_features[] = {
...
static const struct features l2cap_fix_chan[] = {
...
static void del_cid(int in, uint16_t dcid, uint16_t scid)
{
	...
	//L2CAP_TRACE("[del]==del_cid handle:0x%02x scid:0x%02x, dcid:0x%02x, in:%d\n", handle , scid, dcid ,in);

	for (t = 0; t < 2; t++) {
		for (i = 0; i < CID_TABLE_SIZE; i++)
			if (cid_table[t][i].cid == cid[t]) {
				//L2CAP_TRACE("[del cid]del handle:0x%02x cid :0x%02x done\n", cid_table[t][i].handle, cid[t]);
	...
	for (t = 0; t < 2; t++) {
		for (i = 0; i < CID_TABLE_SIZE; i++)
			if (cid_table[t][i].handle == handle) {
				//L2CAP_TRACE("[del handle]del handle:0x%02x cid :0x%02x done\n", cid_table[t][i].handle, cid_table[t][i].cid);
				cid_table[t][i].handle = 0;
				cid_table[t][i].cid    = 0;
				cid_table[t][i].psm    = 0;
				cid_table[t][i].num    = 0;
...
{
	l2cap_cmd_rej *h = (l2cap_cmd_rej *)frm->ptr;
...
{
	l2cap_conn_req *h = (l2cap_conn_req *)frm->ptr;
...
{
    l2cap_conn_rsp *h = (l2cap_conn_rsp *)frm->ptr;
	uint16_t scid = btohs(h->scid);
...
{
	l2cap_conf_req *h = (l2cap_conf_req *)frm->ptr;
	uint16_t dcid = btohs(h->dcid);
...
{
	l2cap_conf_rsp *h = (l2cap_conf_rsp *)frm->ptr;
	uint16_t scid = btohs(h->scid);
	uint16_t result = btohs(h->result);
...
{
	l2cap_disconn_req *h = (l2cap_disconn_req *)frm->ptr;
...
{
	l2cap_disconn_rsp *h = (l2cap_disconn_rsp *)frm->ptr;
	uint16_t dcid = btohs(h->dcid);
	uint16_t scid = btohs(h->scid);

	//if ((psm = get_psm(!frm->in, frm->handle, scid)) == 0x19)
	del_cid(frm->in, dcid, scid);
...
{
	l2cap_info_req *h = (l2cap_info_req *)frm->ptr;
...
{
	l2cap_info_rsp *h = (l2cap_info_rsp *)frm->ptr;
...
{
	l2cap_create_req *h = (l2cap_create_req *)frm->ptr;
	uint16_t psm = btohs(h->psm);
	uint16_t scid = btohs(h->scid);
...
{
	l2cap_create_rsp *h = (l2cap_create_rsp *)frm->ptr;
	uint16_t scid = btohs(h->scid);
	uint16_t dcid = btohs(h->dcid);
	uint16_t result = btohs(h->result);
	uint16_t status = btohs(h->status);
...
{
	l2cap_move_req *h = (l2cap_move_req *)frm->ptr;
...
{
	l2cap_move_rsp *h = (l2cap_move_rsp *)frm->ptr;
	uint16_t icid = btohs(h->icid);
	uint16_t result = btohs(h->result);
...
{
	l2cap_move_cfm *h = (l2cap_move_cfm *)frm->ptr;
	uint16_t icid = btohs(h->icid);
	uint16_t result = btohs(h->result);
...
{
	l2cap_move_cfm_rsp *h = (l2cap_move_cfm_rsp *)frm->ptr;
	uint16_t icid = btohs(h->icid);
...
{
	struct a2mp_command_rej *h = (struct a2mp_command_rej *)frm->ptr;
	uint16_t reason = btohs(h->reason);
...
{
	struct a2mp_discover_req *h = (struct a2mp_discover_req *)frm->ptr;
...
{
	struct a2mp_discover_rsp *h = (struct a2mp_discover_rsp *)frm->ptr;
	uint16_t mtu = btohs(h->mtu);
	uint8_t	 *octet = (uint8_t *)&(h->mask);
	uint16_t mask;
	uint8_t  extension;
...
{
	struct a2mp_ctrl *list = (struct a2mp_ctrl *)frm->ptr;
...
{
	struct a2mp_info_req *h = (struct a2mp_info_req *)frm->ptr;
...
{
	struct a2mp_info_rsp *h = (struct a2mp_info_rsp *)frm->ptr;
...
{
	struct a2mp_assoc_req *h = (struct a2mp_assoc_req *)frm->ptr;
...
{
	struct a2mp_assoc_rsp *h = (struct a2mp_assoc_rsp *)frm->ptr;
...
{
	struct a2mp_create_req *h = (struct a2mp_create_req *)frm->ptr;
...
{
	struct a2mp_create_rsp *h = (struct a2mp_create_rsp *)frm->ptr;
...
{
	struct a2mp_disconn_req *h = (struct a2mp_disconn_req *)frm->ptr;
...
{
	struct a2mp_disconn_rsp *h = (struct a2mp_disconn_rsp *)frm->ptr;
...
static void l2cap_parse(int level, struct frame *frm)
{
	...
	if (cid == 0x1) {
		...
		while (frm->len >= L2CAP_CMD_HDR_SIZE) {
			l2cap_cmd_hdr *hdr = (l2cap_cmd_hdr *)frm->ptr;
		...
		while (frm->len >= A2MP_HDR_SIZE) {
			struct a2mp_hdr *hdr = (struct a2mp_hdr *)frm->ptr;
...
void l2cap_dump(int level, struct frame *frm)
{
	...
	if ((frm->flags & ACL_START) || frm->flags == ACL_START_NO_FLUSH) {
		...
		if (!(fr->data = malloc(dlen + L2CAP_HDR_SIZE))) {
			...
			printf("[hcidump_xr] Error: Can't allocate L2CAP reassembly buffer");
			return;
		}
