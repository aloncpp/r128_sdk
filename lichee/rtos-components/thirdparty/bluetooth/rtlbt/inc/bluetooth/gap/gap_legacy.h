...
/**
 * \defgroup    GAP_LEGACY       GAP Legacy
 *
...
typedef enum
{
    ...
    GAP_RADIO_OFF                       = 0x06   /**< Radio off mode. */
} T_GAP_RADIO_MODE;
...
typedef struct
{
    uint16_t    cause;      /**< Result of change a SCO/eSCO link  */
    uint16_t    handle;     /**< Connection handle of the SCO/eSCO link. */
...
typedef enum
{
    ...
    GAP_ACL_LINK_KEY_NOTIF     = 0x12   /**< ACL link key notification event is received. */
} T_GAP_ACL_STATUS;
...
} T_GAP_SYNC_TRAIN_RCV_CMPL_INFO;
...
#define LEGACY_MSG_GROUP_CSB            0x0600
...
typedef enum
{
    ...
    LINK_KEY_CHANGE_INFO  = (LEGACY_MSG_GROUP_AUTHEN | 0x09),   /**< Information of link key change. Message data is \ref T_GAP_LINK_KEY_CHANGE_INFO. */
    ...
    L2C_DATA_RSP         = (LEGACY_MSG_GROUP_ACL | 0x0B),   /**< Response of L2CAP transmission data with ack flag set. No message data. */
    ...
    SET_LT_ADDR_RSP               = (LEGACY_MSG_GROUP_CSB | 0x00),
    DEL_LT_ADDR_RSP               = (LEGACY_MSG_GROUP_CSB | 0x01),
    SET_SYNC_TRAIN_PARAM_RSP      = (LEGACY_MSG_GROUP_CSB | 0x02),
    SYNC_TRAIN_RSP                = (LEGACY_MSG_GROUP_CSB | 0x03),
    SYNC_TRAIN_CMPL               = (LEGACY_MSG_GROUP_CSB | 0x04),
    SYNC_TRAIN_RCV_RSP            = (LEGACY_MSG_GROUP_CSB | 0x05),
    SYNC_TRAIN_RCV_CMPL_INFO      = (LEGACY_MSG_GROUP_CSB | 0x06),
    SET_CSB_RSP                   = (LEGACY_MSG_GROUP_CSB | 0x07),
    SET_CSB_RCV_RSP               = (LEGACY_MSG_GROUP_CSB | 0x08),
    CSB_TOUT_INFO                 = (LEGACY_MSG_GROUP_CSB | 0x09),
    CSB_DATA_RSP                  = (LEGACY_MSG_GROUP_CSB | 0x0A),
    CSB_DATA_IND                  = (LEGACY_MSG_GROUP_CSB | 0x0B),
    CSB_TX_CMPL_INFO              = (LEGACY_MSG_GROUP_CSB | 0x0C),
    CSB_SET_RBA_ENABLE_OPTION_RSP = (LEGACY_MSG_GROUP_CSB | 0x0D),

    VENDOR_EVT             = (LEGACY_MSG_GROUP_VND | 0x00),
 ...
 * gap_legacy.h
 *
 * \brief    Register a SDP record to BT stack. <b>SDP records should be registered
 *           before start BT stack by \ref gap_start_bt_stack.</b>
 *
 ...
 *
 * \return    Result of registering SDP record.
 * \retval    true          SDP record was registered succesfully.
 * \retval    false         SDP record was failed to register.
 *
 * <b>Example usage</b>
   \code{.c}
 ...
 * \ingroup  GAP_LEGACY
 */
bool legacy_add_sdp_record(void *p_buf);
 ...
 *
 * \param[in]  handle               Connection handle of the SCO/eSCO link. The handle can be get in \ref SCO_CONN_CMPL message.
 *
 ...
 * \ingroup  GAP_LEGACY
 */
T_GAP_CAUSE legacy_send_sco_disconn_req(uint16_t handle);
       ...
       case SCO_DISCONN_IND:
           {
               ...
               legacy_send_sco_disconn_cfm(p_ind->handle);
           }
           break;
 ...
 * \ingroup  GAP_LEGACY
 */
T_GAP_CAUSE legacy_send_sco_disconn_cfm(uint16_t handle);
       ...
       case SCO_CONN_CMPL:
           {
               ...
               {
                   APP_PRINT_TRACE1("SCO connection success: handle 0x%04x", p_info->handle);
                   legacy_send_sco_param_change_req(p_info->handle, 8000, 8000, 7, 0x0360, 0, GAP_PKT_TYPE_EV3 | GAP_PKT_TYPE_EV4 | GAP_PKT_TYPE_EV5);
               }
           }
           break;
 ...
 * \ingroup  GAP_LEGACY
 */
T_GAP_CAUSE legacy_send_sco_param_change_req(uint16_t handle, uint32_t tx_bandwidth,
       ...
       case SCO_CONN_CMPL:
           {
               ...
               if (!p_info->cause)
               {
                   ...
                   if (p_buffer)
                   {
                       ...
                       legacy_send_sco_data(p_info->handle, p_buffer, 60);
                   }
               }
           }
           break;
 ...
 * \ingroup  GAP_LEGACY
 */
T_GAP_CAUSE legacy_send_sco_data(uint16_t handle, void *p_buf, uint8_t length);
...
T_GAP_CAUSE legacy_set_csb(uint8_t enable, uint8_t lt_addr, uint8_t lpo_allowed, uint16_t pkt_type,
                           uint16_t min_interval, uint16_t max_interval, uint16_t csb_supv_tout);

T_GAP_CAUSE legacy_set_csb_rcv(uint8_t enable, uint8_t *bd_addr, uint8_t lt_addr, uint16_t interval,
...
T_GAP_CAUSE legacy_send_csb_data(uint8_t lt_addr, void *p_data, uint16_t length);

T_GAP_CAUSE legacy_set_csb_rba_enable_option(uint8_t *p_data, uint8_t len);
...
T_GAP_CAUSE legacy_vendor_multi_link_policy(uint8_t policy, uint16_t value);
#endif
#ifdef __cplusplus
}
#endif    /*  __cplusplus */
