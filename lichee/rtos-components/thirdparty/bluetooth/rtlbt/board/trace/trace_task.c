...
#define TRACE_QUEUE_LENGTH_EVENT      0xf0
...
#define TRACE_TASK_PRIO  (0)

    os_msg_queue_create(&trace.QueueHandleEvent, TRACE_QUEUE_LENGTH_EVENT, sizeof(uint8_t));
    os_task_create(&trace.handle, "trace_task", trace_task, NULL, 1024*4, TRACE_TASK_PRIO);   /*need up priority */
...
bool trace_print(void *pData, uint16_t Length)
{
    ...
    else
    {
      //  printf("\r\ntrace_print:buffer is full, need %d, left %d\r\n", Length,CycQueueRemainSize());
	      printf("\r\n!!!!!!!trace_print:buffer is full,, need %d, left %d\r\npread: %d, pWrite :%d\r\n", Length,CycQueueRemainSize(), pRead, pWrite);
