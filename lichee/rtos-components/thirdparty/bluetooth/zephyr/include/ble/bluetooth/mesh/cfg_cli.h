 ...
 *  @param net_idx     Network index to encrypt with.
 *  @param addr        Target node address.
 *  @param key_net_idx Network key index the new net key belongs to.
 *  @param net_key     New network key.
 *  @param status      Status response parameter.
 *
 *  @return 0 on success, or (negative) error code on failure.
 */
...
/** @brief Get the current key phase of the target node.
 *
 *  @param net_idx     Network index to encrypt with.
 *  @param addr        Target node address.
 *  @param idx         Network key index the new net key belongs to.
 *  @param net_key     Target node network key idx.
 *  @param pahse       Target node current network key phase.
 *  @param status      Status response parameter.
 *
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_key_refresh_phase_get(uint16_t net_idx, uint16_t addr, uint16_t idx,
			    uint16_t *netkey_idx, uint8_t *phase, uint8_t *status);

/** @brief Set the current key phase of the target node.
 *
 *  @param net_idx     Network index to encrypt with.
 *  @param addr        Target node address.
 *  @param idx         Network key index the new net key belongs to.
 *  @param transition  The phase need to be changed to .
 *  @param netkey_idx  Target node network key idx.
 *  @param phase       Target node current network key phase.
 *  @param status      Status response parameter.
 *
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_key_refresh_phase_set(uint16_t net_idx, uint16_t addr, uint16_t idx,
			    uint8_t transition, uint16_t *netkey_idx, uint8_t *phase, uint8_t *status);
