...
#define AUTHORITY (S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH)
				   ...
				   void *cb_arg, bool filter_duplicates)
{
	...
	fd = open(cf->cf_name, O_RDWR | O_SYNC | O_CREAT);
	if (fd == -1) {
		return -EINVAL;
	}
...
static int32_t settings_file_create_or_replace(int *fd,
					   const char *file_name)
{
	...
	*fd = open(file_name, O_RDWR | O_SYNC | O_CREAT);
	if (*fd == -1) {
		return -EINVAL;
	}
			   ...
			   size_t val_len)
{
	...
	read_fd = open(cf->cf_name, O_RDWR | O_SYNC | O_CREAT);
	if (read_fd == -1) {
		return -EINVAL;
	}
	settings_tmpfile(tmp_file, cf->cf_name, ".cmp");

	if (settings_file_create_or_replace(&write_fd, tmp_file)) {
		close(read_fd);
		return -ENOEXEC;
	}
	...
	rc = close(write_fd);
	rc2 = close(read_fd);
	rc3 = remove(cf->cf_name);
	if (rc == 0 && rc2 == 0 && rc3 == 0) {
		rc3 = rename(tmp_file, cf->cf_name);
		if (rc3) {
			return -ENOENT;
		}
		cf->cf_lines = lines + 1;
...
static int32_t read_handler(void *ctx, off_t off, char *buf, size_t *len)
{
	...
	rc = lseek(*fd, entry_ctx->seek + off, SEEK_SET);
	if (rc < 0) {
		goto end;
	}
...
int32_t settings_backend_init(void)
{
	...
	mkdir(CONFIG_SETTINGS_FS_DIR, AUTHORITY);
