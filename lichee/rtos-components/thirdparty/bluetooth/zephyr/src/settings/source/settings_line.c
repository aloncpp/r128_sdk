...
#define LOG_MODULE_NAME settings_line
...
void settings_line_io_deinit(void)
{
	settings_io_cb.read_cb = NULL;
	settings_io_cb.write_cb = NULL;
	settings_io_cb.get_len_cb = NULL;
	settings_io_cb.rwbs = 0;
}
#endif
			  ...
			  void *cb_arg)
{
	...
	return settings_call_set_handler(name, len, (void *)settings_line_read_cb,
					 &value_ctx, arg);
}
