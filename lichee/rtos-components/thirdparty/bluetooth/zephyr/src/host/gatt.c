...
#if XRADIO
/* GATT handle allocate policy, you can set as
 * 1 to allocate original handle, or 2 for increasing handle.
 */
#define HDL_ALLOC_POLICY	(1)
#endif
...
#if 1 //??
			       BT_GATT_PERM_READ | BT_GATT_PERM_WRITE/*_ENCRYPT*/,
#else
...
#if (XRADIO && (HDL_ALLOC_POLICY == 2))
	struct bt_gatt_service *tmp;
	SYS_SLIST_FOR_EACH_CONTAINER(&db, tmp, node) {
		if (tmp == svc) {
			/* Service had been registered */
			BT_ERR("Service had been registered\n");
			return -EPERM;
		}
	}
#endif
...
#if !defined(CONFIG_BLEHOST_Z_ITERABLE_SECTION)
static int static_gatt_register(struct bt_gatt_service_static *svc)
{
#define STATIC_DB bt_gatt_service_static_list
	const struct bt_gatt_attr *attrs = svc->attrs;
	u16_t count = svc->attr_count;
	BT_DBG("%d", count);
	if (sys_slist_is_empty(&STATIC_DB)) {
		goto populate;
	}
...
populate:
	...
	sys_slist_append(&STATIC_DB, &svc->node);

	return 0;
}

#if defined(CONFIG_BT_DEINIT)
static int static_gatt_unregister(struct bt_gatt_service_static *svc)
{
#define STATIC_DB bt_gatt_service_static_list
	const struct bt_gatt_attr *attrs = svc->attrs;
	u16_t count = svc->attr_count;
	BT_DBG("%d", count);
	if (!sys_slist_is_empty(&STATIC_DB)) {
		if (!sys_slist_find_and_remove(&STATIC_DB, &svc->node)) {
			return -ENOENT;
		}
	}
...
#if defined(CONFIG_BT_VAR_MEM_DYNC_ALLOC)
	subscriptions = (struct gatt_sub *)k_malloc(sizeof(struct gatt_sub) * SUB_MAX);
	if (subscriptions == NULL) {
		BT_ERR("Alloc subscriptions error");
		return ;
	}
	memset(subscriptions, 0, sizeof(struct gatt_sub) * SUB_MAX);
#endif

#if !defined(CONFIG_BLEHOST_Z_ITERABLE_SECTION)
	/* Register mandatory services */
	static_gatt_register(&_2_gap_svc);
	static_gatt_register(&_1_gatt_svc);
#endif
...
#if defined(CONFIG_BT_DEINIT)
int bt_gatt_deinit(void)
{
	while (!sys_slist_is_empty(&db)) {
		struct bt_gatt_service *tmp;
		struct bt_gatt_attr *attrs = NULL;
		uint16_t count = 0;
		SYS_SLIST_FOR_EACH_CONTAINER(&db, tmp, node) {
			attrs = tmp->attrs;
			count = tmp->attr_count;
			for (; attrs && count; attrs++, count--) {
				attrs->handle = 0;
			}
			sys_slist_find_and_remove(&db, &tmp->node);
			last_static_handle -= tmp->attr_count;
		}
	}
...
#if !defined(CONFIG_BLEHOST_Z_ITERABLE_SECTION)
	static_gatt_unregister(&_1_gatt_svc);
	static_gatt_unregister(&_2_gap_svc);
#endif

#if defined(CONFIG_BT_SETTINGS_CCC_STORE_ON_WRITE)
	k_delayed_work_cancel(&gatt_ccc_store.work);
	k_delayed_work_deinit(&gatt_ccc_store.work);
	for (i = 0; i < ARRAY_SIZE(gatt_ccc_store.conn_list), i++) {
		gatt_ccc_store.conn_list[i] = NULL;
	}
#endif

	if (IS_ENABLED(CONFIG_BT_GATT_SERVICE_CHANGED)) {
		k_delayed_work_cancel(&gatt_sc.work);
		k_delayed_work_deinit(&gatt_sc.work);
		gatt_sc.start = 0;
		gatt_sc.end = 0;
		memset(&gatt_sc.params, 0, sizeof(struct bt_gatt_indicate_params));
		memset(&gatt_sc.flags, 0, sizeof(gatt_sc.flags));
	}

#if defined(CONFIG_BT_GATT_CACHING)
	k_delayed_work_cancel(&db_hash_work);
	k_delayed_work_deinit(&db_hash_work);
#endif /* CONFIG_BT_GATT_CACHING */

#if defined(CONFIG_BT_VAR_MEM_DYNC_ALLOC)
	k_free(subscriptions);
	subscriptions = NULL;
#endif

	if (!atomic_cas(&service_init, 1, 0)) {
		return -1;
	}

	if (!atomic_cas(&init, 1, 0)) {
		return -1;
	}
				      ...
				      void *user_data)
{
	...
	if (write != length) {
		BT_WARN("Unable to allocate length %u: only %u written",
			length, write);
		net_buf_unref(buf);
		return -ENOMEM;
	}
			       ...
			       void *user_data)
{
	...
	/*complete subscribe cback*/
	if (params->write) {
		struct bt_gatt_write_params *w_params =
				(struct bt_gatt_write_params *)malloc(sizeof(struct bt_gatt_write_params));
		if (w_params == NULL) {
			return;
		}
		w_params->handle = params->value_handle;
		params->write(conn, err, w_params);
		free(w_params);
		w_params = NULL;
	}
}
...
#if IS_ENABLED(CONFIG_BT_SETTINGS) //defined(CONFIG_BT_SETTINGS) //Wewisetek Modified
...
static int ccc_set_null(const char *name, size_t len_rd, settings_read_cb read_cb,
		   void *cb_arg)
{
	return 0;
}
...
#else
SETTINGS_STATIC_HANDLER_DEFINE(bt_ccc, "bt/ccc", NULL, ccc_set_null, NULL, NULL);
#endif
#elif defined(CONFIG_SETTINGS_DYNAMIC_HANDLERS)
struct settings_handler bt_ccc_handler = {
		.name = "bt/ccc",
		.h_get = NULL,
		.h_set = ccc_set_cb,
		.h_commit = NULL,
...
#endif // IS_ENABLED(CONFIG_BT_SETTINGS)
...
#elif defined(CONFIG_SETTINGS_DYNAMIC_HANDLERS)
struct settings_handler bt_sc_handler = {
		.name = "bt/sc",
		.h_get = NULL,
		.h_set = sc_set,
		.h_commit = sc_commit,
...
#elif defined(CONFIG_SETTINGS_DYNAMIC_HANDLERS)
struct settings_handler bt_cf_handler = {
		.name = "bt/cf",
		.h_get = NULL,
		.h_set = cf_set,
		.h_commit = NULL,
...
SETTINGS_STATIC_HANDLER_DEFINE(bt_hash, "bt/hash", NULL, db_hash_set,
			       db_hash_commit, NULL);
#elif defined(CONFIG_SETTINGS_DYNAMIC_HANDLERS)
struct settings_handler bt_hash_handler = {
		.name = "bt/hash",
		.h_get = NULL,
		.h_set = db_hash_set,
		.h_commit = db_hash_commit,
		.h_export = NULL
};
#endif
#endif /*CONFIG_BT_GATT_CACHING */
...
void bt_gatt_mem_deinit(void)
{
...
#endif
	{
		memset(&subscriptions[i], 0, sizeof(struct gatt_sub));
	}

	for (i = 0; i < ARRAY_SIZE(sc_cfg); i++) {
		memset(&sc_cfg[i], 0, sizeof(struct gatt_sc_cfg));
	}

	for (i = 0; i < ARRAY_SIZE(cf_cfg); i++) {
		memset(&cf_cfg[i], 0, sizeof(struct gatt_cf_cfg));
	}

#if defined(CONFIG_BT_GATT_CACHING)
	memset(&db_hash, 0, 16);
	memset(&stored_hash, 0, 16);
#endif
	...
	for (i = 0; i < ARRAY_SIZE(sc_restore_params); i++) {
		//memset(&sc_restore_params[i], 0, sizeof(struct bt_gatt_indicate_params));
		sc_restore_params[i].uuid = NULL;
		sc_restore_params[i].attr = NULL;
		sc_restore_params[i].func = NULL;
		sc_restore_params[i].data = NULL;
		sc_restore_params[i].len = 0;
	}
}
#endif
