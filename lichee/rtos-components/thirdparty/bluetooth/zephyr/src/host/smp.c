...
#else
static struct bt_smp_br *bt_smp_br_pool;
#endif
#endif /* CONFIG_BT_BREDR */
...
static uint8_t compute_and_send_master_dhcheck(struct bt_smp *smp)
{
	...
	if (smp_f6(smp->mackey, smp->prnd, smp->rrnd, r, &smp->preq[1],
		...
		return BT_SMP_ERR_UNSPECIFIED;
			...
			printf("e: %02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x\n",
...
#if XRADIO
		/*
		 * If pairing is not completed, the key information
		 * is incomplete, which will result the next pairing
		 * can't recognize the device address and the next
		 * next pairing will use the incomplete key. This
		 * circumstance will cause hci_id_add twice same
		 * address to controller without hci_id_del.
		 */
		if (smp->local_dist || smp->remote_dist) {
			bt_keys_clear(keys);
		}
...
#if defined(CONFIG_BT_VAR_MEM_DYNC_ALLOC)
	bt_smp_pool = (struct bt_smp *)k_malloc(sizeof(struct bt_smp) * CONFIG_BT_MAX_CONN);
	if (bt_smp_pool == NULL) {
		BT_ERR("Alloc bt smp pool err");
		return -ENOMEM;
	}
	memset(bt_smp_pool, 0, sizeof(struct bt_smp) * CONFIG_BT_MAX_CONN);
#if defined(CONFIG_BT_BREDR)
	bt_smp_br_pool = (struct bt_smp_br *)k_malloc(sizeof(struct bt_smp_br) * CONFIG_BT_MAX_CONN);
	if (bt_smp_br_pool == NULL) {
		BT_ERR("Alloc bt smp br pool err");
		return -ENOMEM;
	}
	memset(bt_smp_br_pool, 0, sizeof(struct bt_smp_br) * CONFIG_BT_MAX_CONN);
#endif
#endif
...
{
#if !defined(CONFIG_BLEHOST_Z_ITERABLE_SECTION)
	bt_l2cap_le_fixed_chan_unregister(smp_fixed_chan.cid);
#endif
...
void bt_smp_mem_deinit(void)
{
	...
	k_sem_delete(&sc_local_pkey_ready);
        ...
        bt_smp_pool = NULL;
#if defined(CONFIG_BT_BREDR)
        k_free(bt_smp_br_pool);
        bt_smp_br_pool = NULL;
#endif
#else
	memset(&bt_smp_pool, 0, sizeof(struct bt_smp) * ARRAY_SIZE(bt_smp_pool));
#endif
}
#endif
