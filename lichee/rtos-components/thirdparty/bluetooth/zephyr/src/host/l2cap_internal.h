...
#else
/* this should be optimize like Z_STRUCT_SECTION_ITERABLE or type##_list {const type *; sys_snode_t node} in zephyr */
#define BT_L2CAP_CHANNEL_DEFINE(_name, _cid, _accept, _destroy)         \
	/* const */struct bt_l2cap_fixed_chan _name = { \
				.cid = _cid,                            \
				.accept = _accept,                      \
				.destroy = _destroy,                    \
			}
...
#define BT_L2CAP_BR_CHANNEL_DEFINE(_name, _cid, _accept)		\
	/* const */struct bt_l2cap_br_fixed_chan _name = { \
				.cid = _cid,			\
				.accept = _accept,		\
			}
...
#if !defined(CONFIG_BLEHOST_Z_ITERABLE_SECTION)
/* Unregister a fixed L2CAP channel from L2CAP */
void bt_l2cap_le_fixed_chan_unregister(struct bt_l2cap_fixed_chan *chan)
#endif
#endif
...
#if defined(CONFIG_BT_DEINIT)
/* Deinitialize L2CAP and supported channels */
void bt_l2cap_deinit(void);
/* Deinitialize l2cap variable memory */
...
#if defined(CONFIG_BT_DEINIT)
/* Uninitialize BR/EDR L2CAP signal layer */
