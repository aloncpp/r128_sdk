...
static int health_cli_init(struct bt_mesh_model *model)
{
	...
	/*the variable msg_timeout is not init, so when we use health client model
	and send command to health server model, timeout is triggered soon, and
	operation will not success, so we should give it a init value 2s to make
	sure the operation can be completed before timeout occur, add by liuzhang,
	20210407*/
	msg_timeout = 2 * MSEC_PER_SEC;
