...
static int node_id_adv(struct bt_mesh_subnet *sub, int32_t duration)
{
	...
	int err;
#if XRADIO
	struct bt_le_adv_param temp_fast_adv_param;
#endif
...
#if XRADIO
	memcpy(&temp_fast_adv_param, &fast_adv_param, sizeof(fast_adv_param));
	temp_fast_adv_param.options |= BT_LE_ADV_OPT_USE_NAME;

	err = bt_mesh_adv_start(&temp_fast_adv_param, duration, node_id_ad,
...
static size_t gatt_prov_adv_create(struct bt_data prov_sd[2])
{
	...
	if (prov->uri) {
		...
		if (uri_len > 29) {
			...
			prov_sd[0].data = (uint8_t *)prov->uri; //Wewisetek : modify for compiler error
			sd_space -= 2 + uri_len;
			prov_sd_len++;
		}
	}
	...
	if (sd_space > 2 && name_len > 0) {
		...
		prov_sd[prov_sd_len].data = (uint8_t *)name; //Wewisetek : modify for compiler error
		prov_sd_len++;
	}
