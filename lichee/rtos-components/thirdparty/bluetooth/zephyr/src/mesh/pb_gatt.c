		    ...
		    void *cb_data)
{
	...
	BT_DBG("%d %p", __LINE__, &link.prot_timer);
...
#if XRADIO
	/*zephyr original code is not consider about the callback function transfer
	after the packet send complete, so it will lead to provision can not
	complete when we use input oob method, so we modify to make the callback
	execution here to make sure the provision process can be execution
	complete, add by liuzhang, 20210407*/
	err = bt_mesh_proxy_send(link.conn, BT_MESH_PROXY_PROV, buf);
