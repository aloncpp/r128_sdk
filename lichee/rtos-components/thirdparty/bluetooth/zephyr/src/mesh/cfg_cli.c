...
struct key_refresh_phase_param {
	uint8_t *status;
	uint16_t *netkey_idx;
	uint8_t  *phase;
};
...
static void krp_status(struct bt_mesh_model *model,
			   ...
			   struct net_buf_simple *buf)
{
	struct key_refresh_phase_param *param;
	...
	param = cli->op_param;
	*param->status = net_buf_simple_pull_u8(buf);
	*param->netkey_idx = net_buf_simple_pull_le16(buf) & 0xfff;
			    ...
			    uint8_t transition, uint16_t *netkey_idx, uint8_t *phase, uint8_t *status)
{
	...
	if (transition > BT_MESH_KR_PHASE_3) {
        return -EINVAL;
	}
