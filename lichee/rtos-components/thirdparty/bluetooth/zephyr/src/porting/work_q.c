...
static sys_slist_t k_delayed_work_timeout_list;
#endif
...
#if defined(CONFIG_BT_DEINIT)
static int k_delayed_work_timeout_deinit(void)
{
	if (!sys_slist_is_empty(&k_delayed_work_timeout_list)) {
		struct k_delayed_work *tmp;
		SYS_SLIST_FOR_EACH_CONTAINER(&k_delayed_work_timeout_list, tmp, node) {
			k_delayed_work_deinit(tmp);
		}
	}
	return 0;
}

static void k_delayed_work_timeout_add(struct k_delayed_work *work)
{
	struct k_delayed_work *tmp;
	SYS_SLIST_FOR_EACH_CONTAINER(&k_delayed_work_timeout_list, tmp, node) {
		if (tmp == work) {
	...
	sys_slist_append(&k_delayed_work_timeout_list, &work->node);
}

static void k_delayed_work_timeout_remove(struct k_delayed_work *work)
{
	struct k_delayed_work *tmp;
	SYS_SLIST_FOR_EACH_CONTAINER(&k_delayed_work_timeout_list, tmp, node) {
		if (tmp == work) {
			sys_slist_find_and_remove(&k_delayed_work_timeout_list, &tmp->node);
			break;
		}
	}
}
...
#if defined(CONFIG_BT_DEINIT)
	k_delayed_work_timeout_remove(w);
#endif
}
...
void k_delayed_work_init(struct k_delayed_work *work, k_work_handler_t handler)
{
	...
	work->work_q = NULL;
#if defined(CONFIG_BT_DEINIT)
	memset(&work->node, 0 , sizeof(work->node));
#endif
}
...
#if defined(CONFIG_BT_DEINIT)
	k_delayed_work_timeout_add(work);
#endif
...
#if defined(CONFIG_BT_DEINIT)
		k_delayed_work_timeout_remove(work);
#endif
	}
