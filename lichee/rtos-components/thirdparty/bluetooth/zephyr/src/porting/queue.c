...
{
	OS_SemaphoreRelease(&queue->wait_q.waitq);
}
#endif /* CONFIG_POLL */
...
#if defined(CONFIG_POLL)
static void *k_queue_poll(struct k_queue *queue, int32_t timeout)
{
...
void *k_queue_get(struct k_queue *queue, k_timeout_t timeout)
{
	 ...
	 */
	irq_unlock(key);
	while (likely(sys_sflist_is_empty(&queue->data_q)) && ret == OS_OK)
		ret = OS_SemaphoreWait(&queue->wait_q.waitq, timeout);
