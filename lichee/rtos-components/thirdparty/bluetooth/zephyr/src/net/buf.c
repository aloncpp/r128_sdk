...
#define NET_BUF_DBG(fmt, ...) LOG_DBG("(%p) " fmt, __func__/*k_current_get()*/, \
					  ##__VA_ARGS__)
...
#endif
{
	uint32_t alloc_start = k_uptime_get_32();
	struct net_buf *buf;
	unsigned int key;
...
success:
	...
	if (size) {
		...
		if (!K_TIMEOUT_EQ(timeout, K_NO_WAIT) &&
			...
			uint32_t diff = k_uptime_get_32() - alloc_start;
...
#if defined(CONFIG_NET_BUF_DEBUG_POOL) && defined(CONFIG_NET_BUF_LOG)
	if (CONFIG_NET_BUF_DEBUG_POOL == pool)
#if defined(CONFIG_NET_BUF_POOL_USAGE)
		NET_BUF_INFO("%s():%d: allocated buf, %d available",
					 func, line, atomic_get(&pool->avail_count));
#else
		NET_BUF_INFO("%s():%d: allocated buf", func, line);
#endif /* defined(CONFIG_NET_BUF_POOL_USAGE) */
#endif /* defined(CONFIG_NET_BUF_DEBUG_POOL) */
...
#endif
{
	...
	while (buf) {
		...
		if (CONFIG_NET_BUF_DEBUG_POOL == pool)
#if defined(CONFIG_NET_BUF_POOL_USAGE)
			NET_BUF_INFO("%s():%d: free buf, %d available",
						 func, line, atomic_get(&pool->avail_count));
#else
			NET_BUF_INFO("%s():%d: free buf", func, line);
#endif /* defined(CONFIG_NET_BUF_POOL_USAGE) */
#endif /* defined(CONFIG_NET_BUF_DEBUG_POOL) */
