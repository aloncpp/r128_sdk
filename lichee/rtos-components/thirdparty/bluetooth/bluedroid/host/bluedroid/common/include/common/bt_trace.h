...
// Mapping between LOG_LEVEL and BT_TRACE_LEVEL
...
#define log_timestamp() OS_TicksToMSecs(OS_GetTicks())
#define log_write(level, tag1, format, time, tag2, ...) printf(format, time, tag2, ##__VA_ARGS__)
#else
#define LOG_FORMAT(letter, format)  LOG_COLOR_ ## letter #letter " %s: " format LOG_RESET_COLOR "\n"
#define log_timestamp() (0)
#define log_write(level, tag1, format, time, tag2, ...) printf(format, tag2, ##__VA_ARGS__)
#endif

#ifndef BT_PRINT_E
#define BT_PRINT_E(tag, format, ...)   {xr_log_write(XR_LOG_ERROR,   tag, LOG_FORMAT(E, format), xr_log_timestamp(), tag, ##__VA_ARGS__); }
#endif
#ifndef BT_PRINT_W
#define BT_PRINT_W(tag, format, ...)   {xr_log_write(XR_LOG_WARN,    tag, LOG_FORMAT(W, format), xr_log_timestamp(), tag, ##__VA_ARGS__); }
#endif
#ifndef BT_PRINT_I
#define BT_PRINT_I(tag, format, ...)   {xr_log_write(XR_LOG_INFO,    tag, LOG_FORMAT(I, format), xr_log_timestamp(), tag, ##__VA_ARGS__); }
#endif
#ifndef BT_PRINT_D
#define BT_PRINT_D(tag, format, ...)   {xr_log_write(XR_LOG_DEBUG,   tag, LOG_FORMAT(D, format), xr_log_timestamp(), tag, ##__VA_ARGS__); }
#endif
#ifndef BT_PRINT_V
#define BT_PRINT_V(tag, format, ...)   {xr_log_write(XR_LOG_VERBOSE, tag, LOG_FORMAT(V, format), xr_log_timestamp(), tag, ##__VA_ARGS__); }
#endif

#ifndef assert
#define assert(x)   do { if (!(x)) BT_PRINT_E("", "bt host error %s %u: %s\n", __FILE__, __LINE__, #x); } while (0)
#endif

#define TP printf("tp <%s : %d>\n", __func__, __LINE__)

#ifndef CONFIG_BTTRC_DUMP_BUFFER
#define CONFIG_BTTRC_DUMP_BUFFER 0
#endif

static inline void trc_dump_buffer(const char *s, const uint8_t *data, uint16_t len, int type)
{
    uint16_t i;
...
static inline void hcidump(bool is_received, const uint8_t *data, uint16_t len, int type)
{
#if (CONFIG_BTTRC_DUMP_BUFFER == 1)
    uint32_t length_he = 0;
    uint32_t flags = is_received;
    ...
    uint8_t *tmp = calloc(1,length_he+2);
    tmp++;
    tmp++;
    memcpy(tmp, data, length_he);

    /***change for fit with hcidump tool****/
    tmp--;
    *tmp = (uint8_t)type;
    ...
    *tmp = (uint8_t)flags;
    ...
    process_hci_data(PARSE, tmp, length_he + 1);

    free(tmp);

#elif (CONFIG_BTTRC_DUMP_BUFFER == 2)
	if (is_received)
		trc_dump_buffer("rx", data, len, type);
	else
		trc_dump_buffer("tx", data, len, type);
#elif (CONFIG_BTTRC_DUMP_BUFFER == 3)
    return;
#elif (CONFIG_BTTRC_DUMP_BUFFER == 4)
        hci_grep_buffer(is_received, data, len, type);
#endif
}
#else
static inline void hcidump(bool is_received, const uint8_t *data, uint16_t len, int type)
...
#if CONFIG_BTTRC_DUMP_BUFFER
#define BTTRC_DUMP_BUFFER(_is_received, _data, _len, _type) hcidump(_is_received, _data, _len, _type)
#else
#define BTTRC_DUMP_BUFFER(_is_received, _data, _len, _type)
#endif

#define BT_HDR_DUMP(s, bt_hdr) trc_dump_buffer(s, bt_hdr->data, bt_hdr->len + 1, 0)
...
//It is not affected by the print level.
#define LOG_TRACE(format, ... ) {log_write(LOG_INFO,   "BT_LOG", LOG_FORMAT(I, format), log_timestamp(), "BT_LOG", ##__VA_ARGS__);}

#define LOG_ERROR(format, ... )             {if (LOG_LOCAL_LEVEL >= LOG_ERROR)    log_write(LOG_ERROR,   "BT_LOG", LOG_FORMAT(E, format), log_timestamp(), "BT_LOG", ##__VA_ARGS__); }
#define LOG_WARN(format, ... )              {if (LOG_LOCAL_LEVEL >= LOG_WARN)     log_write(LOG_WARN,    "BT_LOG", LOG_FORMAT(W, format), log_timestamp(), "BT_LOG", ##__VA_ARGS__); }
#define LOG_INFO(format, ... )              {if (LOG_LOCAL_LEVEL >= LOG_INFO)     log_write(LOG_INFO,    "BT_LOG", LOG_FORMAT(I, format), log_timestamp(), "BT_LOG", ##__VA_ARGS__); }
#define LOG_DEBUG(format, ... )             {if (LOG_LOCAL_LEVEL >= LOG_DEBUG)    log_write(LOG_DEBUG,   "BT_LOG", LOG_FORMAT(D, format), log_timestamp(), "BT_LOG", ##__VA_ARGS__); }
#define LOG_VERBOSE(format, ... )           {if (LOG_LOCAL_LEVEL >= LOG_VERBOSE)  log_write(LOG_VERBOSE, "BT_LOG", LOG_FORMAT(V, format), log_timestamp(), "BT_LOG", ##__VA_ARGS__); }
