...
#define OS_Queue_t XR_OS_Queue_t
#define OS_QueueReceive XR_OS_QueueReceive
#define OS_QueueIsValid XR_OS_QueueIsValid
#define OS_ThreadDelete XR_OS_ThreadDelete
#define OS_QueueCreate XR_OS_QueueCreate
#define OS_QueueSend XR_OS_QueueSend
#define OS_QueueDelete XR_OS_QueueDelete
...
#define BTI_A2DP_THREAD_IND	1

/* BTC source command event definition */
enum {
#if (CONFIG_BT_SRC_PROCESS && BTI_A2DP_THREAD_IND)
    BTC_A2DP_SOURCE_DATA_EVT,
#endif
    BTC_MEDIA_TASK_INIT,
...
#if (CONFIG_BT_SRC_PROCESS && BTI_A2DP_THREAD_IND)
#ifndef CONFIG_ARCH_RISCV_RV64
#define CONFIG_A2DP_SOURCE_TASK_STACK_SIZE 2048
#else
#define CONFIG_A2DP_SOURCE_TASK_STACK_SIZE 3072
#endif
#define BTI_A2DP_SOURCE_TASK_STACK_SIZE       (CONFIG_A2DP_SOURCE_TASK_STACK_SIZE + BT_TASK_EXTRA_STACK_SIZE) // by menuconfig
#define BTI_A2DP_SOURCE_TASK_NAME             "BtA2dSourceT"
#define BTI_A2DP_SOURCE_TASK_PRIO             (BTC_A2DP_SOURCE_TASK_PRIO)
#define BTI_A2DP_SOURCE_DATA_QUEUE_LEN        (3)
#define BTI_A2DP_SOURCE_CTRL_QUEUE_LEN        (5)
#define BTI_A2DP_SOURCE_TASK_QUEUE_SET_LEN    (BTI_A2DP_SOURCE_DATA_QUEUE_LEN + BTI_A2DP_SOURCE_CTRL_QUEUE_LEN)

static OS_Thread_t  bti_aa_src_thread;
static OS_Queue_t bti_aa_src_data_ctrl_queue;
static osi_sem_t bti_aa_src_data_sem;
static osi_sem_t bti_aa_src_ctrl_sem;
#endif
...
static xr_a2d_source_data_cb_t btc_aa_src_data_cb = NULL;
...
void btc_a2dp_src_reg_data_cb(xr_a2d_source_data_cb_t callback)
{
 ...
 *****************************************************************************/
static inline void btc_aa_cb_to_app(xr_a2d_cb_event_t event, xr_a2d_cb_param_t *param)
{
    xr_a2d_cb_t btc_aa_cb = (xr_a2d_cb_t)btc_profile_cb_get(BTC_PID_A2DP);
    if (btc_aa_cb) {
...
#if (CONFIG_BT_SRC_PROCESS && BTI_A2DP_THREAD_IND)
static void btc_a2dp_source_ctrl(uint32_t sig, void *par)
{
    BtTaskEvt_t evt;
    ...
    if (osi_sem_take(&bti_aa_src_ctrl_sem, OSI_SEM_MAX_TIMEOUT) != 0) {
        APPL_TRACE_WARNING("%s cannot take sem: bti_aa_src_ctrl_sem\n", __func__);
        return;
    }

    if (OS_QueueSend(&bti_aa_src_data_ctrl_queue, &evt, OS_WAIT_FOREVER) != OS_OK) {
        APPL_TRACE_WARNING("bti_aa_src_data_ctrl_queue failed, sig 0x%x\n", sig);
    }
}
...
static void btc_a2dp_source_data_post(int32_t data_type)
{
    ...
    if (osi_sem_take(&bti_aa_src_data_sem, 0) != 0) {
        APPL_TRACE_DEBUG("%s cannot tak sem: bti_aa_src_data_sem\n", __func__);
        return;
    }

    if (OS_QueueSend(&bti_aa_src_data_ctrl_queue, &evt, 0) != OS_OK) {
...
static void btc_a2dp_source_task_handler(void *arg)
{
    BtTaskEvt_t e;
    for (;;) {
        OS_QueueReceive(&bti_aa_src_data_ctrl_queue, &e, OS_WAIT_FOREVER);

        switch (e.sig) {
        case BTC_A2DP_SOURCE_DATA_EVT:
            btc_a2dp_source_handle_timer(NULL);
            osi_sem_give(&bti_aa_src_data_sem);
            break;
        default:
            btc_a2dp_source_ctrl_handler(&e);
            osi_sem_give(&bti_aa_src_ctrl_sem);
            break;
        }
    }
}
...
#if (CONFIG_BT_SRC_PROCESS && BTI_A2DP_THREAD_IND)
    memset(&bti_aa_src_data_ctrl_queue, 0, sizeof(bti_aa_src_data_ctrl_queue));
       if (OS_QueueCreate(&bti_aa_src_data_ctrl_queue, BTI_A2DP_SOURCE_TASK_QUEUE_SET_LEN, sizeof(BtTaskEvt_t)) != OS_OK) {
           goto error_exit;
       }

       memset(&bti_aa_src_data_sem, 0, sizeof(bti_aa_src_data_sem));
       memset(&bti_aa_src_ctrl_sem, 0, sizeof(bti_aa_src_ctrl_sem));
       if (osi_sem_new(&bti_aa_src_data_sem, BTI_A2DP_SOURCE_DATA_QUEUE_LEN, BTI_A2DP_SOURCE_DATA_QUEUE_LEN) != 0) {
           goto error_exit;
       }
       if (osi_sem_new(&bti_aa_src_ctrl_sem, BTI_A2DP_SOURCE_CTRL_QUEUE_LEN, BTI_A2DP_SOURCE_CTRL_QUEUE_LEN) != 0) {
           goto error_exit;
       }

       memset(&bti_aa_src_thread, 0, sizeof(OS_Thread_t));
       if (OS_ThreadCreate(&bti_aa_src_thread, BTI_A2DP_SOURCE_TASK_NAME, btc_a2dp_source_task_handler,
                           NULL, BTI_A2DP_SOURCE_TASK_PRIO, BTI_A2DP_SOURCE_TASK_STACK_SIZE) != OS_OK) {
           goto error_exit;
       }
...
#if (CONFIG_BT_SRC_PROCESS && BTI_A2DP_THREAD_IND)
    btc_a2dp_source_ctrl(BTC_MEDIA_TASK_INIT, NULL);
#else
...
#if (CONFIG_BT_SRC_PROCESS && BTI_A2DP_THREAD_IND)
    if (OS_ThreadIsValid(&bti_aa_src_thread)) {
        OS_ThreadDelete(&bti_aa_src_thread);
    }

    if (OS_QueueIsValid(&bti_aa_src_data_ctrl_queue)) {
        OS_QueueDelete(&bti_aa_src_data_ctrl_queue);
    }
    if (osi_sem_is_valid(&bti_aa_src_data_sem)) {
        osi_sem_free(&bti_aa_src_data_sem);
    }
    if (osi_sem_is_valid(&bti_aa_src_ctrl_sem)) {
        osi_sem_free(&bti_aa_src_ctrl_sem);
    }
#endif
...
#if (CONFIG_BT_SRC_PROCESS && BTI_A2DP_THREAD_IND)
    /*
     * delete thread and queue.
     * !!! Notice: This may cause some problems.
     */
    OS_ThreadDelete(&bti_aa_src_thread);
    OS_QueueDelete(&bti_aa_src_data_ctrl_queue);
...
static void btc_a2dp_source_aa_stop_tx(void)
{
    ...
    if (send_ack) {
        btc_a2dp_control_command_ack(XR_A2D_MEDIA_CTRL_ACK_SUCCESS);
    }
...
static void btc_a2dp_source_aa_start_tx(void)
{
    ...
    a2dp_source_local_param.btc_aa_src_cb.media_alarm = osi_alarm_periodic_new("aaTx", btc_a2dp_source_alarm_cb, NULL, BTC_MEDIA_TIME_TICK_MS);
    ...
    osi_alarm_set(a2dp_source_local_param.btc_aa_src_cb.media_alarm, BTC_MEDIA_TIME_TICK_MS);
}
