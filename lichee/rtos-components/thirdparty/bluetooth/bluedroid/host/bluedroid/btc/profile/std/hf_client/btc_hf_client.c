...
#ifndef XRADIO_HFP_ADJUST_VOLUME
...
if (! hf_client_local_param.btc_hf_client_cb.initialized || \
    hf_client_local_param.btc_hf_client_cb.state != XR_HF_CLIENT_CONNECTION_STATE_SLC_CONNECTED) { \
    return BT_STATUS_NOT_READY; \
} \
} while (0)

static inline void btc_hf_client_cb_to_app(xr_hf_client_cb_event_t event, xr_hf_client_cb_param_t *param)
{
    xr_hf_client_cb_t btc_hf_client_callback = (xr_hf_client_cb_t)btc_profile_cb_get(BTC_PID_HF_CLIENT);
...
{
    if (((hf_client_local_param.btc_hf_client_cb.state == XR_HF_CLIENT_CONNECTION_STATE_CONNECTED) ||
            (hf_client_local_param.btc_hf_client_cb.state == XR_HF_CLIENT_CONNECTION_STATE_SLC_CONNECTED))&&
...
void btc_hf_client_reg_data_cb(xr_hf_client_incoming_data_cb_t recv,
...
bt_status_t btc_hf_client_init(void)
{
    ...
    data_path = XR_SCO_DATA_PATH_HCI;
#else
    data_path = XR_SCO_DATA_PATH_PCM;
#endif
...
static bt_status_t connect_int( bt_bdaddr_t *bd_addr, uint16_t uuid )
{
    ...
    hf_client_local_param.btc_hf_client_cb.state = XR_HF_CLIENT_CONNECTION_STATE_CONNECTING;
...
bt_status_t btc_hf_client_connect_audio( bt_bdaddr_t *bd_addr )
{
    ...
    if (is_connected(bd_addr))
    {
        ...
        do {
            xr_hf_client_cb_param_t param;
            memset(&param, 0, sizeof(xr_hf_client_cb_param_t));
            param.audio_stat.state = XR_HF_CLIENT_AUDIO_STATE_CONNECTING;
            memcpy(param.audio_stat.remote_bda, &hf_client_local_param.btc_hf_client_cb.connected_bda, sizeof(xr_bd_addr_t));
            btc_hf_client_cb_to_app(XR_HF_CLIENT_AUDIO_STATE_EVT, &param);
        } while (0);
...
**
*******************************************************************************/
static bt_status_t btc_hf_client_volume_update(xr_hf_volume_control_target_t type, int volume)
{
    CHECK_HF_CLIENT_SLC_CONNECTED();

    switch (type)
    {
        case XR_HF_VOLUME_CONTROL_TARGET_SPK:
            BTA_HfClientSendAT(hf_client_local_param.btc_hf_client_cb.handle, BTA_HF_CLIENT_AT_CMD_VGS, volume, 0, NULL);
            break;
        case XR_HF_VOLUME_CONTROL_TARGET_MIC:
...
static bt_status_t btc_hf_client_send_chld_cmd(xr_hf_chld_type_t type, int idx)
{
    CHECK_HF_CLIENT_SLC_CONNECTED();

    switch (type)
    {
    case XR_HF_CHLD_TYPE_REL:
    ...
    case XR_HF_CHLD_TYPE_MERGE:
        ...
        return BT_STATUS_UNSUPPORTED;
    case XR_HF_CHLD_TYPE_MERGE_DETACH:
...
static void process_ind_evt(tBTA_HF_CLIENT_IND *ind)
{
    ...
    switch (ind->type)
    {
        case BTA_HF_CLIENT_IND_CALL:
            ...
            btc_hf_client_cb_to_app(XR_HF_CLIENT_CIND_CALL_EVT, &param);
            break;

        case BTA_HF_CLIENT_IND_CALLSETUP:
            param.call_setup.status = ind->value;
            btc_hf_client_cb_to_app(XR_HF_CLIENT_CIND_CALL_SETUP_EVT, &param);
            break;
        case BTA_HF_CLIENT_IND_CALLHELD:
            param.call_held.status = ind->value;
            btc_hf_client_cb_to_app(XR_HF_CLIENT_CIND_CALL_HELD_EVT, &param);
            break;

        case BTA_HF_CLIENT_IND_SERVICE:
            param.service_availability.status = ind->value;
            btc_hf_client_cb_to_app(XR_HF_CLIENT_CIND_SERVICE_AVAILABILITY_EVT, &param);
            break;

        case BTA_HF_CLIENT_IND_SIGNAL:
            param.signal_strength.value = ind->value;
            btc_hf_client_cb_to_app(XR_HF_CLIENT_CIND_SIGNAL_STRENGTH_EVT, &param);
            break;

        case BTA_HF_CLIENT_IND_ROAM:
            param.roaming.status = ind->value;
            btc_hf_client_cb_to_app(XR_HF_CLIENT_CIND_ROAMING_STATUS_EVT, &param);
            break;

        case BTA_HF_CLIENT_IND_BATTCH:
            param.battery_level.value = ind->value;
            btc_hf_client_cb_to_app(XR_HF_CLIENT_CIND_BATTERY_LEVEL_EVT, &param);
            break;
    ...
    switch (event)
    {
        ...
        case BTA_HF_CLIENT_OPEN_EVT:
            if (p_data->open.status == BTA_HF_CLIENT_SUCCESS)
            {
                ...
                hf_client_local_param.btc_hf_client_cb.state = XR_HF_CLIENT_CONNECTION_STATE_CONNECTED;
                hf_client_local_param.btc_hf_client_cb.peer_feat = 0;
                hf_client_local_param.btc_hf_client_cb.chld_feat = 0;
                //clear_phone_state();
            }
            else if (hf_client_local_param.btc_hf_client_cb.state == XR_HF_CLIENT_CONNECTION_STATE_CONNECTING)
            {
                hf_client_local_param.btc_hf_client_cb.state = XR_HF_CLIENT_CONNECTION_STATE_DISCONNECTED;
            }
            else
            {
            ...
            do {
                ...
                btc_hf_client_cb_to_app(XR_HF_CLIENT_CONNECTION_STATE_EVT, &param);
            } while (0);

            if (hf_client_local_param.btc_hf_client_cb.state == XR_HF_CLIENT_CONNECTION_STATE_DISCONNECTED)
        ...
        case BTA_HF_CLIENT_CONN_EVT:
            ...
            if (hf_client_local_param.btc_hf_client_cb.peer_feat & BTA_HF_CLIENT_PEER_INBAND)
            {
                do {
                    memset(&param, 0, sizeof(xr_hf_client_cb_param_t));
                    param.bsir.state =  XR_HF_CLIENT_IN_BAND_RINGTONE_PROVIDED;
                    btc_hf_client_cb_to_app(XR_HF_CLIENT_BSIR_EVT, &param);
                } while (0);
            }
        ...
        case BTA_HF_CLIENT_CLOSE_EVT:
            ...
            do {
                memset(&param, 0, sizeof(xr_hf_client_cb_param_t));
                param.conn_stat.state = XR_HF_CLIENT_CONNECTION_STATE_DISCONNECTED;
                param.conn_stat.peer_feat = 0;
                param.conn_stat.chld_feat = 0;
        ...
        case BTA_HF_CLIENT_MIC_EVT:
            do {
                memset(&param, 0, sizeof(xr_hf_client_cb_param_t));
                param.volume_control.type = XR_HF_VOLUME_CONTROL_TARGET_MIC;
                param.volume_control.volume = p_data->val.value;

                btc_hf_client_cb_to_app(XR_HF_CLIENT_VOLUME_CONTROL_EVT, &param);
            } while (0);
            break;
        case BTA_HF_CLIENT_SPK_EVT:
            do {
                memset(&param, 0, sizeof(xr_hf_client_cb_param_t));
                param.volume_control.type = XR_HF_VOLUME_CONTROL_TARGET_SPK;
                param.volume_control.volume = p_data->val.value;
        ...
        case BTA_HF_CLIENT_VOICE_REC_EVT:
            do {
                ...
                btc_hf_client_cb_to_app(XR_HF_CLIENT_BVRA_EVT, &param);
            } while (0);
            break;
        case BTA_HF_CLIENT_OPERATOR_NAME_EVT:
            do {
                ...
                btc_hf_client_cb_to_app(XR_HF_CLIENT_COPS_CURRENT_OPERATOR_EVT, &param);
            } while (0);
            break;
        case BTA_HF_CLIENT_CLIP_EVT:
            do {
                ...
                btc_hf_client_cb_to_app(XR_HF_CLIENT_CLIP_EVT, &param);
            } while (0);
            break;
        case BTA_HF_CLIENT_BINP_EVT:
            do {
                ...
                btc_hf_client_cb_to_app(XR_HF_CLIENT_BINP_EVT, &param);
            } while (0);
            break;
        case BTA_HF_CLIENT_CCWA_EVT:
            do {
                ...
                btc_hf_client_cb_to_app(XR_HF_CLIENT_CCWA_EVT, &param);
            } while (0);
            break;
        case BTA_HF_CLIENT_AT_RESULT_EVT:
            do {
                ...
                btc_hf_client_cb_to_app(XR_HF_CLIENT_AT_RESPONSE_EVT, &param);
            } while (0);
            break;
        case BTA_HF_CLIENT_CLCC_EVT:
            do {
                ...
                param.clcc.dir = p_data->clcc.inc ? XR_HF_CURRENT_CALL_DIRECTION_INCOMING :
                    XR_HF_CURRENT_CALL_DIRECTION_OUTGOING;
                param.clcc.status = p_data->clcc.status;
                param.clcc.mpty = p_data->clcc.mpty ? XR_HF_CURRENT_CALL_MPTY_TYPE_MULTI :
                    XR_HF_CURRENT_CALL_MPTY_TYPE_SINGLE;
                param.clcc.number = p_data->clcc.number_present ? p_data->clcc.number : NULL;
                btc_hf_client_cb_to_app(XR_HF_CLIENT_CLCC_EVT, &param);
            } while (0);
            break;
        case BTA_HF_CLIENT_CNUM_EVT:
            do {
                ...
                if (p_data->cnum.service == 4) {
                    param.cnum.type = XR_HF_SUBSCRIBER_SERVICE_TYPE_VOICE;
                } else if (p_data->cnum.service == 5) {
                    param.cnum.type = XR_HF_SUBSCRIBER_SERVICE_TYPE_FAX;
                } else {
                    param.cnum.type = XR_HF_SUBSCRIBER_SERVICE_TYPE_UNKNOWN;
                }
                btc_hf_client_cb_to_app(XR_HF_CLIENT_CNUM_EVT, &param);
                break;
            } while (0);
            break;
        case BTA_HF_CLIENT_BTRH_EVT:
            if (p_data->val.value <= XR_HF_BTRH_STATUS_REJECTED) {
                do {
                    memset(&param, 0, sizeof(xr_hf_client_cb_param_t));
                    param.btrh.status = p_data->val.value;

                    btc_hf_client_cb_to_app(XR_HF_CLIENT_BTRH_EVT, &param);
                } while (0);
            }
            break;
        case BTA_HF_CLIENT_BSIR_EVT:
            ...
            {
                param.bsir.state = XR_HF_CLIENT_IN_BAND_RINGTONE_NOT_PROVIDED;
            }
            btc_hf_client_cb_to_app(XR_HF_CLIENT_BSIR_EVT, &param);
            break;
        case BTA_HF_CLIENT_AUDIO_OPEN_EVT:
            do {
                memset(&param, 0, sizeof(xr_hf_client_cb_param_t));
                param.audio_stat.state = XR_HF_CLIENT_AUDIO_STATE_CONNECTED;
        ...
        case BTA_HF_CLIENT_AUDIO_CLOSE_EVT:
            do {
                memset(&param, 0, sizeof(xr_hf_client_cb_param_t));
                param.audio_stat.state = XR_HF_CLIENT_AUDIO_STATE_DISCONNECTED;
                memcpy(param.audio_stat.remote_bda, &hf_client_local_param.btc_hf_client_cb.connected_bda,
                       sizeof(xr_bd_addr_t));
                btc_hf_client_cb_to_app(XR_HF_CLIENT_AUDIO_STATE_EVT, &param);
            } while (0);
        ...
        case BTA_HF_CLIENT_RING_INDICATION:
            do {
                btc_hf_client_cb_to_app(XR_HF_CLIENT_RING_IND_EVT, NULL);
            } while (0);
            break;
        default:
