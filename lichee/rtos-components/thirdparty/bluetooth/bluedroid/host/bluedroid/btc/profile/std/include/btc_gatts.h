...
typedef union {
    ...
    struct create_attr_tab_args{
        ...
        uint8_t max_nb_attr;
        xr_gatts_attr_db_t *gatts_attr_db;
    }create_attr_tab;
    ...
    struct add_char_args {
        ...
        xr_attr_control_t attr_control;
        xr_attr_value_t char_val;
    } add_char;
...
typedef struct {
    ...
    xr_bt_uuid_t svc_uuid;
    bool        is_tab_creat_svc;
    bool      is_use_svc;
    uint8_t   num_handle;
    uint8_t   handle_idx;
    uint16_t handles[XR_GATT_ATTR_HANDLE_MAX];
} xr_btc_creat_tab_t;

#if GATT_DYNAMIC_MEMORY == TRUE
extern xr_btc_creat_tab_t *btc_creat_tab_env_ptr;
