...
#define BTC_TIMEOUT_AV_GEN_MS           200
...
typedef struct {
    ...
    osi_alarm_t *av_gen_tmr; /* av general alarm */
    UINT8 av_gen_event;
} btc_av_cb_t;
...
******************************************************************************/
static inline void btc_a2d_cb_to_app(xr_a2d_cb_event_t event, xr_a2d_cb_param_t *param)
{
    xr_a2d_cb_t btc_a2d_cb = (xr_a2d_cb_t)btc_profile_cb_get(BTC_PID_A2DP);
    if (btc_a2d_cb) {
...
**  Static functions
******************************************************************************/
static void btc_report_connection_state(xr_a2d_connection_state_t state, bt_bdaddr_t *bd_addr, int disc_rsn)
{
    // todo: add callback for SRC
    xr_a2d_cb_param_t param;
    memset(&param, 0, sizeof(xr_a2d_cb_param_t));

    param.conn_stat.state = state;
    if (bd_addr) {
        memcpy(param.conn_stat.remote_bda, bd_addr, sizeof(xr_bd_addr_t));
    }
    if (state == XR_A2D_CONNECTION_STATE_DISCONNECTED) {
        param.conn_stat.disc_rsn = (disc_rsn == 0) ? XR_A2D_DISC_RSN_NORMAL :
                                   XR_A2D_DISC_RSN_ABNORMAL;
    }
    btc_a2d_cb_to_app(XR_A2D_CONNECTION_STATE_EVT, &param);
}

static void btc_report_audio_state(xr_a2d_audio_state_t state, bt_bdaddr_t *bd_addr)
{
    // todo: add callback for SRC
    xr_a2d_cb_param_t param;
    memset(&param, 0, sizeof(xr_a2d_cb_param_t));
    ...
    if (bd_addr) {
        memcpy(param.audio_stat.remote_bda, bd_addr, sizeof(xr_bd_addr_t));
    }
    btc_a2d_cb_to_app(XR_A2D_AUDIO_STATE_EVT, &param);
}
...
**
*******************************************************************************/
int sec_flag = BTA_SEC_AUTHENTICATE;
void set_sec_flag(int flag)
{
    switch(flag){
        case 0:sec_flag = BTA_SEC_NONE;break;
        case 1:sec_flag = BTA_SEC_AUTHORIZE;break;
        case 2:sec_flag = BTA_SEC_AUTHENTICATE;break;
        case 3:sec_flag = BTA_SEC_ENCRYPT;break;
        case 4:sec_flag = BTA_SEC_MODE4_LEVEL4;break;
        case 5:sec_flag = BTA_SEC_MITM;break;
    }
}
static BOOLEAN btc_av_state_idle_handler(btc_sm_event_t event, void *p_data)
{
    ...
    case BTC_AV_DISCONNECT_REQ_EVT:
        ...
        btc_report_connection_state(XR_A2D_CONNECTION_STATE_DISCONNECTED, &((btc_av_disconn_req_t *)p_data)->target_bda, 0);
        break;
...
static BOOLEAN btc_av_state_opening_handler(btc_sm_event_t event, void *p_data)
{
    ...
    case BTC_SM_ENTER_EVT:
        ...
        btc_report_connection_state(XR_A2D_CONNECTION_STATE_CONNECTING, &(btc_av_cb.peer_bda), 0);
        break;
    ...
    case BTA_AV_REJECT_EVT:
        ...
        btc_report_connection_state(XR_A2D_CONNECTION_STATE_DISCONNECTED, &(btc_av_cb.peer_bda), 0);
        btc_sm_change_state(btc_av_cb.sm_handle, BTC_AV_STATE_IDLE);
        break;

    case BTA_AV_OPEN_EVT: {
        tBTA_AV *p_bta_data = (tBTA_AV *)p_data;
        xr_a2d_connection_state_t conn_stat;
        ...
        if (p_bta_data->open.status == BTA_AV_SUCCESS) {
            ...
            conn_stat = XR_A2D_CONNECTION_STATE_CONNECTED;
            av_state = BTC_AV_STATE_OPENED;
        } else {
            BTC_TRACE_WARNING("BTA_AV_OPEN_EVT::FAILED status: %d\n", p_bta_data->open.status);

            conn_stat = XR_A2D_CONNECTION_STATE_DISCONNECTED;
    ...
    case BTC_AV_SINK_CONFIG_REQ_EVT: {
        if (btc_av_cb.peer_sep == AVDT_TSEP_SRC) {
            xr_a2d_cb_param_t param;
            memcpy(param.audio_cfg.remote_bda, &btc_av_cb.peer_bda, sizeof(xr_bd_addr_t));
            memcpy(&param.audio_cfg.mcc, p_data, sizeof(xr_a2d_mcc_t));
            btc_a2d_cb_to_app(XR_A2D_AUDIO_CFG_EVT, &param);
        }
    } break;
    ...
    case BTC_AV_CONNECT_REQ_EVT:
        ...
        if (memcmp ((bt_bdaddr_t *)p_data, &(btc_av_cb.peer_bda),
            ...
            btc_report_connection_state(XR_A2D_CONNECTION_STATE_DISCONNECTED, (bt_bdaddr_t *)p_data, 0);
            btc_queue_advance();
            break;
        }
...
static BOOLEAN btc_av_state_closing_handler(btc_sm_event_t event, void *p_data)
{
    ...
    case BTA_AV_CLOSE_EVT: {
        ...
        btc_report_connection_state(XR_A2D_CONNECTION_STATE_DISCONNECTED, &(btc_av_cb.peer_bda), close->disc_rsn);
        btc_sm_change_state(btc_av_cb.sm_handle, BTC_AV_STATE_IDLE);
        break;
    }
...
static BOOLEAN btc_av_state_opened_handler(btc_sm_event_t event, void *p_data)
{
    ...
    case BTC_AV_DISCONNECT_REQ_EVT:
        ...
        /* inform the application that we are disconnecting */
        btc_report_connection_state(XR_A2D_CONNECTION_STATE_DISCONNECTING, &(btc_av_cb.peer_bda), 0);
        break;
    ...
    case BTA_AV_CLOSE_EVT: {
        ...
        btc_report_connection_state(XR_A2D_CONNECTION_STATE_DISCONNECTED, &(btc_av_cb.peer_bda),
                                    close->disc_rsn);

        if (btc_av_cb.flags & BTC_AV_FLAG_PENDING_START) {
            btc_a2dp_control_command_ack(XR_A2D_MEDIA_CTRL_ACK_FAILURE);
            /* pending start flag will be cleared when exit current state */
        }
    ...
    case BTC_AV_CONNECT_REQ_EVT:
        if (memcmp (&((btc_av_connect_req_t *)p_data)->target_bda, &(btc_av_cb.peer_bda),
            ...
            btc_report_connection_state(XR_A2D_CONNECTION_STATE_DISCONNECTED,
                                        (bt_bdaddr_t *)p_data, XR_A2D_DISC_RSN_NORMAL);
        }
        btc_queue_advance();
        break;
...
static BOOLEAN btc_av_state_started_handler(btc_sm_event_t event, void *p_data)
{
    ...
    case BTC_SM_ENTER_EVT:
        ...
        btc_report_audio_state(XR_A2D_AUDIO_STATE_STARTED, &(btc_av_cb.peer_bda));
    ...
    case BTA_AV_SUSPEND_EVT:
        ...
        if (p_av->suspend.initiator != TRUE) {
            ...
            btc_report_audio_state(XR_A2D_AUDIO_STATE_REMOTE_SUSPEND, &(btc_av_cb.peer_bda));
        } else {
            btc_report_audio_state(XR_A2D_AUDIO_STATE_STOPPED, &(btc_av_cb.peer_bda));
        }
...
static void btc_av_gen_tmr_hdlr(void *arg)
{
    UNUSED(arg);
    btc_msg_t msg;
...
static bt_status_t btc_av_init(int service_id)
{
    ...
    btc_av_cb.av_gen_tmr = osi_alarm_new("AVgen", btc_av_gen_tmr_hdlr, &btc_av_cb.av_gen_event, BTC_TIMEOUT_AV_GEN_MS);
    ...
    if (btc_av_cb.sm_handle == NULL) {
        ...
        xr_a2d_cb_param_t param;
        memset(&param, 0, sizeof(xr_a2d_cb_param_t));
        param.a2d_prof_stat.init_state = XR_A2D_INIT_SUCCESS;
        btc_a2d_cb_to_app(XR_A2D_PROF_STATE_EVT, &param);
...
static void btc_av_safe_deinit(void)
{
#if A2D_DYNAMIC_MEMORY == TRUE
    if (!btc_av_cb_ptr)
        return;
    ...
    if (state == BTC_AV_STATE_IDLE) {
        if (btc_av_cb.av_gen_tmr) {
            osi_alarm_free(btc_av_cb.av_gen_tmr);
            btc_av_cb.av_gen_tmr = NULL;
        }
        ...
        xr_a2d_cb_param_t param;
        memset(&param, 0, sizeof(xr_a2d_cb_param_t));
        param.a2d_prof_stat.init_state = XR_A2D_DEINIT_SUCCESS;
        btc_a2d_cb_to_app(XR_A2D_PROF_STATE_EVT, &param);
    } else {
        /* as AV state machine not idle, to delay checking whether it can deinit.
         * Warning: av_gen_event should not be modified, before the timer expire.
         */
        btc_av_cb.av_gen_event = BTC_AV_DEINIT_SAFE_EVT;
        osi_alarm_set(btc_av_cb.av_gen_tmr, BTC_TIMEOUT_AV_GEN_MS);
    }
}
...
BOOLEAN btc_av_stream_ready(void)
{
    ...
    BTC_TRACE_DEBUG("btc_av_stream_ready : sm hdl %d, state %d, flags %x\n",
              (int)(uintptr_t)btc_av_cb.sm_handle, state, btc_av_cb.flags);
...
static void bte_av_media_callback(tBTA_AV_EVT event, tBTA_AV_MEDIA *p_data)
{
    ...
    if (event == BTA_AV_MEDIA_SINK_CFG_EVT) {
        ...
        if (a2d_status == A2D_SUCCESS) {
            ...
            arg.mcc.type = XR_A2D_MCT_SBC;
            memcpy(arg.mcc.cie.sbc, (uint8_t *)p_data + 3, XR_A2D_CIE_LEN_SBC);
...
bt_status_t btc_av_execute_service(BOOLEAN b_enable, UINT8 tsep)
{
    if (b_enable) {
        ...
        if (g_av_with_rc) {
            ...
            BTA_AvEnable(sec_flag, BTA_AV_FEAT_NO_SCO_SSPD, bte_av_callback);
