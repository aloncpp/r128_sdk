...
#define BLE_ISVALID_PARAM(x, min, max)  (((x) >= (min) && (x) <= (max)) || ((x) == XR_BLE_CONN_PARAM_UNDEF))
...
typedef union {
    ...
    struct config_adv_data_args {
        xr_ble_adv_data_t adv_data;
    } cfg_adv_data;
    //BTC_GAP_BLE_ACT_SET_SCAN_PARAM,
    struct set_scan_params_args {
        xr_ble_scan_params_t  scan_params;
    } set_scan_param;
    //BTC_GAP_BLE_ACT_START_SCAN,
    ...
    struct update_white_list_args {
        ...
        xr_ble_wl_addr_type_t wl_addr_type;
    } update_white_list;
    //BTC_GAP_BLE_UPDATE_DUPLICATE_SCAN_EXCEPTIONAL_LIST
    struct update_duplicate_exceptional_list_args {
        uint8_t  subcode;
        uint32_t  info_type;
        xr_duplicate_info_t device_info;
...
#define XR_GAP_DEVICE_NAME_MAX (32)
        char device_name[XR_GAP_DEVICE_NAME_MAX + 1];
    } set_dev_name;
    //BTC_GAP_BLE_ACT_CFG_ADV_DATA_RAW,
    ...
    struct set_encryption_args {
        xr_bd_addr_t bd_addr;
        xr_ble_sec_act_t sec_act;
    } set_encryption;
    //BTC_GAP_BLE_SET_SECURITY_PARAM_EVT
    struct set_security_param_args {
        xr_ble_sm_param_t param_type;
        uint8_t len;
        uint8_t *value;
    } set_security_param;
    //BTC_GAP_BLE_SECURITY_RSP_EVT
    ...
    struct read_rssi_args {
        xr_bd_addr_t remote_addr;
    } read_rssi;
    // BTC_GAP_BLE_SET_AFH_CHANNELS
    struct set_channels_args {
       xr_gap_ble_channels channels;
    } set_channels;
} btc_ble_gap_args_t;
