...
typedef union 
{
    ...
    struct vra_param {
        ...
        xr_hf_vr_state_t value;
    } vra_rep;

    // BTC_HF_VOLUME_CONTROL_EVT
    struct volcon_args {
        bt_bdaddr_t                     remote_addr;
        xr_hf_volume_control_target_t  target_type;
        int                             volume;
    } volcon;
    ...
    struct at_ok_err_args {
        bt_bdaddr_t               remote_addr;
        xr_hf_at_response_code_t response_code;
        xr_hf_cme_err_t          error_code;
    } ext_at;
    ...
    struct indchange_status {
        ...
        xr_hf_network_state_t     ntk_state;
        int                        signal;
    } ind_change;
    ...
    struct cind_args {
        ...
        xr_hf_network_state_t                   ntk_state;
        int                                      signal;
        xr_hf_roaming_status_t                  roam;
        int                                      batt_lev;
        xr_hf_call_held_status_t                call_held_state;
    } cind_rep;
    ...
    struct clcc_args {
        bt_bdaddr_t                     remote_addr;
        int                             index;
        xr_hf_current_call_direction_t dir;
        xr_hf_current_call_status_t    current_call_state;
        xr_hf_current_call_mode_t      mode;
        xr_hf_current_call_mpty_type_t mpty;
        char                            *number;
        xr_hf_call_addr_type_t         type;
    } clcc_rep;
    ...
    struct cnum_args {
        bt_bdaddr_t                      remote_addr;
        char                             *number;
        xr_hf_subscriber_service_type_t type;
    } cnum_rep;
    ...
    struct bsir_args {
        bt_bdaddr_t remote_addr;
        xr_hf_in_band_ring_state_t state;  
    } bsir;
    ...
    struct phone_args {
        ...
        xr_hf_call_setup_status_t call_setup_state;
        char                       *number;
        xr_hf_call_addr_type_t    call_addr_type;
    } phone;

    // BTC_HF_REGISTER_DATA_CALLBACK_EVT
    struct reg_data_callback {
        xr_hf_incoming_data_cb_t recv;
        xr_hf_outgoing_data_cb_t send;
    } reg_data_cb;
...
typedef struct
{
    ...
    xr_hf_connection_state_t          connection_state;
    xr_hf_vr_state_t                  vr_state;
    int                                num_active;
    int                                num_held;
    xr_hf_call_status_t               call_state;
    xr_hf_call_setup_status_t         call_setup_state;
...
typedef struct
{
    ...
    xr_hf_incoming_data_cb_t          btc_hf_incoming_data_cb;
    xr_hf_outgoing_data_cb_t          btc_hf_outgoing_data_cb;
} hf_local_param_t;
