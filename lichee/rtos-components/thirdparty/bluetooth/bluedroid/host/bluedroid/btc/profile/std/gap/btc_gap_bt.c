...
static inline void btc_gap_bt_cb_to_app(xr_bt_gap_cb_event_t event, xr_bt_gap_cb_param_t *param)
{
    xr_bt_gap_cb_t cb = (xr_bt_gap_cb_t)btc_profile_cb_get(BTC_PID_GAP_BT);
    if (cb) {
        cb(event, param);
    }
}

static void btc_bt_set_scan_mode(xr_bt_connection_mode_t c_mode, xr_bt_discovery_mode_t d_mode, xr_bt_paired_mode_t pair_mode, xr_bt_paired_con_mode_t conn_paired_only)
{
    tBTA_DM_DISC disc_mode;
    tBTA_DM_CONN conn_mode;

    switch (c_mode) {
    case XR_BT_NON_CONNECTABLE:
        conn_mode = BTA_DM_NON_CONN;
        break;
    case XR_BT_CONNECTABLE:
    ...
    switch (d_mode) {
    case XR_BT_NON_DISCOVERABLE:
        disc_mode = BTA_DM_NON_DISC;
        break;
    case XR_BT_LIMITED_DISCOVERABLE:
        disc_mode = BTA_DM_LIMITED_DISC;
        break;
    case XR_BT_GENERAL_DISCOVERABLE:
    ...
    BTA_DmSetVisibility(disc_mode, conn_mode, pair_mode, conn_paired_only);
    return;
}

static void btc_set_ipscan_param(uint32_t window, uint32_t interval, uint8_t type)
{
    BTA_DmSetIpscanParam(window, interval, type);
    return;
}
...
static void btc_gap_bt_start_discovery(btc_gap_bt_args_t *arg)
{
    ...
    inq_params.mode = (arg->start_disc.mode == XR_BT_INQ_MODE_GENERAL_INQUIRY) ?
...
{
    xr_bt_uuid_t *uuid = &arg->get_rmt_srv_rcd.uuid;
...
static void bte_search_devices_evt(tBTA_DM_SEARCH_EVT event, tBTA_DM_SEARCH *p_data)
{
    ...
    if (event == BTA_DM_INQ_RES_EVT) {
        p_data->inq_res.remt_name_not_required = check_eir_remote_name(p_data, (char *)name, &length);
        BCM_STRNCPY_S((char *)p_data->inq_res.bd_name,(char *)name, length + 1);
    }
...
static void btc_gap_bt_search_devices_evt(tBTA_DM_SEARCH_PARAM *p_data)
{
    ...
    case BTA_DM_DISC_RES_EVT: {
        ...
        if (bdname_len) {
            xr_bt_gap_dev_prop_t prop[1];

            BTC_STORAGE_FILL_PROPERTY(&prop[0], XR_BT_GAP_DEV_PROP_BDNAME, bdname_len + 1, p_data->p_data->disc_res.bd_name);

            xr_bt_gap_cb_param_t param;
            bdcpy(param.disc_res.bda, p_data->p_data->disc_res.bd_addr);
            param.disc_res.num_prop = 1;
            param.disc_res.prop = prop;
            btc_gap_bt_cb_to_app(XR_BT_GAP_DISC_RES_EVT, &param);
        }
        break;
    }
    case BTA_DM_INQ_RES_EVT: {
        ...
        do {
            xr_bt_gap_dev_prop_t prop[4];
            int num_prop = 0;
            uint32_t bdname_len = strlen((const char *)p_data->p_data->inq_res.bd_name);

            memset(prop, 0, sizeof(prop));
            if (bdname_len) {
                BTC_STORAGE_FILL_PROPERTY(&prop[0], XR_BT_GAP_DEV_PROP_BDNAME, bdname_len + 1, p_data->p_data->inq_res.bd_name);
            }

            BTC_STORAGE_FILL_PROPERTY(&prop[1], XR_BT_GAP_DEV_PROP_COD, sizeof(cod), &cod);
            num_prop++;

            BTC_STORAGE_FILL_PROPERTY(&prop[2], XR_BT_GAP_DEV_PROP_RSSI, 1, &(p_data->p_data->inq_res.rssi));
            num_prop++;

            if (p_data->p_data->inq_res.p_eir) {
                BTC_STORAGE_FILL_PROPERTY(&prop[3], XR_BT_GAP_DEV_PROP_EIR, HCI_EXT_INQ_RESPONSE_LEN, p_data->p_data->inq_res.p_eir);
                num_prop++;
            }
    ...
    case BTA_DM_DISC_CMPL_EVT: {
        xr_bt_gap_cb_param_t param;
        param.disc_st_chg.state = XR_BT_GAP_DISCOVERY_STOPPED;
        btc_gap_bt_cb_to_app(XR_BT_GAP_DISC_STATE_CHANGED_EVT, &param);
        break;
    }
...
static void btc_gap_bt_search_service_record(char *p_param)
{
    ...
    case BTA_DM_DISC_RES_EVT: {
        ...
        if (p_data->p_data->disc_res.p_raw_data && p_data->p_data->disc_res.raw_data_size > 0) {
            param.rmt_srvc_rec.stat = XR_BT_STATUS_SUCCESS;
            ...
            param.rmt_srvc_rec.stat = XR_BT_STATUS_FAIL;
            // param.rmt_srvc_rec.raw_data_size = 0;
            // param.rmt_srvc_rec.raw_data = NULL;
        }
        btc_gap_bt_cb_to_app(XR_BT_GAP_RMT_SRVC_REC_EVT, &param);
    }
    break;
    case BTA_DM_DISC_CMPL_EVT:
    default:
...
static void btc_gap_bt_search_services(char *p_param)
{
    ...
    case BTA_DM_DISC_RES_EVT: {
        xr_bt_gap_cb_param_t param;
        xr_bt_uuid_t *uuid_list = NULL;
        memcpy(param.rmt_srvcs.bda, p_data->p_data->disc_res.bd_addr, BD_ADDR_LEN);

        param.rmt_srvcs.stat = XR_BT_STATUS_FAIL;
        if (p_data->p_data->disc_res.result == BTA_SUCCESS) {
            uuid_list = osi_malloc(sizeof(xr_bt_uuid_t) * p_data->p_data->disc_res.num_uuids);
            if (uuid_list) {
                param.rmt_srvcs.stat = XR_BT_STATUS_SUCCESS;
                ...
                xr_bt_uuid_t *o_uu = uuid_list;
                for (int i = 0; i < p_data->p_data->disc_res.num_uuids; i++, i_uu += XR_UUID_LEN_128, o_uu++) {
                    uuid128_be_to_xr_uuid(o_uu, i_uu);
                }
            }
        }

        if (param.rmt_srvcs.stat == XR_BT_STATUS_FAIL) {
            param.rmt_srvcs.num_uuids = 0;
            param.rmt_srvcs.uuid_list = NULL;
        }
        btc_gap_bt_cb_to_app(XR_BT_GAP_RMT_SRVCS_EVT, &param);
...
{
    tBTA_UTL_COD p_cod;
    xr_bt_cod_t *cod = &(arg->set_cod.cod);
...
static void btc_gap_bt_read_rssi_delta_cmpl_callback(void *p_data)
{
    ...
    param.read_rssi_delta.stat = btc_btm_status_to_xr_status(result->status);
    param.read_rssi_delta.rssi_delta = result->rssi;

    ret = btc_transfer_context(&msg, &param,
                               sizeof(xr_bt_gap_cb_param_t), NULL);
...
static xr_err_t btc_gap_bt_remove_bond_device(btc_gap_bt_args_t *arg)
{
    BD_ADDR bd_addr;
...
static xr_err_t btc_gap_bt_set_security_param(btc_gap_bt_args_t *arg)
{
    xr_err_t ret;
    switch(arg->set_security_param.param_type) {
    case XR_BT_SP_IOCAP_MODE:{
        uint8_t iocap = 0;
...
static void btc_gap_bt_config_eir(btc_gap_bt_args_t *arg)
{
    ...
    xr_bt_eir_data_t *eir_data = &arg->config_eir.eir_data;
...
static void btc_gap_bt_set_afh_channels_cmpl_callback(void *p_data)
{
    ...
    param.set_afh_channels.stat = btc_btm_status_to_xr_status(result->status);
...
static void btc_gap_bt_read_remote_name_cmpl_callback(void *p_data)
{
    ...
    param.read_rmt_name.stat = btc_btm_status_to_xr_status(result->status);
    memcpy(param.read_rmt_name.rmt_name,result->remote_bd_name,XR_BT_GAP_MAX_BDNAME_LEN);

    ret = btc_transfer_context(&msg, &param, sizeof(xr_bt_gap_cb_param_t), NULL);
    if (ret != BT_STATUS_SUCCESS) {
        BTC_TRACE_ERROR("%s btc_transfer_context failed\n", __func__);
    }
}
...
static void btc_gap_bt_set_sniff_mode(btc_gap_bt_args_t *arg)
{
    BD_ADDR bd_addr;
    memcpy(bd_addr, arg->config_sniff.bda.address, sizeof(BD_ADDR));
    if (arg->config_sniff.sniff_mode == XR_BT_SNIFF_MODE_ON) {
        BTM_SetSniffMode(bd_addr, arg->config_sniff.min_period,
            arg->config_sniff.max_period, arg->config_sniff.attempts,
            arg->config_sniff.timeout);
    } else {
        BTM_CancelSniffMode(bd_addr);
    }
}

static void btc_gap_bt_read_power_mode(btc_gap_bt_args_t *arg)
{
    BD_ADDR bd_addr;
    xr_bt_gap_cb_param_t param;
    tBTM_PM_MODE p_mode;
    ...
    memcpy(bd_addr, arg->config_sniff.bda.address, sizeof(BD_ADDR));
    BTM_ReadPowerMode(bd_addr, &p_mode);

    param.power_mode.mode = p_mode;
    btc_gap_bt_cb_to_app(XR_BT_GAP_POWER_MODE_EVT, &param);
}


static void btc_gap_bt_set_role_cmpl_callback(void *p_data)
{
    tBTM_ROLE_SWITCH_CMPL *result = (tBTM_ROLE_SWITCH_CMPL *)p_data;
    xr_bt_gap_cb_param_t param;
    btc_msg_t msg;
    bt_status_t ret;
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_GAP_BT;
    msg.act = BTC_GAP_BT_SET_ROLE_COMPLETE_EVT;

    param.set_role_cmpl.status = btc_btm_status_to_xr_status(result->hci_status);
    memcpy(param.set_role_cmpl.bda, result->remote_bd_addr, sizeof(BD_ADDR));
    param.set_role_cmpl.new_role = result->role;
...
static void btc_gap_bt_set_role(btc_gap_bt_args_t *arg)
{
    BD_ADDR bd_addr;
    UINT8 Role;
    memcpy(bd_addr, arg->config_role.bda.address, sizeof(BD_ADDR));
    BTM_GetRole(bd_addr, &Role);
    if (Role == (UINT8)(arg->config_role.role)) {
        BTC_TRACE_WARNING("%s current role is %d, cannot switch role\n",__func__, Role);
        return;
    }

    //BTM_SwitchRole (bd_addr, (UINT8)(arg->config_role.role_mode), NULL);
    BTM_SwitchRole (bd_addr, (UINT8)(arg->config_role.role), btc_gap_bt_set_role_cmpl_callback);
}

static void btc_gap_bt_read_role(btc_gap_bt_args_t *arg)
{
    BD_ADDR bd_addr;
    UINT8 Role;
    tBTM_STATUS status;
    xr_bt_gap_cb_param_t param;
    memcpy(bd_addr, arg->read_role.bda.address, sizeof(BD_ADDR));
    status = BTM_GetRole(bd_addr, &Role);

    param.get_role_cmpl.status = status;
    memcpy(param.get_role_cmpl.bda, arg->read_role.bda.address, sizeof(BD_ADDR));
    param.get_role_cmpl.role = Role;

    btc_gap_bt_cb_to_app(XR_BT_GAP_GET_ROLE_COMPLETE_EVT, &param);
}

static void btc_gap_bt_set_auto_sniff_mode(btc_gap_bt_args_t *arg)
{
    BTA_DmSetAutoSniffMode(arg->auto_sniff_mode.mode);
}
...
void btc_gap_bt_arg_deep_copy(btc_msg_t *msg, void *p_dest, void *p_src)
{
    ...
    case BTC_GAP_BT_ACT_SET_IPSCAN_PARAM:
    ...
    case BTC_GAP_BT_ACT_SET_ROLE:
    case BTC_GAP_BT_ACT_SET_AUTO_SNIFF_MODE:
        break;
#if (BT_SSP_INCLUDED == TRUE)
...
void btc_gap_bt_arg_deep_free(btc_msg_t *msg)
{
    ...
    case BTC_GAP_BT_ACT_SET_AFH_CHANNELS:
    case BTC_GAP_BT_ACT_READ_REMOTE_NAME:
    case BTC_GAP_BT_ACT_SET_SNIFF_MODE:
    case BTC_GAP_BT_ACT_SET_ROLE:
    case BTC_GAP_BT_ACT_POWER_MODE:
    case BTC_GAP_BT_ACT_READ_ROLE:
    case BTC_GAP_BT_ACT_SET_AUTO_SNIFF_MODE:
        break;
#if (BT_SSP_INCLUDED == TRUE)
    case BTC_GAP_BT_ACT_PASSKEY_REPLY:
...
void btc_gap_bt_call_handler(btc_msg_t *msg)
{
    ...
    switch (msg->act) {
    case BTC_GAP_BT_ACT_SET_SCAN_MODE: {
        btc_bt_set_scan_mode(arg->set_scan_mode.c_mode, arg->set_scan_mode.d_mode, arg->set_scan_mode.pair_mode, arg->set_scan_mode.conn_paired_only);
        break;
    }
    case BTC_GAP_BT_ACT_SET_IPSCAN_PARAM: {
        btc_set_ipscan_param(arg->set_ipscan_param.window, arg->set_ipscan_param.interval, arg->set_ipscan_param.type);
        break;
    }
    case BTC_GAP_BT_ACT_START_DISCOVERY: {
    ...
    case BTC_GAP_BT_ACT_SET_SNIFF_MODE: {
        btc_gap_bt_set_sniff_mode(arg);
        break;
    }
    case BTC_GAP_BT_ACT_SET_ROLE: {
        btc_gap_bt_set_role(arg);
        break;
    }
    case BTC_GAP_BT_ACT_POWER_MODE: {
        btc_gap_bt_read_power_mode(arg);
        break;
    }
    case BTC_GAP_BT_ACT_READ_ROLE: {
        btc_gap_bt_read_role(arg);
        break;
    }
    case BTC_GAP_BT_ACT_SET_AUTO_SNIFF_MODE: {
        btc_gap_bt_set_auto_sniff_mode(arg);
        break;
    }
    default:
        break;
...
void btc_gap_bt_busy_level_updated(uint8_t bl_flags)
{
    ...
    if (bl_flags == BTM_BL_INQUIRY_STARTED) {
        param.disc_st_chg.state = XR_BT_GAP_DISCOVERY_STARTED;
        btc_gap_bt_cb_to_app(XR_BT_GAP_DISC_STATE_CHANGED_EVT, &param);
...
#endif ///BT_SSP_INCLUDED == TRUE
    case BTC_GAP_BT_SET_ROLE_COMPLETE_EVT:
    case BTC_GAP_BT_READ_LOCAL_NAME_EVT:
        break;
    default:
...
void btc_gap_bt_cb_handler(btc_msg_t *msg)
{
    ...
    case BTC_GAP_BT_READ_RSSI_DELTA_EVT:{
        btc_gap_bt_cb_to_app(XR_BT_GAP_READ_RSSI_DELTA_EVT, (xr_bt_gap_cb_param_t *)msg->arg);
        break;
    }
    case BTC_GAP_BT_CONFIG_EIR_DATA_EVT: {
        btc_gap_bt_cb_to_app(XR_BT_GAP_CONFIG_EIR_DATA_EVT, (xr_bt_gap_cb_param_t *)msg->arg);
        break;
    }
    case BTC_GAP_BT_AUTH_CMPL_EVT:{
        btc_gap_bt_cb_to_app(XR_BT_GAP_AUTH_CMPL_EVT, (xr_bt_gap_cb_param_t *)msg->arg);
        break;
    }
    case BTC_GAP_BT_PIN_REQ_EVT:{
        btc_gap_bt_cb_to_app(XR_BT_GAP_PIN_REQ_EVT, (xr_bt_gap_cb_param_t *)msg->arg);
        break;
    }
#if (BT_SSP_INCLUDED == TRUE)
    case BTC_GAP_BT_CFM_REQ_EVT:{
        btc_gap_bt_cb_to_app(XR_BT_GAP_CFM_REQ_EVT, (xr_bt_gap_cb_param_t *)msg->arg);
        break;
    }
    case BTC_GAP_BT_KEY_NOTIF_EVT:{
        btc_gap_bt_cb_to_app(XR_BT_GAP_KEY_NOTIF_EVT, (xr_bt_gap_cb_param_t *)msg->arg);
        break;
    }
    case BTC_GAP_BT_READ_LOCAL_NAME_EVT:{
        btc_gap_bt_cb_to_app(XR_BT_GAP_READ_LOCAL_NAME_EVT, (xr_bt_gap_cb_param_t *)msg->arg);
        break;
    }
    case BTC_GAP_BT_KEY_REQ_EVT:{
        btc_gap_bt_cb_to_app(XR_BT_GAP_KEY_REQ_EVT, (xr_bt_gap_cb_param_t *)msg->arg);
        break;
    }
#endif ///BT_SSP_INCLUDED == TRUE
    case BTC_GAP_BT_SET_AFH_CHANNELS_EVT:{
        btc_gap_bt_cb_to_app(XR_BT_GAP_SET_AFH_CHANNELS_EVT, (xr_bt_gap_cb_param_t *)msg->arg);
        break;
    }
#if (SDP_INCLUDED == TRUE)
    case BTC_GAP_BT_READ_REMOTE_NAME_EVT:{
        btc_gap_bt_cb_to_app(XR_BT_GAP_READ_REMOTE_NAME_EVT,(xr_bt_gap_cb_param_t *)msg->arg);
        break;
    }
#endif
    case BTC_GAP_BT_REMOVE_BOND_DEV_COMPLETE_EVT:{
        btc_gap_bt_cb_to_app(XR_BT_GAP_REMOVE_BOND_DEV_COMPLETE_EVT,(xr_bt_gap_cb_param_t *)msg->arg);
        break;
    }
    case BTC_GAP_BT_SET_ROLE_COMPLETE_EVT:{
        btc_gap_bt_cb_to_app(XR_BT_GAP_SET_ROLE_COMPLETE_EVT,(xr_bt_gap_cb_param_t *)msg->arg);
        break;
    }
    default:
        BTC_TRACE_ERROR("%s: Unhandled event (%d)!\n", __FUNCTION__, msg->act);
