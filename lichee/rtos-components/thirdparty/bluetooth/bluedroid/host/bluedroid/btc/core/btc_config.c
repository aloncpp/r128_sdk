...
bool btc_config_clean_up(void)
{
    ...
    osi_mutex_free(&lock);
    config = NULL;
#ifdef CONFIG_BLUEDROID_USE_SETTINGS
#if defined(CONFIG_BT_DEINIT) //todo: need to modify
    settings_subsys_deinit();
#endif
#endif
    return true;
}
...
bool btc_config_set_int(const char *section, const char *key, int value)
{
    ...
    return config_set_int(config, section, key, value);
}
...
bool btc_config_set_str(const char *section, const char *key, const char *value)
{
    ...
    return config_set_string(config, section, key, value, false);
}
...
bool btc_config_set_bin(const char *section, const char *key, const uint8_t *value, size_t length)
{
    ...
    ret = config_set_string(config, section, key, str, false);
