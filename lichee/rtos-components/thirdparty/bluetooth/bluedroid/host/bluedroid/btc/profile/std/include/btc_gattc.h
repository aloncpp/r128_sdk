...
typedef union {
    ...
    struct app_unreg_arg {
        xr_gatt_if_t gattc_if;
    } app_unreg;
    //BTC_GATTC_ACT_OPEN,
    struct open_arg {
        xr_gatt_if_t gattc_if;
        xr_bd_addr_t remote_bda;
        xr_ble_addr_type_t remote_addr_type;
        bool is_direct;
    } open;
    //BTC_GATTC_ACT_CLOSE,
    struct close_arg {
    ...
    struct read_multiple_arg {
        uint16_t conn_id;
        uint8_t  num_attr;
        uint16_t handles[XR_GATT_MAX_READ_MULTI_HANDLES];
        xr_gatt_auth_req_t auth_req;
    } read_multiple;
    //BTC_GATTC_ACT_READ_CHAR_DESCR,
    ...
    struct cache_assoc_arg {
        xr_gatt_if_t gattc_if;
        xr_bd_addr_t src_addr;
        xr_bd_addr_t assoc_addr;
        bool is_assoc;
    } cache_assoc;
    //BTC_GATTC_ATC_CACHE_GET_ADDR_LIST
