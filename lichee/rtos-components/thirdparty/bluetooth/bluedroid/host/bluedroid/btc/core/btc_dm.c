...
static void btc_dm_auth_cmpl_evt (tBTA_DM_AUTH_CMPL *p_auth_cmpl)
{
    ...
    if ( (p_auth_cmpl->success == TRUE) && (p_auth_cmpl->key_present) ) {
        ...
        if ((p_auth_cmpl->key_type < HCI_LKEY_TYPE_DEBUG_COMB) ||
            ...
            if (1) {
                ...
                ret = btc_storage_add_bonded_device(&bd_addr, p_auth_cmpl->key,
                                                    p_auth_cmpl->key_type, 16,
                                                    p_auth_cmpl->sc_support, &p_auth_cmpl->bd_name);
    ...
    memcpy(param.auth_cmpl.bda, p_auth_cmpl->bd_addr, XR_BD_ADDR_LEN);
    memcpy(param.auth_cmpl.device_name, p_auth_cmpl->bd_name, XR_BT_GAP_MAX_BDNAME_LEN + 1);
...
static void btc_dm_pin_req_evt(tBTA_DM_PIN_REQ *p_pin_req)
{
    ...
    memcpy(param.pin_req.bda, p_pin_req->bd_addr, XR_BD_ADDR_LEN);
...
static void btc_dm_sp_cfm_req_evt(tBTA_DM_SP_CFM_REQ *p_cfm_req)
{
    ...
    memcpy(param.cfm_req.bda, p_cfm_req->bd_addr, XR_BD_ADDR_LEN);
...
static void btc_dm_sp_key_notif_evt(tBTA_DM_SP_KEY_NOTIF *p_key_notif)
{
    ...
    memcpy(param.key_notif.bda, p_key_notif->bd_addr, XR_BD_ADDR_LEN);
...
static void btc_dm_sp_key_req_evt(tBTA_DM_SP_KEY_REQ *p_key_req)
{
    ...
    memcpy(param.key_req.bda, p_key_req->bd_addr, XR_BD_ADDR_LEN);
...
static void btc_dm_read_local_name_evt(tBTA_DM_READ_LOCAL_NAME *read_name_req)
{
#if (BTC_GAP_BT_INCLUDED == TRUE)
    xr_bt_gap_cb_param_t param;
    bt_status_t ret;
    ...
    msg.sig = BTC_SIG_API_CB;
    msg.pid = BTC_PID_GAP_BT;
    msg.act = BTC_GAP_BT_READ_LOCAL_NAME_EVT;
    param.read_local_name.name = read_name_req->name;
...
static void btc_dm_dev_unpaired_evt(tBTA_DM_LINK_DOWN *p_link_down)
{
    ...
    memcpy(param.remove_bond_dev_cmpl.bda, p_link_down->bd_addr, XR_BD_ADDR_LEN);
    ...
    if (p_link_down->status == HCI_SUCCESS) {
        ...
        param.remove_bond_dev_cmpl.status = XR_BT_STATUS_FAIL;
    }
...
void btc_dm_sec_cb_handler(btc_msg_t *msg)
{
    ...
    case BTA_DM_ENABLE_EVT: {
        ...
        if (p_data->enable.status == BTA_SUCCESS) {
            const char *initial_device_name = "Xradio";
    ...
    case BTA_DM_BLE_DEV_UNPAIRED_EVT: {
        ...
        ble_msg.act = XR_GAP_BLE_REMOVE_BOND_DEV_COMPLETE_EVT;
    ...
    case BTA_DM_BLE_AUTH_CMPL_EVT: {
        rsp_app = true;
        ble_msg.act = XR_GAP_BLE_AUTH_CMPL_EVT;
    ...
    case BTA_DM_BLE_KEY_EVT: {
        rsp_app = true;
        ble_msg.act = XR_GAP_BLE_KEY_EVT;
        ...
        switch (p_data->ble_key.key_type) {
            ...
            case BTM_LE_KEY_PID: {
                ...
                memcpy(&param.ble_security.ble_key.p_key_value.pid_key.irk,
                             &p_data->ble_key.p_key_value->pid_key.irk, XR_BT_OCTET16_LEN);
                ...
                memcpy(&param.ble_security.ble_key.p_key_value.pid_key.static_addr,
                             &p_data->ble_key.p_key_value->pid_key.static_addr, XR_BD_ADDR_LEN);
                break;
            }
    ...
    case BTA_DM_BLE_PASSKEY_REQ_EVT: {
        rsp_app = true;
        ble_msg.act = XR_GAP_BLE_PASSKEY_REQ_EVT;
        memcpy(param.ble_security.ble_req.bd_addr, p_data->ble_req.bd_addr, BD_ADDR_LEN);
        break;
    }
    case BTA_DM_BLE_OOB_REQ_EVT: {
        rsp_app = true;
        ble_msg.act = XR_GAP_BLE_OOB_REQ_EVT;
        memcpy(param.ble_security.ble_req.bd_addr, p_data->ble_req.bd_addr, BD_ADDR_LEN);
        break;
    }
    case BTA_DM_BLE_LOCAL_IR_EVT: {
        rsp_app = true;
        ble_msg.act = XR_GAP_BLE_LOCAL_IR_EVT;
    ...
    case BTA_DM_BLE_LOCAL_ER_EVT: {
        rsp_app = true;
        ble_msg.act = XR_GAP_BLE_LOCAL_ER_EVT;
        memcpy(&param.ble_security.ble_id_keys, &p_data->ble_id_keys, sizeof(tBTA_BLE_LOCAL_ID_KEYS));
    ...
    case BTA_DM_BLE_NC_REQ_EVT: {
        rsp_app = true;
        ble_msg.act = XR_GAP_BLE_NC_REQ_EVT;
    ...
    case BTA_DM_ROLE_CHG_EVT:
        BTC_TRACE_DEBUG( "btc_dm_sec_cback : unhandled event (%d)\n", msg->act );
        break;
    case BTA_DM_READ_LOCAL_NAME_EVT:
        btc_dm_read_local_name_evt(&p_data->read_local_name);
        break;
    default:
        BTC_TRACE_DEBUG( "btc_dm_sec_cback : unhandled event (%d)\n", msg->act );
        break;
