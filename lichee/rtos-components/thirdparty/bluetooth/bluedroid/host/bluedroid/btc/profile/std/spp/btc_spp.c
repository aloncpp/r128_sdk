...
#include "osi/list.h"
//#include "freertos/ringbuf.h" //zipmodi
#include "osi/mutex.h"
#include "osi/alarm.h"
#include <sys/errno.h>
#include <sys/lock.h>
...
#if 0 //origin
#include "xr_vfs.h"
#include "xr_vfs_dev.h"
#endif //zipmodi
#include "stack/port_api.h"
#include "FreeRTOS.h" //zipmodi
#include "event_groups.h" //zipmodi
...
typedef struct {
    ...
    xr_spp_role_t role;
    xr_spp_sec_t security;
    xr_bd_addr_t addr;
    slot_data_t rx;
    slot_data_t tx;
    uint8_t service_uuid[16];
    char service_name[XR_SPP_SERVER_NAME_MAX + 1];
} spp_slot_t;

typedef int xr_vfs_id_t;

typedef struct {
    spp_slot_t *spp_slots[MAX_RFC_PORTS + 1];
    uint32_t spp_slot_id;
    xr_spp_mode_t spp_mode;
    osi_mutex_t spp_slot_mutex;
    EventGroupHandle_t tx_event_group;
    xr_vfs_id_t spp_vfs_id;
} spp_local_param_t;
...
static spp_slot_t *spp_malloc_slot(void)
{
    ...
    for (size_t i = 1; i <= MAX_RFC_PORTS; i++) {
        ...
        if ((*slot) == NULL) {
            ...
            if (spp_local_param.spp_mode == XR_SPP_MODE_VFS) {
#if 0
                if (xr_vfs_register_fd(spp_local_param.spp_vfs_id, &(*slot)->fd) != XR_OK) {
                    BTC_TRACE_ERROR("%s unable to register fd!", __func__);
                    err_no = 3;
                    break;
                }
#else
                BTC_TRACE_WARNING("%s unable to register fd in this version!", __func__);
#endif
            }
            return (*slot);
        }
...
static void spp_free_slot(spp_slot_t *slot)
{
    ...
    if (spp_local_param.spp_mode == XR_SPP_MODE_VFS) {
#if 0
        (void) xr_vfs_unregister_fd(spp_local_param.spp_vfs_id, slot->fd);
#else
        BTC_TRACE_WARNING("%s can not unregister_fd in this version", __func__);
#endif
...
static inline void btc_spp_cb_to_app(xr_spp_cb_event_t event, xr_spp_cb_param_t *param)
{
    xr_spp_cb_t *btc_spp_cb = (xr_spp_cb_t *)btc_profile_cb_get(BTC_PID_SPP);
    if (btc_spp_cb) {
...
static void *btc_spp_rfcomm_inter_cb(tBTA_JV_EVT event, tBTA_JV *p_data, void *user_data)
{
    ...
    case BTA_JV_RFCOMM_START_EVT:
        ...
        if (!slot) {
            ...
            p_data->rfc_start.status = XR_SPP_NO_CONNECTION;
            break;
        }
    ...
    case BTA_JV_RFCOMM_SRV_OPEN_EVT:
        slot = p_data->rfc_srv_open.handle ? spp_find_slot_by_id(id) : spp_find_slot_by_scn((uint32_t)(uintptr_t)user_data);
        if (!slot) {
            BTC_TRACE_ERROR("%s unable to find RFCOMM slot!", __func__);
            p_data->rfc_srv_open.status = XR_SPP_NO_CONNECTION;
            break;
        }

        if (p_data->rfc_srv_open.handle) {
            new_user_data = (void *)(uintptr_t)slot->id;
            memcpy(slot->addr, p_data->rfc_srv_open.rem_bda, XR_BD_ADDR_LEN);
            slot->connected = TRUE;
        ...
        if (p_data->rfc_srv_open.handle != p_data->rfc_srv_open.new_listen_handle) {
            ...
            if (!slot_new) {
                ...
                p_data->rfc_srv_open.status = XR_SPP_NO_RESOURCE;
                break;
            }
    ...
    case BTA_JV_RFCOMM_OPEN_EVT:
        ...
        if (!slot) {
            BTC_TRACE_ERROR("%s unable to find RFCOMM slot!", __func__);
            p_data->rfc_open.status = XR_SPP_NO_CONNECTION;
            break;
        }
        slot->connected = TRUE;
    ...
    case BTA_JV_RFCOMM_CLOSE_EVT:
        ...
        if (!slot) {
            BTC_TRACE_ERROR("%s unable to find RFCOMM slot!", __func__);
            p_data->rfc_close.status = XR_SPP_NO_CONNECTION;
            break;
        }
        if (slot->connected && p_data->rfc_close.port_status != PORT_LOCAL_CLOSED) {
            BTA_JvRfcommClose(slot->rfc_handle, NULL, (void *)(uintptr_t)slot->id);
        }
    ...
    case BTA_JV_FREE_SCN_EVT:
        if (user_data) {
            ...
            if (slot) {
                ...
                BTC_TRACE_ERROR("%s unable to find RFCOMM slot!", __func__);
                p_data->free_scn.status = XR_SPP_NO_CONNECTION;
            }
            osi_free(user_data);
        }
        break;
...
static void btc_spp_dm_inter_cb(tBTA_JV_EVT event, tBTA_JV *p_data, void *user_data)
{
    ...
    case BTA_JV_GET_SCN_EVT:
        ...
        slot->scn = p_data->scn;
        BTA_JvCreateRecordByUser(slot->service_name, slot->scn, (void *)(uintptr_t)slot->id);
        osi_mutex_unlock(&spp_local_param.spp_slot_mutex);
        break;
    case BTA_JV_CREATE_RECORD_EVT:
        ...
        if (p_data->create_rec.status == BTA_JV_SUCCESS) {
            ...
            BTA_JvRfcommStartServer(slot->security, slot->role, slot->scn,
                                    slot->max_session, (tBTA_JV_RFCOMM_CBACK *)btc_spp_rfcomm_inter_cb, (void *)(uintptr_t)slot->id);
        } else {
    ...
    case BTA_JV_DISCOVERY_COMP_EVT:
        memset(&tmp_jv, 0, sizeof(tBTA_JV));
        if (!p_data->disc_comp.status) {
            memcpy(&tmp_jv, p_data, sizeof(tBTA_JV_DISCOVERY_COMP));
        } else {
            tmp_jv.disc_comp.status = p_data->disc_comp.status;
        }
        ...
        status = btc_transfer_context(&msg, &tmp_jv, sizeof(tBTA_JV), NULL);
...
static void btc_spp_init(btc_spp_args_t *arg)
{
    ...
    if (ret != XR_SPP_SUCCESS) {
        xr_spp_cb_param_t param;
        param.init.status = ret;
        btc_spp_cb_to_app(XR_SPP_INIT_EVT, &param);
    }
}
...
static void btc_spp_uninit(void)
{
        ...
        for (size_t i = 1; i <= MAX_RFC_PORTS; i++) {
            if (spp_local_param.spp_slots[i] != NULL && spp_local_param.spp_slots[i]->connected) {
                BTA_JvRfcommClose(spp_local_param.spp_slots[i]->rfc_handle, (tBTA_JV_RFCOMM_CBACK *)btc_spp_rfcomm_inter_cb,
                                  (void *)(uintptr_t)spp_local_param.spp_slots[i]->id);
            }
        }
        ...
        for (size_t i = 1; i <= MAX_RFC_PORTS; i++) {
            if (spp_local_param.spp_slots[i] != NULL && !spp_local_param.spp_slots[i]->connected) {
                ...
                if (user_data) {
                    ...
                    xr_spp_cb_param_t param;
                    BTC_TRACE_ERROR("%s unable to malloc user data!", __func__);
                    param.srv_stop.status = XR_SPP_NO_RESOURCE;
                    param.srv_stop.scn = spp_local_param.spp_slots[i]->scn;
                    btc_spp_cb_to_app(XR_SPP_SRV_STOP_EVT, &param);
                }
    ...
    if (ret != XR_SPP_SUCCESS) {
        ...
        btc_spp_cb_to_app(XR_SPP_UNINIT_EVT, &param);
    }
}
...
static void btc_spp_start_discovery(btc_spp_args_t *arg)
{
    ...
    if (ret != XR_SPP_SUCCESS) {
        ...
        memset(param.disc_comp.scn, 0xff, XR_SPP_MAX_SCN);
        btc_spp_cb_to_app(XR_SPP_DISCOVERY_COMP_EVT, &param);
    }
}
...
static void btc_spp_connect(btc_spp_args_t *arg)
{
    ...
    do {
        ...
        memcpy(slot->addr, arg->connect.peer_bd_addr, XR_BD_ADDR_LEN);
        BTA_JvRfcommConnect(arg->connect.sec_mask, arg->connect.role, arg->connect.remote_scn,
                            arg->connect.peer_bd_addr, (tBTA_JV_RFCOMM_CBACK *)btc_spp_rfcomm_inter_cb, (void *)(uintptr_t)slot->id);
        osi_mutex_unlock(&spp_local_param.spp_slot_mutex);
    } while (0);
    ...
    if (ret != XR_SPP_SUCCESS) {
        ...
        param.open.handle = 0;
        param.open.fd = -1;
        memset(param.open.rem_bda, 0, XR_BD_ADDR_LEN);
        btc_spp_cb_to_app(XR_SPP_OPEN_EVT, &param);
    }
}
...
static void btc_spp_disconnect(btc_spp_args_t *arg)
{
    ...
    do {
        ...
        if (!slot || (slot && !slot->connected)) {
            ...
            ret = XR_SPP_NO_CONNECTION;
            break;
        }
        BTA_JvRfcommClose(arg->disconnect.handle, (tBTA_JV_RFCOMM_CBACK *)btc_spp_rfcomm_inter_cb, (void *)(uintptr_t)slot->id);
        osi_mutex_unlock(&spp_local_param.spp_slot_mutex);
    } while(0);
    ...
    if (ret != XR_SPP_SUCCESS) {
        ...
        btc_spp_cb_to_app(XR_SPP_CLOSE_EVT, &param);
    }
}
...
static void btc_spp_start_srv(btc_spp_args_t *arg)
{
    ...
    do {
        ...
        BTA_JvGetChannelId(BTA_JV_CONN_TYPE_RFCOMM, (void *)(uintptr_t)slot->id, arg->start_srv.local_scn);
        osi_mutex_unlock(&spp_local_param.spp_slot_mutex);
    } while(0);
...
static void btc_spp_write(btc_spp_args_t *arg)
{
    ...
    if (ret != XR_SPP_SUCCESS && spp_local_param.spp_mode == XR_SPP_MODE_CB) {
        xr_spp_cb_param_t param;
        param.write.status = ret;
        param.write.handle = 0;
        param.write.len = -1;
        param.write.cong = false;
        btc_spp_cb_to_app(XR_SPP_WRITE_EVT, &param);
    }
}
...
void btc_spp_cb_handler(btc_msg_t *msg)
{
    ...
    case BTA_JV_RFCOMM_CL_INIT_EVT:
        ...
        btc_spp_cb_to_app(XR_SPP_CL_INIT_EVT, &param);
        break;
    case BTA_JV_RFCOMM_OPEN_EVT:
        do {
            if (spp_local_param.spp_mode == XR_SPP_MODE_VFS) {
                ...
                if (!slot) {
                    osi_mutex_unlock(&spp_local_param.spp_slot_mutex);
                    BTC_TRACE_ERROR("%s unable to find RFCOMM slot!", __func__);
                    param.open.status = XR_SPP_NO_CONNECTION;
                    break;
                }
        ...
        memcpy(param.open.rem_bda, p_data->rfc_open.rem_bda, XR_BD_ADDR_LEN);
        btc_spp_cb_to_app(XR_SPP_OPEN_EVT, &param);
        break;
    case BTA_JV_RFCOMM_START_EVT:
    ...
    case BTA_JV_RFCOMM_SRV_OPEN_EVT:
        if (p_data->rfc_srv_open.handle) {
            do {
                if (spp_local_param.spp_mode == XR_SPP_MODE_VFS) {
                    ...
                    if (!slot) {
                        osi_mutex_unlock(&spp_local_param.spp_slot_mutex);
                        BTC_TRACE_ERROR("%s unable to find RFCOMM slot!", __func__);
                        param.srv_open.status = XR_SPP_NO_CONNECTION;
                        break;
                    }
            ...
            memcpy(param.srv_open.rem_bda, p_data->rfc_srv_open.rem_bda, XR_BD_ADDR_LEN);
            btc_spp_cb_to_app(XR_SPP_SRV_OPEN_EVT, &param);
        }
        break;
    case BTA_JV_RFCOMM_WRITE_EVT:
        ...
        if (spp_local_param.spp_mode == XR_SPP_MODE_CB){
            param.write.status = slot ? p_data->rfc_write.status : XR_SPP_NO_CONNECTION;
    ...
    case BTA_JV_RFCOMM_CLOSE_EVT:
        ...
        if (spp_local_param.spp_mode == XR_SPP_MODE_CB) {
            ...
            do {
                ...
                slot = spp_find_slot_by_handle(p_data->rfc_close.handle);
                if (!slot) {
                    param.close.status = XR_SPP_NO_CONNECTION;
                    osi_mutex_unlock(&spp_local_param.spp_slot_mutex);
                    BTC_TRACE_ERROR("%s unable to find RFCOMM slot!", __func__);
                    break;
                }
                ...
                if (slot->close_alarm == NULL && slot->rx.queue && fixed_queue_length(slot->rx.queue) > 0) {
                    ...
                    if ((p_arg = malloc(sizeof(tBTA_JV))) == NULL) {
                        param.close.status = XR_SPP_NO_RESOURCE;
                        osi_mutex_unlock(&spp_local_param.spp_slot_mutex);
                    ...
                    if (osi_alarm_set(slot->close_alarm, VFS_CLOSE_TIMEOUT) != OSI_ALARM_ERR_PASS) {
                        free(p_arg);
                        osi_alarm_free(slot->close_alarm);
                        param.close.status = XR_SPP_BUSY;
                        osi_mutex_unlock(&spp_local_param.spp_slot_mutex);
    ...
    case BTA_JV_RFCOMM_CONG_EVT:
        if (spp_local_param.spp_mode == XR_SPP_MODE_CB) {
            ...
            btc_spp_cb_to_app(XR_SPP_CONG_EVT, &param);
        } else {
            osi_mutex_lock(&spp_local_param.spp_slot_mutex, OSI_MUTEX_MAX_TIMEOUT);
    ...
    case BTA_JV_DISABLE_EVT:
        param.uninit.status = XR_SPP_SUCCESS;
        BTA_JvFree();
...
static ssize_t spp_vfs_write(int fd, const void * data, size_t size)
{
    ...
    while (1) {
        ...
        if ((slot = spp_local_param.spp_slots[serial]) != NULL) {
            ...
            if (tx_len == 0) {
                xr_spp_write(slot->rfc_handle, 0, NULL);
            }
            sent += write_size;
            size -= write_size;
            p_buf = NULL;
...
static int spp_vfs_close(int fd)
{
    ...
    xr_spp_disconnect(slot->rfc_handle);
    osi_mutex_unlock(&spp_local_param.spp_slot_mutex);
    return 0;
}
...
xr_err_t btc_spp_vfs_register(void)
{
#if 0
    if (!is_spp_init()) {
        BTC_TRACE_ERROR("%s SPP have not been init\n", __func__);
    ...
    // file descriptors
    if (xr_vfs_register_with_id(&vfs, NULL, &spp_local_param.spp_vfs_id) != XR_OK) {
        return XR_FAIL;
    }
#else
    BTC_TRACE_WARNING("%s SPP can not register vfs in this version\n", __func__);
#endif
