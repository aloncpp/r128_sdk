...
#else
hf_local_param_t *hf_local_param_ptr;
...
if (! hf_local_param[idx].btc_hf_cb.initialized || \
    hf_local_param[idx].btc_hf_cb.connection_state != XR_HF_CONNECTION_STATE_SLC_CONNECTED) { \
BTIF_TRACE_WARNING("CHECK_HF_INIT: %s: HF AG not initialized", __FUNCTION__);\
return BT_STATUS_NOT_READY; \
} \
else\
...
#define clear_phone_state() \
    hf_local_param[idx].btc_hf_cb.call_state = XR_HF_CALL_STATUS_NO_CALLS; \
    hf_local_param[idx].btc_hf_cb.call_setup_state = XR_HF_CALL_SETUP_STATUS_IDLE;\
...
static BOOLEAN is_connected(bt_bdaddr_t *bd_addr)
{
    ...
    if (((hf_local_param[idx].btc_hf_cb.connection_state == XR_HF_CONNECTION_STATE_CONNECTED) ||
            (hf_local_param[idx].btc_hf_cb.connection_state == XR_HF_CONNECTION_STATE_SLC_CONNECTED)) &&
...
static int btc_hf_latest_connected_idx(void)
{
    ...
    for (int i = 0; i < btc_max_hf_clients; i++) {
        if (hf_local_param[i].btc_hf_cb.connection_state  == XR_HF_CONNECTION_STATE_SLC_CONNECTED) {
            // todo
...
**  Cb and Evt
************************************************************************************/
static inline void btc_hf_cb_to_app(xr_hf_cb_event_t event, xr_hf_cb_param_t *param)
{
    xr_hf_cb_t btc_hf_callbacks = (xr_hf_cb_t)btc_profile_cb_get(BTC_PID_HF);
    if (btc_hf_callbacks) {
...
static void btc_hf_cind_evt(tBTA_AG_IND *ind)
{
    ...
    btc_hf_cb_to_app(XR_HF_CIND_RESPONSE_EVT, &param);
}
...
************************************************************************************/
void btc_hf_reg_data_cb(xr_hf_incoming_data_cb_t recv, xr_hf_outgoing_data_cb_t send)
{
...
// set audio path
    ...
    uint8_t data_path = XR_SCO_DATA_PATH_HCI;
#else
    uint8_t data_path = XR_SCO_DATA_PATH_PCM;
#endif
    //xr_bredr_sco_datapath_set(data_path);
    return BT_STATUS_SUCCESS;
}
...
static bt_status_t connect_init(bt_bdaddr_t *bd_addr, uint16_t uuid)
{
    ...
    if (!is_connected(bd_addr)) {
        hf_local_param[idx].btc_hf_cb.connection_state  = XR_HF_CONNECTION_STATE_CONNECTING;
...
bt_status_t btc_hf_connect_audio(bt_bdaddr_t *bd_addr)
{
    ...
    if (is_connected(bd_addr) && (idx != BTC_HF_INVALID_IDX)) {
        ...
        do {
            xr_hf_cb_param_t param;
            memset(&param, 0, sizeof(xr_hf_cb_param_t));
            param.audio_stat.state = XR_HF_AUDIO_STATE_CONNECTING;
            memcpy(param.audio_stat.remote_addr, &hf_local_param[idx].btc_hf_cb.connected_bda, sizeof(xr_bd_addr_t));
            btc_hf_cb_to_app(XR_HF_AUDIO_STATE_EVT, &param);
        } while (0);
        return BT_STATUS_SUCCESS;
    }
    return BT_STATUS_FAIL;
...
************************************************************************************/
//AT+VRA
static bt_status_t btc_hf_vra(bt_bdaddr_t *bd_addr, xr_hf_vr_state_t value)
{
    int idx = btc_hf_idx_by_bdaddr(bd_addr);
    CHECK_HF_SLC_CONNECTED();
    if ((idx < 0) || (idx >= BTC_HF_NUM_CB)) {
...
static bt_status_t btc_hf_volume_control(bt_bdaddr_t *bd_addr, xr_hf_volume_type_t type, int volume)
{
    int idx = btc_hf_idx_by_bdaddr(bd_addr);
    CHECK_HF_SLC_CONNECTED();
    if ((idx < 0) || (idx >= BTC_HF_NUM_CB)) {
    ...
    if (is_connected(bd_addr) && (idx != BTC_HF_INVALID_IDX)) {
        ag_res.num = volume;
        BTA_AgResult(hf_local_param[idx].btc_hf_cb.handle, (type == XR_HF_VOLUME_TYPE_SPK) ? BTA_AG_SPK_RES : BTA_AG_MIC_RES, &ag_res);
        return BT_STATUS_SUCCESS;
    }
    return BT_STATUS_FAIL;
}
...
// +CMEE:<response>
static bt_status_t btc_hf_cmee_response(bt_bdaddr_t *bd_addr, xr_hf_at_response_code_t response_code, xr_hf_cme_err_t error_code)
{
    int idx = btc_hf_idx_by_bdaddr(bd_addr);
    CHECK_HF_INIT();
    if ((idx < 0) || (idx >= BTC_HF_NUM_CB)) {
    ...
    if (is_connected(bd_addr) && (idx != BTC_HF_INVALID_IDX)) {
        tBTA_AG_RES_DATA    ag_res;
        memset(&ag_res, 0, sizeof(ag_res));
        if (response_code == XR_HF_AT_RESPONSE_CODE_OK) {
...
static bt_status_t btc_hf_cind_response(bt_bdaddr_t *bd_addr,
                                        xr_hf_call_setup_status_t call_status, 
                                        xr_hf_call_setup_status_t call_setup_status,
                                        xr_hf_network_state_t ntk_state, int signal, xr_hf_service_type_t roam, int batt_lev,
...
//AT+CLCC response
static bt_status_t btc_hf_clcc_response(bt_bdaddr_t *bd_addr, int index, xr_hf_current_call_direction_t dir,
                                        xr_hf_current_call_status_t current_call_state,
                                        xr_hf_current_call_mode_t mode, xr_hf_current_call_mpty_type_t mpty, 
                                        const char *number, xr_hf_call_addr_type_t type)
{
    int idx = btc_hf_idx_by_bdaddr(bd_addr);
    CHECK_HF_SLC_CONNECTED();
    if ((idx < 0) || (idx >= BTC_HF_NUM_CB)) {
    ...
    if (is_connected(bd_addr) && (idx != BTC_HF_INVALID_IDX)) {
        ...
        if (index == 0) {
            ...
            if (number) {
               if ((type == XR_HF_CALL_ADDR_TYPE_INTERNATIONAL) && (*number != '+')) {
...
//AT+CNUM
static bt_status_t btc_hf_cnum_response(bt_bdaddr_t *bd_addr, const char *number, xr_hf_subscriber_service_type_t type)
{
    int idx = btc_hf_idx_by_bdaddr(bd_addr);
    CHECK_HF_SLC_CONNECTED();
...
//+BSIR
static bt_status_t btc_hf_inband_ring(bt_bdaddr_t *bd_addr, xr_hf_in_band_ring_state_t state)
{
    int idx = btc_hf_idx_by_bdaddr(bd_addr);
    CHECK_HF_SLC_CONNECTED();
...
static bt_status_t btc_hf_phone_state_update(bt_bdaddr_t *bd_addr,int num_active, int num_held,
                                            ...
                                            char *number, xr_hf_call_addr_type_t type)
{
    ...
    if (num_active == 0 && num_held == 0 && call_state == XR_HF_CALL_STATUS_NO_CALLS && call_setup_state == XR_HF_CALL_SETUP_STATUS_IDLE) {
        BTC_TRACE_DEBUG("%s: Phone on hook", __FUNCTION__);

        /* Record call termination timestamp if there was an active/held call or call_setup_state > XR_HF_CALL_SETUP_STATUS_IDLE */
        if ((hf_local_param[idx].btc_hf_cb.call_state != XR_HF_CALL_STATUS_NO_CALLS) ||
            (hf_local_param[idx].btc_hf_cb.call_setup_state != XR_HF_CALL_SETUP_STATUS_IDLE) ||
    ...
    */
    ...
    if (((num_active + num_held) > 0) && 
        ...
        if (call_setup_state != XR_HF_CALL_SETUP_STATUS_IDLE) {
    ...
    if (call_setup_state != hf_local_param[idx].btc_hf_cb.call_setup_state) {
        ...
        {
            case XR_HF_CALL_SETUP_STATUS_IDLE:
            {
                switch(hf_local_param[idx].btc_hf_cb.call_setup_state)
                {
                    case XR_HF_CALL_SETUP_STATUS_INCOMING:
                    {
                    ...
                    case XR_HF_CALL_SETUP_STATUS_OUTGOING_DIALING:
                    case XR_HF_CALL_SETUP_STATUS_OUTGOING_ALERTING:
                    {
                        if (num_active > hf_local_param[idx].btc_hf_cb.num_active) {
                            res = BTA_AG_OUT_CALL_CONN_RES;
            ...
            case XR_HF_CALL_SETUP_STATUS_OUTGOING_ALERTING:
            {
                if ((hf_local_param[idx].btc_hf_cb.call_setup_state == XR_HF_CALL_SETUP_STATUS_IDLE) &&
                    !(num_active + num_held)) {
                    ag_res.audio_handle = hf_local_param[idx].btc_hf_cb.handle;
        ...
        if (call_setup_state == XR_HF_CALL_SETUP_STATUS_IDLE) {
            ...
            if ((num_held > 0) && (num_active > 0))
                send_indicator_update(BTA_AG_IND_CALLHELD, XR_HF_CALL_HELD_STATUS_HELD_AND_ACTIVE);
            goto update_call_states;
        }
    }
    ...
    switch (event) {
        ...
        case BTA_AG_OPEN_EVT:
        {
            if (p_data->open.status == BTA_AG_SUCCESS) 
            {
                ...
                hf_local_param[idx].btc_hf_cb.connection_state  = XR_HF_CONNECTION_STATE_CONNECTED;
                hf_local_param[idx].btc_hf_cb.peer_feat = 0;
                hf_local_param[idx].btc_hf_cb.chld_feat = 0;
                //clear_phone_state();
            } else if (hf_local_param[idx].btc_hf_cb.connection_state  == XR_HF_CONNECTION_STATE_CONNECTING) {
                hf_local_param[idx].btc_hf_cb.connection_state  = XR_HF_CONNECTION_STATE_DISCONNECTED;
            } else {
            ...
            do {
                memset(&param, 0, sizeof(xr_hf_cb_param_t));
                memcpy(param.conn_stat.remote_bda, &hf_local_param[idx].btc_hf_cb.connected_bda, sizeof(xr_bd_addr_t));
                ...
                btc_hf_cb_to_app(XR_HF_CONNECTION_STATE_EVT, &param);
            } while (0);

            if (hf_local_param[idx].btc_hf_cb.connection_state  == XR_HF_CONNECTION_STATE_DISCONNECTED)
        ...
        case BTA_AG_CLOSE_EVT:
        {
            ...
            do {
                memset(&param, 0, sizeof(xr_hf_cb_param_t));
                param.conn_stat.state = XR_HF_CONNECTION_STATE_DISCONNECTED;
                param.conn_stat.peer_feat = 0;
                param.conn_stat.chld_feat = 0;
                memcpy(param.conn_stat.remote_bda, &hf_local_param[idx].btc_hf_cb.connected_bda, sizeof(xr_bd_addr_t));
                btc_hf_cb_to_app(XR_HF_CONNECTION_STATE_EVT, &param);
        ...
        case BTA_AG_AUDIO_OPEN_EVT:
        {
            do {
                memset(&param, 0, sizeof(xr_hf_cb_param_t));
                param.audio_stat.state = XR_HF_AUDIO_STATE_CONNECTED;
                memcpy(param.audio_stat.remote_addr, &hf_local_param[idx].btc_hf_cb.connected_bda,sizeof(xr_bd_addr_t));
                btc_hf_cb_to_app(XR_HF_AUDIO_STATE_EVT, &param);
            } while(0);
            break;
        ...
        case BTA_AG_AUDIO_CLOSE_EVT:
        {
            do {
                memset(&param, 0, sizeof(xr_hf_cb_param_t));
                param.audio_stat.state = XR_HF_AUDIO_STATE_DISCONNECTED;
                memcpy(param.audio_stat.remote_addr, &hf_local_param[idx].btc_hf_cb.connected_bda, sizeof(xr_bd_addr_t));
                btc_hf_cb_to_app(XR_HF_AUDIO_STATE_EVT, &param);
            } while(0);
            break;
        ...
        case BTA_AG_AT_BVRA_EVT:
        {
            do {
                ...
                btc_hf_cb_to_app(XR_HF_BVRA_RESPONSE_EVT, &param);
                if (p_data->val.num) {
        ...
        case BTA_AG_MIC_EVT:
        {
            do {
                memset(&param, 0, sizeof(xr_hf_cb_param_t));
                param.volume_control.type = (event == BTA_AG_SPK_EVT) ? XR_HF_VOLUME_CONTROL_TARGET_SPK : XR_HF_VOLUME_CONTROL_TARGET_MIC;
                param.volume_control.volume = p_data->val.num;
                btc_hf_cb_to_app(XR_HF_VOLUME_CONTROL_EVT, &param);
            } while (0);
            break;
        }
        ...
        case BTA_AG_AT_UNAT_EVT:
        {
            ...
            btc_hf_cb_to_app(XR_HF_UNAT_RESPONSE_EVT, &param);
            break;
        }

        case BTA_AG_AT_CBC_EVT:
        {
            btc_hf_cb_to_app(XR_HF_IND_UPDATE_EVT, NULL);
            break;
        }
        ...
        {
            btc_hf_cb_to_app(XR_HF_COPS_RESPONSE_EVT, NULL);
            break;
        }

        case BTA_AG_AT_CLCC_EVT:
        {
            btc_hf_cb_to_app(XR_HF_CLCC_RESPONSE_EVT, NULL);
            break;
        }

        case BTA_AG_AT_CNUM_EVT:
        {
            btc_hf_cb_to_app(XR_HF_CNUM_RESPONSE_EVT, NULL);
            break;
        }
        ...
        case BTA_AG_AT_VTS_EVT:
        {
            do {
                ...
                btc_hf_cb_to_app(XR_HF_VTS_RESPONSE_EVT, &param);
            } while(0);
            break;
        }
        ...
        case BTA_AG_AT_NREC_EVT:
        {
            do {
                ...
                btc_hf_cb_to_app(XR_HF_NREC_RESPONSE_EVT, &param);
            } while(0);
            break;
        }
        ...
        {
            btc_hf_cb_to_app(XR_HF_ATA_RESPONSE_EVT, NULL);
            break;
        }

        case BTA_AG_AT_CHUP_EVT:
        {
            btc_hf_cb_to_app(XR_HF_CHUP_RESPONSE_EVT, NULL);
            break;
        }
        ...
        case BTA_AG_AT_D_EVT:
        {
            do {
                if (event == BTA_AG_AT_D_EVT && p_data->val.str) {           // dial_number_or_memory
                    ...
                    btc_hf_cb_to_app(XR_HF_DIAL_EVT, &param);
        ...
        case BTA_AG_WBS_EVT:
        {
            ...
            btc_hf_cb_to_app(XR_HF_WBS_RESPONSE_EVT, &param);
            break;
        }
        ...
        case BTA_AG_AT_BCS_EVT:
        {
            ...
            /* No XR_HF_WBS_NONE case, becuase HFP 1.6 supported device can send BCS */
            btc_hf_cb_to_app(XR_HF_BCS_RESPONSE_EVT, &param);
            break;
        }
#endif
        default:
