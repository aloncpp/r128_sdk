...
const char* dump_hf_conn_state(UINT16 event)
{
    switch(event)
    {
        CASE_RETURN_STR(XR_HF_CONNECTION_STATE_DISCONNECTED)
        CASE_RETURN_STR(XR_HF_CONNECTION_STATE_CONNECTING)
        CASE_RETURN_STR(XR_HF_CONNECTION_STATE_CONNECTED)
        CASE_RETURN_STR(XR_HF_CONNECTION_STATE_SLC_CONNECTED)
        CASE_RETURN_STR(XR_HF_CONNECTION_STATE_DISCONNECTING)
        default:
...
const char* dump_hf_call_state(xr_hf_call_status_t  call_state)
{
    switch(call_state)
    {
        CASE_RETURN_STR(XR_HF_CALL_STATUS_NO_CALLS)
        CASE_RETURN_STR(XR_HF_CALL_STATUS_CALL_IN_PROGRESS)
        default:
            return "UNKNOWN CALL STATE";
    }
}

const char* dump_hf_call_setup_state(xr_hf_call_setup_status_t call_setup_state)
{
    switch(call_setup_state)
    {
        CASE_RETURN_STR(XR_HF_CALL_SETUP_STATUS_IDLE)
        CASE_RETURN_STR(XR_HF_CALL_SETUP_STATUS_INCOMING)
        CASE_RETURN_STR(XR_HF_CALL_SETUP_STATUS_OUTGOING_DIALING)
        CASE_RETURN_STR(XR_HF_CALL_SETUP_STATUS_OUTGOING_ALERTING)
        default:
...
void uuid128_be_to_xr_uuid(xr_bt_uuid_t *u, uint8_t* uuid128)
{
    if (memcmp(base_uuid_be+4, uuid128 + 4, 12) != 0) {
        u->len = XR_UUID_LEN_128;
        uint8_t *p_i = uuid128 + XR_UUID_LEN_128 - 1;
        uint8_t *p_o = u->uuid.uuid128;
        uint8_t *p_end = p_o + XR_UUID_LEN_128;
...
xr_bt_status_t btc_hci_to_xr_status(uint8_t hci_status)
{
    xr_bt_status_t xr_status = XR_BT_STATUS_FAIL;
    switch(hci_status) {
        case HCI_SUCCESS:
            xr_status = XR_BT_STATUS_SUCCESS;
            break;
        case HCI_ERR_HOST_TIMEOUT:
            xr_status = XR_BT_STATUS_TIMEOUT;
            break;
        case HCI_ERR_ILLEGAL_COMMAND:
            xr_status = XR_BT_STATUS_PENDING;
            break;
        case HCI_ERR_UNACCEPT_CONN_INTERVAL:
            xr_status = XR_BT_STATUS_UNACCEPT_CONN_INTERVAL;
            break;
        case HCI_ERR_PARAM_OUT_OF_RANGE:
            xr_status = XR_BT_STATUS_PARAM_OUT_OF_RANGE;
            break;
        case HCI_ERR_ILLEGAL_PARAMETER_FMT:
            xr_status = XR_BT_STATUS_ERR_ILLEGAL_PARAMETER_FMT;
            break;
        default:
            xr_status = XR_BT_STATUS_FAIL;
            break;
    }
...
xr_bt_status_t btc_btm_status_to_xr_status (uint8_t btm_status)
{
    xr_bt_status_t xr_status = XR_BT_STATUS_FAIL;
    switch(btm_status){
        case BTM_SUCCESS:
            ...
            break;
        case BTM_BUSY:
            xr_status = XR_BT_STATUS_BUSY;
            break;
        case BTM_NO_RESOURCES:
            xr_status = XR_BT_STATUS_NOMEM;
            break;
        case BTM_ILLEGAL_VALUE:
            xr_status = XR_BT_STATUS_PARM_INVALID;
            break;
        case BTM_ERR_PROCESSING:
            xr_status = XR_BT_STATUS_PENDING;
            break;
        case BTM_PEER_LE_DATA_LEN_UNSUPPORTED:
            xr_status = XR_BT_STATUS_PEER_LE_DATA_LEN_UNSUPPORTED;
            break;
        case BTM_CONTROL_LE_DATA_LEN_UNSUPPORTED:
            xr_status = XR_BT_STATUS_CONTROL_LE_DATA_LEN_UNSUPPORTED;
            break;
...
xr_bt_status_t btc_bta_status_to_xr_status (uint8_t bta_status)
{
    xr_bt_status_t xr_status = XR_BT_STATUS_FAIL;
    switch(bta_status){
        case BTA_SUCCESS:
        ...
        case BTA_NO_RESOURCES:
            ...
            break;
        case BTA_WRONG_MODE:
            xr_status = XR_BT_STATUS_NOT_READY;
            break;
        case BTA_EIR_TOO_LARGE:
            xr_status = XR_BT_STATUS_EIR_TOO_LARGE;
            break;
        default:
            xr_status = XR_BT_STATUS_FAIL;
            break;
