...
enum {
    ...
    BTA_DM_API_SET_IPSCAN_PARAM_EVT,
...
#endif
    BTA_DM_API_AUTO_SNIFF_MODE_EVT,//xradio define
    BTA_DM_MAX_EVT
};
...
/* data type for BTA_DM_API_GET_NAME_EVT */
typedef struct {
    BT_HDR              hdr;
...
/* data type for BTA_DM_API_SET_IPSCAN_PARAM_EVT */
typedef struct {
    BT_HDR         hdr;
    UINT8          type;
    tBTA_DM_WIN    window;
    tBTA_DM_INT    interval;
} tBTA_DM_API_SET_IPSCAN_PARAM;
#define INQUIRY_TYPE 0
#define PAGE_TYPE 1
...
typedef struct {
    BT_HDR      hdr;
    UINT8     mode;/*If TRUE host will auto enter sniff mode, otherwise will not.*/
} tBTA_DM_API_AUTO_SNIFF_MODE;
...
typedef union {
    ...
    tBTA_DM_API_SET_IPSCAN_PARAM set_ipscan_param;
    ...
    tBTA_DM_API_AUTO_SNIFF_MODE         auto_sniff_mode;
...
extern void bta_dm_set_auto_enter_sniff_mode(tBTA_DM_MSG *p_data);
extern void bta_dm_set_ipscan_param (tBTA_DM_MSG *p_data);
