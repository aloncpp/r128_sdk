...
void bta_dm_get_dev_name (tBTA_DM_MSG *p_data)
{
    if (bta_dm_cb.p_sec_cback) {
        ...
        BTM_ReadLocalDeviceName((char **)&sec_event.read_local_name.name);
        bta_dm_cb.p_sec_cback(BTA_DM_READ_LOCAL_NAME_EVT, &sec_event);
    }
}
...
/*******************************************************************************
**
** Function         bta_dm_set_ipscan_param
**
** Description      set inquiry scan or page scan param.
****
*******************************************************************************/
void bta_dm_set_ipscan_param(tBTA_DM_MSG *pdata)
{
    if(pdata->set_ipscan_param.type == INQUIRY_TYPE) {
        bta_dm_cb.inquiry_scan_window = pdata->set_ipscan_param.window;
        bta_dm_cb.inquiry_scan_interval = pdata->set_ipscan_param.interval;
    } else if (pdata->set_ipscan_param.type == PAGE_TYPE) {
        bta_dm_cb.page_scan_window = pdata->set_ipscan_param.window;
        bta_dm_cb.page_scan_interval = pdata->set_ipscan_param.interval;
    } else {
        APPL_TRACE_ERROR("Error type %d\n", pdata->set_ipscan_param.type);
    }
}
