...
#if (GATTC_CACHE_NVS == TRUE) //zipmodi
...
static void cacheClose(BD_ADDR bda)
{
    ...
    if ((index = bta_gattc_co_find_addr_in_cache(bda)) != INVALID_ADDR_NUM) {
        if (cache_env->cache_addr[index].is_open) {
            fclose(cache_env->cache_addr[index].cache_fp);/*xr*/
...
static bool cacheOpen(BD_ADDR bda, bool to_save, UINT8 *index)
{
    ...
    if (((*index = bta_gattc_co_find_addr_in_cache(bda)) != INVALID_ADDR_NUM) ||
        ...
        if (cache_env->cache_addr[*index].is_open) {
            ...
            cache_env->cache_addr[*index].cache_fp = fopen(fname, "a+");//rewrite is ok?
            if (NULL == cache_env->cache_addr[*index].cache_fp) {
    ...
    return ((status == XR_OK) ? true : false);
}
    ...
    if ((index = bta_gattc_co_find_addr_in_cache(bda)) != INVALID_ADDR_NUM) {
        ...
        if(cache_env->num_addr > 0) {
            ...
            if (cache_env->is_open) {
                if (fwrite(p_buf, sizeof(UINT8), length, cache_env->addr_fp) != sizeof(UINT8) * length) {
...
tBTA_GATT_STATUS bta_gattc_co_cache_load(tBTA_GATTC_NV_ATTR *attr, UINT8 index)
{
    ...
    fseek (cache_env->cache_addr[index].cache_fp, 0L, SEEK_END);
    length = ftell (cache_env->cache_addr[index].cache_fp);
    fseek (cache_env->cache_addr[index].cache_fp, 0L, SEEK_SET);
    fread (attr, sizeof(UINT8), length, cache_env->addr_fp);//attr maybe wrong, read buf first and assign it to attr
    ...
    APPL_TRACE_DEBUG("%s() - read=%d, status=%d",
                     __func__, num_attr, status);/*xr*/
                              ...
                              tBTA_GATTC_NV_ATTR *p_attr_list)
{
    ...
    if (cacheOpen(server_bda, TRUE, &index)) {
        /*
        xr_err_t err_code = nvs_set_blob(cache_env->cache_addr[index].cache_fp, cache_key,
                                          p_attr_list, sizeof(tBTA_GATTC_NV_ATTR)*num_attr);
        status = (err_code == XR_OK) ? BTA_GATT_OK : BTA_GATT_ERROR;
        */
        if (fwrite(p_attr_list, sizeof(tBTA_GATTC_NV_ATTR), num_attr, cache_env->cache_addr[index].cache_fp) != sizeof(tBTA_GATTC_NV_ATTR)*num_attr) {
            status = BTA_GATT_ERROR;
        }
        else {
            status = BTA_GATT_OK;
...
void bta_gattc_co_cache_addr_init(void)
{
    ...
    fp = fopen(cache_addr, "a+");
    if (NULL != fp) {
        cache_env->addr_fp = fp;
        cache_env->is_open = TRUE;
        // Read previously saved blob if available
        if(fread (p_buf, sizeof(UINT8), length, fp)) {
...
void bta_gattc_co_cache_addr_deinit(void)
{
    ...
    fclose(cache_env->addr_fp); /*xr*/
    cache_env->is_open = false;

    for(UINT8 i = 0; i< cache_env->num_addr; i++) {
        cache_addr_info_t *addr_info = &cache_env->cache_addr[i];
        if(addr_info) {
            fclose(addr_info->cache_fp); /*xr*/
...
void bta_gattc_co_cache_addr_save(BD_ADDR bd_addr, hash_key_t hash_key)
{
    ...
    if (cache_env->is_open) {
        if (fwrite(p_buf, sizeof(UINT8), length, cache_env->addr_fp) != length) {
