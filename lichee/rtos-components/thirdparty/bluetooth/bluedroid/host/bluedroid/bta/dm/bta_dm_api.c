...
void BTA_DmSetAutoSniffMode(uint8_t mode)
{
    tBTA_DM_API_AUTO_SNIFF_MODE *p_msg;
    if ((p_msg = (tBTA_DM_API_AUTO_SNIFF_MODE *)osi_malloc(sizeof(tBTA_DM_API_AUTO_SNIFF_MODE))) != NULL) {
        p_msg->hdr.event = BTA_DM_API_AUTO_SNIFF_MODE_EVT;
        p_msg->mode = mode;
        bta_sys_sendmsg(p_msg);
    }
}
...
void BTA_DmSetIpscanParam(tBTA_DM_WIN window, tBTA_DM_INT interval, UINT8 type)
{
    ...
    tBTA_DM_API_SET_IPSCAN_PARAM    *p_msg;

    if ((p_msg = (tBTA_DM_API_SET_IPSCAN_PARAM *) osi_malloc(sizeof(tBTA_DM_API_SET_IPSCAN_PARAM))) != NULL) {
        p_msg->hdr.event = BTA_DM_API_SET_IPSCAN_PARAM_EVT;
        p_msg->type = type;
        p_msg->window = window;
