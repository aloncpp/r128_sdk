...
static void __attribute__((unused)) fc_init(void)
{
    ...
    APPL_TRACE_WARNING("fc_init can not pthread_once in this version");
}
        ...
        if ((p_cb->max_sess > 1) &&
            ...
            if (used < p_cb->max_sess &&
                ...
                if ((RFCOMM_CreateConnection(p_cb->sec_id, p_cb->scn, TRUE,
                    ...
                    p_pcb->user_data = p_cb->p_cback(BTA_JV_RFCOMM_SRV_OPEN_EVT, &evt_data, (void *)(uintptr_t)scn_num);
                }
            }
        }
...
void bta_jv_free_scn(tBTA_JV_MSG *p_data)
{
    ...
    if (fc->user_data)
    {
        ...
        if (user_data->server_status == BTA_JV_SERVER_RUNNING && find_rfc_pcb((void *)(uintptr_t)user_data->slot_id, &p_cb, &p_pcb)) {
...
static void bta_jv_port_mgmt_cl_cback(UINT32 code, UINT16 port_handle)
{
    ...
    if (code == PORT_SUCCESS) {
        ...
        // open it to free server record, after RFCOMM_CreateConnection fail.
...
static int find_rfc_pcb(void *user_data, tBTA_JV_RFC_CB **cb, tBTA_JV_PCB **pcb)
{
    ...
    *pcb = NULL;
    ...
    APPL_TRACE_DEBUG("find_rfc_pcb: cannot find rfc_cb from user data:%d", (UINT32)(uintptr_t)user_data);
    return 0;
}
...
static void bta_jv_pm_state_change(tBTA_JV_PM_CB *p_cb, const tBTA_JV_CONN_STATE state)
{
    APPL_TRACE_API("bta_jv_pm_state_change(p_cb: 0x%x, handle: 0x%x, busy/idle_state: %d"
                   ", app_id: %d, conn_state: %d)", (uint32_t)(uintptr_t)p_cb, p_cb->handle, p_cb->state,
                   p_cb->app_id, state);
