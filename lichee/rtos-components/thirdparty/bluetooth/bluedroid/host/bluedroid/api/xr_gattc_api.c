...
xr_err_t xr_ble_gattc_app_register(uint16_t app_id)
{
    ...
    return (btc_transfer_context(&msg, &arg, sizeof(btc_ble_gattc_args_t), NULL) == BT_STATUS_SUCCESS ? XR_OK : XR_FAIL);
}
...
xr_err_t xr_ble_gattc_open(xr_gatt_if_t gattc_if, xr_bd_addr_t remote_bda, xr_ble_addr_type_t remote_addr_type, bool is_direct)
{
    ...
    memcpy(arg.open.remote_bda, remote_bda, XR_BD_ADDR_LEN);
...
xr_err_t xr_ble_gattc_search_service(xr_gatt_if_t gattc_if, uint16_t conn_id, xr_bt_uuid_t *filter_uuid)
{
    ...
    if (filter_uuid) {
        ...
        memcpy(&arg.search_srvc.filter_uuid, filter_uuid, sizeof(xr_bt_uuid_t));
    } else {
                                               ...
                                               uint16_t *count)
{
    ...
    if ((start_handle == 0 && end_handle == 0) && (type != XR_GATT_DB_DESCRIPTOR)) {
        *count = 0;
        return XR_GATT_INVALID_HANDLE;
    }
                                      ...
                                      xr_gatt_auth_req_t auth_req)
{
    ...
    if (uuid == NULL) {
        return XR_GATT_ILLEGAL_PARAMETER;
    }
    ...
    memcpy(&(arg.read_by_type.uuid), uuid, sizeof(xr_bt_uuid_t));
                                   ...
                                   xr_gatt_auth_req_t auth_req)
{
    ...
    arg.write_char.value_len = value_len > XR_GATT_MAX_ATTR_LEN ? XR_GATT_MAX_ATTR_LEN : value_len;
    arg.write_char.value = value;
    arg.write_char.write_type = write_type;
    arg.write_char.auth_req = auth_req;
    if(write_type == XR_GATT_WRITE_TYPE_NO_RSP){
        l2ble_update_att_acl_pkt_num(L2CA_ADD_BTC_NUM, NULL);
    }
    return (btc_transfer_context(&msg, &arg, sizeof(btc_ble_gattc_args_t), btc_gattc_arg_deep_copy) == BT_STATUS_SUCCESS ? XR_OK : XR_FAIL);
}
                                          ...
                                          xr_gatt_auth_req_t auth_req)
{
    ...
    arg.write_descr.value_len = value_len > XR_GATT_MAX_ATTR_LEN ? XR_GATT_MAX_ATTR_LEN : value_len;
                                      ...
                                      xr_gatt_auth_req_t auth_req)
{
    ...
    arg.prep_write.value_len = value_len > XR_GATT_MAX_ATTR_LEN ? XR_GATT_MAX_ATTR_LEN : value_len; // length check ?
                                                 ...
                                                 xr_gatt_auth_req_t auth_req)
{
    ...
    arg.prep_write_descr.value_len = value_len > XR_GATT_MAX_ATTR_LEN ? XR_GATT_MAX_ATTR_LEN : value_len; // length check ?
...
xr_err_t xr_ble_gattc_register_for_notify (xr_gatt_if_t gattc_if,
                                             xr_bd_addr_t server_bda, uint16_t handle)
{
    btc_msg_t msg;
    btc_ble_gattc_args_t arg;
    ...
    memcpy(arg.reg_for_notify.remote_bda, server_bda, sizeof(xr_bd_addr_t));
                                               ...
                                               xr_bd_addr_t server_bda, uint16_t handle)
{
    ...
    memcpy(arg.unreg_for_notify.remote_bda, server_bda, sizeof(xr_bd_addr_t));
    return (btc_transfer_context(&msg, &arg, sizeof(btc_ble_gattc_args_t), NULL) == BT_STATUS_SUCCESS ? XR_OK : XR_FAIL);
}
...
xr_err_t xr_ble_gattc_cache_refresh(xr_bd_addr_t remote_bda)
{
    ...
    memcpy(arg.cache_refresh.remote_bda, remote_bda, sizeof(xr_bd_addr_t));
...
xr_err_t xr_ble_gattc_cache_clean(xr_bd_addr_t remote_bda)
{
    ...
    memcpy(arg.cache_clean.remote_bda, remote_bda, sizeof(xr_bd_addr_t));

    return (btc_transfer_context(&msg, &arg, sizeof(btc_ble_gattc_args_t), NULL) == BT_STATUS_SUCCESS ? XR_OK : XR_FAIL);
}

xr_err_t xr_ble_gattc_cache_assoc(xr_gatt_if_t gattc_if, xr_bd_addr_t src_addr, xr_bd_addr_t assoc_addr, bool is_assoc)
{
    btc_msg_t msg;
    btc_ble_gattc_args_t arg;
    ...
    memcpy(arg.cache_assoc.src_addr, src_addr, sizeof(xr_bd_addr_t));
    memcpy(arg.cache_assoc.assoc_addr, assoc_addr, sizeof(xr_bd_addr_t));
