...
static xr_err_t xr_ble_gatts_add_char_desc_param_check(xr_attr_value_t *char_val, xr_attr_control_t *control);
...
xr_err_t xr_ble_gatts_register_callback(xr_gatts_cb_t callback)
{
    ...
    return (btc_profile_cb_set(BTC_PID_GATTS, callback) == 0 ? XR_OK : XR_FAIL);
}
...
xr_err_t xr_ble_gatts_app_register(uint16_t app_id)
{
    ...
    //if (app_id < XR_APP_ID_MIN || app_id > XR_APP_ID_MAX) {
    ...
    return (btc_transfer_context(&msg, &arg, sizeof(btc_ble_gatts_args_t), NULL) == BT_STATUS_SUCCESS ? XR_OK : XR_FAIL);
}
                                       ...
                                       xr_gatt_srvc_id_t *service_id, uint16_t num_handle)
{
    ...
    memcpy(&arg.create_srvc.service_id, service_id, sizeof(xr_gatt_srvc_id_t));
                                        ...
                                        uint8_t srvc_inst_id)
{
    ...
    arg.create_attr_tab.gatts_attr_db = (xr_gatts_attr_db_t *)gatts_attr_db;
                                 ...
                                 xr_attr_control_t *control)
{
    ...
    status = xr_ble_gatts_add_char_desc_param_check(char_val, control);
    if (status != XR_OK){
        return status;
    }
    ...
    memcpy(&arg.add_char.char_uuid, char_uuid, sizeof(xr_bt_uuid_t));

    return (btc_transfer_context(&msg, &arg, sizeof(btc_ble_gatts_args_t), btc_gatts_arg_deep_copy) == BT_STATUS_SUCCESS ? XR_OK : XR_FAIL);
}
                                        ...
                                        xr_attr_control_t *control)
{
    ...
    /* parameter validation check */
    status = xr_ble_gatts_add_char_desc_param_check(char_descr_val, control);
    if (status != XR_OK){
        return status;
    }
    ...
    memcpy(&arg.add_descr.descr_uuid, descr_uuid, sizeof(xr_bt_uuid_t));
                                      ...
                                      uint16_t value_len, uint8_t *value, bool need_confirm)
{
    ...
    return (btc_transfer_context(&msg, &arg, sizeof(btc_ble_gatts_args_t),
                                 btc_gatts_arg_deep_copy) == BT_STATUS_SUCCESS ? XR_OK : XR_FAIL);
}
...
xr_gatt_status_t xr_ble_gatts_get_attr_value(uint16_t attr_handle, uint16_t *length, const uint8_t **value)
{
    ...
    if (attr_handle == XR_GATT_ILLEGAL_HANDLE) {
        *length = 0;
...
xr_err_t xr_ble_gatts_open(xr_gatt_if_t gatts_if, xr_bd_addr_t remote_bda, bool is_direct)
{
    ...
    memcpy(&arg.open.remote_bda, remote_bda, sizeof(xr_bd_addr_t));
...
xr_err_t xr_ble_gatts_send_service_change_indication(xr_gatt_if_t gatts_if, xr_bd_addr_t remote_bda)
{
    ...
    if(remote_bda) {
        memcpy(&arg.send_service_change.remote_bda, remote_bda, sizeof(xr_bd_addr_t));   
    } else {
        memset(arg.send_service_change.remote_bda, 0, sizeof(xr_bd_addr_t));
    }
...
static xr_err_t xr_ble_gatts_add_char_desc_param_check(xr_attr_value_t *char_val, xr_attr_control_t *control)
{
    if ((control != NULL) && ((control->auto_rsp != XR_GATT_AUTO_RSP) && (control->auto_rsp != XR_GATT_RSP_BY_APP))){
            LOG_ERROR("Error in %s, line=%d, control->auto_rsp should be set to XR_GATT_AUTO_RSP or XR_GATT_RSP_BY_APP\n",\
                            __func__, __LINE__);
            return XR_ERR_INVALID_ARG;
    }

    if ((control != NULL) && (control->auto_rsp == XR_GATT_AUTO_RSP)){
        if (char_val == NULL){
