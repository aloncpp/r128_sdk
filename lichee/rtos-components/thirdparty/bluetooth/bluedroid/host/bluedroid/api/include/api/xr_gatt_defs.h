...
#ifndef __XR_GATT_DEFS_H__
#define __XR_GATT_DEFS_H__
...
/// GATT INVALID UUID
#define XR_GATT_ILLEGAL_UUID               0
/// GATT INVALID HANDLE
#define XR_GATT_ILLEGAL_HANDLE             0
/// GATT attribute max handle
#define XR_GATT_ATTR_HANDLE_MAX            100
#define XR_GATT_MAX_READ_MULTI_HANDLES     10           /* Max attributes to read in one request */


/**@{
 * All "XR_GATT_UUID_xxx" is attribute types
 */
...
#define XR_GATT_UUID_Automation_IO_SVC             0x1815          /* Automation IO Service*/
...
#define XR_GATT_UUID_ENVIRONMENTAL_SENSING_SVC     0x181A          /* Environmental Sensing Service*/
#define XR_GATT_UUID_BODY_COMPOSITION              0x181B          /* Body Composition Service*/
...
#define XR_GATT_UUID_BOND_MANAGEMENT_SVC           0x181E          /* Bond Management Service*/
#define XR_GATT_UUID_CONT_GLUCOSE_MONITOR_SVC      0x181F          /* Continuous Glucose Monitoring Service*/
...
#define XR_GATT_UUID_EXT_RPT_REF_DESCR             0x2907          /*  External Report Reference */
#define XR_GATT_UUID_RPT_REF_DESCR                 0x2908          /*  Report Reference */
#define XR_GATT_UUID_NUM_DIGITALS_DESCR            0x2909          /*  Number of Digitals */
#define XR_GATT_UUID_VALUE_TRIGGER_DESCR           0x290A          /*  Value Trigger Setting */
#define XR_GATT_UUID_ENV_SENSING_CONFIG_DESCR      0x290B          /*  Environmental Sensing Configuration */
#define XR_GATT_UUID_ENV_SENSING_MEASUREMENT_DESCR 0x290C          /*  Environmental Sensing Measurement */
#define XR_GATT_UUID_ENV_SENSING_TRIGGER_DESCR     0x290D          /*  Environmental Sensing Trigger Setting */
#define XR_GATT_UUID_TIME_TRIGGER_DESCR            0x290E          /*  Time Trigger Setting */
...
typedef enum {
    XR_GATT_PREP_WRITE_CANCEL    = 0x00,       /*!< Prepare write cancel */  /* relate to BTA_GATT_PREP_WRITE_CANCEL in bta/bta_gatt_api.h */
    XR_GATT_PREP_WRITE_EXEC      = 0x01,       /*!< Prepare write execute */ /* relate to BTA_GATT_PREP_WRITE_EXEC in bta/bta_gatt_api.h */
} xr_gatt_prep_write_type;
...
} xr_gatt_conn_reason_t;

/**
 * @brief Gatt id, include uuid and instance id
 */
typedef struct {
    xr_bt_uuid_t   uuid;                   /*!< UUID */
    uint8_t         inst_id;                /*!< Instance id */
} __attribute__((packed)) xr_gatt_id_t;
 ...
 */
typedef struct {
    xr_gatt_id_t   id;                     /*!< Gatt id, include uuid and instance */
    bool            is_primary;             /*!< This service is primary or not */
} __attribute__((packed)) xr_gatt_srvc_id_t;
...
#define    XR_GATT_PERM_READ_AUTHORIZATION    (1 << 9)   /* bit 9 -  0x0200 */
#define    XR_GATT_PERM_WRITE_AUTHORIZATION   (1 << 10)  /* bit 10 - 0x0400 */
typedef uint16_t xr_gatt_perm_t;
...
typedef uint8_t xr_gatt_char_prop_t;

/// GATT maximum attribute length
#define XR_GATT_MAX_ATTR_LEN   600 //as same as GATT_MAX_ATTR_LEN

typedef enum {
    XR_GATT_SERVICE_FROM_REMOTE_DEVICE         = 0,                                       /* relate to BTA_GATTC_SERVICE_INFO_FROM_REMOTE_DEVICE in bta_gattc_int.h */
    XR_GATT_SERVICE_FROM_NVS_FLASH             = 1,                                       /* relate to BTA_GATTC_SERVICE_INFO_FROM_NVS_FLASH in bta_gattc_int.h */
    XR_GATT_SERVICE_FROM_UNKNOWN               = 2,                                       /* relate to BTA_GATTC_SERVICE_INFO_FROM_UNKNOWN in bta_gattc_int.h */
} xr_service_source_t;
...
typedef struct
{
    xr_attr_control_t      attr_control;                   /*!< The attribute control type */
    xr_attr_desc_t         att_desc;                       /*!< The attribute type */
} xr_gatts_attr_db_t;
...
} xr_gatts_incl_svc_desc_t;                                /*!< Gatt include service entry element */
...
} xr_gatts_incl128_svc_desc_t;                             /*!< Gatt  include 128 bit service entry element */
...
typedef union {
    xr_gatt_value_t attr_value;                            /*!< Gatt attribute structure */
    uint16_t            handle;                             /*!< Gatt attribute handle */
} xr_gatt_rsp_t;
...
/**
  ...
  */
typedef enum {
    XR_GATT_WRITE_TYPE_NO_RSP  =   1,                      /*!< Gatt write attribute need no response */
    XR_GATT_WRITE_TYPE_RSP,                                /*!< Gatt write attribute need remote response */
} xr_gatt_write_type_t;
...
} xr_gatt_conn_params_t;

#define XR_GATT_IF_NONE    0xff                            /*!< If callback report gattc_if/gatts_if as this macro, means this event is not correspond to any app */

typedef uint8_t    xr_gatt_if_t;                           /*!< Gatt interface type, different application on GATT client use different gatt_if */

/**
  * @brief the type of attribute element
  */
typedef enum {
    XR_GATT_DB_PRIMARY_SERVICE,                            /*!< Gattc primary service attribute type in the cache */
    XR_GATT_DB_SECONDARY_SERVICE,                          /*!< Gattc secondary service attribute type in the cache */
    XR_GATT_DB_CHARACTERISTIC,                             /*!< Gattc characteristic attribute type in the cache */
    XR_GATT_DB_DESCRIPTOR,                                 /*!< Gattc characteristic descriptor attribute type in the cache */
    XR_GATT_DB_INCLUDED_SERVICE,                           /*!< Gattc include service attribute type in the cache */
    XR_GATT_DB_ALL,                                        /*!< Gattc all the attribute (primary service & secondary service & include service & char & descriptor) type in the cache */
} xr_gatt_db_attr_type_t;                                  /*!< Gattc attribute type element */
...
typedef struct {
    ...
    uint16_t handles[XR_GATT_MAX_READ_MULTI_HANDLES];      /*!< The handles list */
} xr_gattc_multi_t;                                        /*!< The gattc multiple read element */

/**
  * @brief data base attribute element
  */
typedef struct {
    xr_gatt_db_attr_type_t     type;                       /*!< The attribute type */
    uint16_t                    attribute_handle;           /*!< The attribute handle, it's valid for all of the type */
    uint16_t                    start_handle;               /*!< The service start handle, it's valid only when the type = XR_GATT_DB_PRIMARY_SERVICE or XR_GATT_DB_SECONDARY_SERVICE */
    uint16_t                    end_handle;                 /*!< The service end handle, it's valid only when the type = XR_GATT_DB_PRIMARY_SERVICE or XR_GATT_DB_SECONDARY_SERVICE */
    xr_gatt_char_prop_t        properties;                 /*!< The characteristic properties, it's valid only when the type = XR_GATT_DB_CHARACTERISTIC */
    xr_bt_uuid_t               uuid;                       /*!< The attribute uuid, it's valid for all of the type */
} xr_gattc_db_elem_t;                                      /*!< The gattc service data base element in the cache */
...
typedef struct {
    ...
    xr_bt_uuid_t               uuid;                       /*!< The uuid of the service */
} xr_gattc_service_elem_t;                                 /*!< The gattc service element */
...
typedef struct {
    ...
    xr_gatt_char_prop_t        properties;                 /*!< The characteristic properties */
    xr_bt_uuid_t               uuid;                       /*!< The characteristic uuid */
} xr_gattc_char_elem_t;                                    /*!< The gattc characteristic element */
...
typedef struct {
    ...
    xr_bt_uuid_t              uuid;                        /*!< The characteristic descriptor uuid */
} xr_gattc_descr_elem_t;                                   /*!< The gattc descriptor type element */
...
typedef struct {
    ...
    xr_bt_uuid_t              uuid;                        /*!< The include service uuid */
} xr_gattc_incl_svc_elem_t;                                /*!< The gattc include service element */
