...
xr_err_t xr_blufi_register_callbacks(xr_blufi_callbacks_t *callbacks)
{
    if (xr_bluedroid_get_status() == XR_BLUEDROID_STATUS_UNINITIALIZED) {
    ...
    return (btc_profile_cb_set(BTC_PID_BLUFI, callbacks->event_cb) == 0 ? XR_OK : XR_FAIL);
}

xr_err_t xr_blufi_send_wifi_conn_report(wifi_mode_t opmode, xr_blufi_sta_conn_state_t sta_conn_state, uint8_t softap_conn_num, xr_blufi_extra_info_t *extra_info)
{
    btc_msg_t msg;
    ...
    return (btc_transfer_context(&msg, &arg, sizeof(btc_blufi_args_t), btc_blufi_call_deep_copy) == BT_STATUS_SUCCESS ? XR_OK : XR_FAIL);
}

xr_err_t xr_blufi_send_wifi_list(uint16_t apCount, xr_blufi_ap_record_t *list)
{
    btc_msg_t msg;
    btc_blufi_args_t arg;
...
xr_err_t xr_blufi_profile_init(void)
{
    btc_msg_t msg;
...
xr_err_t xr_blufi_profile_deinit(void)
{
    btc_msg_t msg;
...
uint16_t xr_blufi_get_version(void)
{
    return btc_blufi_get_version();
}

xr_err_t xr_blufi_close(xr_gatt_if_t gatts_if, uint16_t conn_id)
{
    btc_msg_t msg;
...
xr_err_t xr_blufi_send_error_info(xr_blufi_error_state_t state)
{
    btc_msg_t msg;
    btc_blufi_args_t arg;
    ...
    return (btc_transfer_context(&msg, &arg, sizeof(btc_blufi_args_t), NULL) == BT_STATUS_SUCCESS ? XR_OK : XR_FAIL);
}

xr_err_t xr_blufi_send_custom_data(uint8_t *data, uint32_t data_len)
{
    btc_msg_t msg;
    btc_blufi_args_t arg;
