    ...
    /* Finished enabling Encryption after role switch */
        ...
        if ((p_dev_rec = btm_find_dev (p->remote_addr)) != NULL) {
            ...
            BTM_TRACE_WARNING("btm_acl_encrypt_change: tBTM_SEC_DEV:0x%x rs_disc_pending=%d\n",
                            (UINT32)(uintptr_t)p_dev_rec, p_dev_rec->rs_disc_pending);
...
**
** Description      This function is called to set a connection into sniff
mode.
**                  A check is made if the connection is already in sniff or
park
                              ...
                              UINT16 attempt, UINT16 timeout)
{
    ...
    if (p != (tACL_CONN *)NULL)
    {
        ...
        BTM_ReadPowerMode(remote_bda, &p_mode);
        if (p_mode != BTM_ACL_MODE_NORMAL)
            return(BTM_WRONG_MODE);
...
**
** Description      This function is called to put a connection out of sniff
mode.
