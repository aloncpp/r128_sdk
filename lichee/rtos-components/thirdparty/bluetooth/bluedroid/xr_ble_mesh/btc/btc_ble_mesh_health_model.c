...
static inline void btc_ble_mesh_health_client_cb_to_app(xr_ble_mesh_health_client_cb_event_t event,
                                                        xr_ble_mesh_health_client_cb_param_t *param)
{
    xr_ble_mesh_health_client_cb_t btc_ble_mesh_cb =
        (xr_ble_mesh_health_client_cb_t)btc_profile_cb_get(BTC_PID_HEALTH_CLIENT);
    if (btc_ble_mesh_cb) {
...
void btc_ble_mesh_health_client_arg_deep_copy(btc_msg_t *msg, void *p_dest, void *p_src)
{
    ...
    case BTC_BLE_MESH_ACT_HEALTH_CLIENT_GET_STATE: {
        dst->health_client_get_state.params = (xr_ble_mesh_client_common_param_t *)bt_mesh_malloc(sizeof(xr_ble_mesh_client_common_param_t));
        ...
        if (src->health_client_get_state.get_state) {
            dst->health_client_get_state.get_state = (xr_ble_mesh_health_client_get_state_t *)bt_mesh_malloc(sizeof(xr_ble_mesh_health_client_get_state_t));
            if (dst->health_client_get_state.get_state) {
                memcpy(dst->health_client_get_state.get_state, src->health_client_get_state.get_state,
                    sizeof(xr_ble_mesh_health_client_get_state_t));
            } else {
                BT_ERR("%s, Out of memory, act %d", __func__, msg->act);
            }
        }
    ...
    case BTC_BLE_MESH_ACT_HEALTH_CLIENT_SET_STATE: {
        dst->health_client_set_state.params = (xr_ble_mesh_client_common_param_t *)bt_mesh_malloc(sizeof(xr_ble_mesh_client_common_param_t));
        dst->health_client_set_state.set_state = (xr_ble_mesh_health_client_set_state_t *)bt_mesh_malloc(sizeof(xr_ble_mesh_health_client_set_state_t));
        if (dst->health_client_set_state.params && dst->health_client_set_state.set_state) {
            ...
            memcpy(dst->health_client_set_state.set_state, src->health_client_set_state.set_state,
                   sizeof(xr_ble_mesh_health_client_set_state_t));
        } else {
            BT_ERR("%s, Out of memory, act %d", __func__, msg->act);
        }
        break;
...
{
    xr_ble_mesh_health_client_cb_param_t *p_dest_data = (xr_ble_mesh_health_client_cb_param_t *)p_dest;
    xr_ble_mesh_health_client_cb_param_t *p_src_data = (xr_ble_mesh_health_client_cb_param_t *)p_src;
    uint16_t length = 0U;
    ...
    switch (msg->act) {
    case XR_BLE_MESH_HEALTH_CLIENT_GET_STATE_EVT:
    case XR_BLE_MESH_HEALTH_CLIENT_SET_STATE_EVT:
    case XR_BLE_MESH_HEALTH_CLIENT_PUBLISH_EVT:
        if (p_src_data->params) {
    ...
    case XR_BLE_MESH_HEALTH_CLIENT_TIMEOUT_EVT:
        break;
    default:
        break;
    }
...
{
    xr_ble_mesh_health_client_cb_param_t *arg = NULL;
    ...
    arg = (xr_ble_mesh_health_client_cb_param_t *)(msg->arg);
...
static void btc_ble_mesh_health_client_callback(xr_ble_mesh_health_client_cb_param_t *cb_params, uint8_t act)
{
    btc_msg_t msg = {0};
    ...
    btc_transfer_context(&msg, cb_params, sizeof(xr_ble_mesh_health_client_cb_param_t),
...
{
    xr_ble_mesh_health_client_cb_param_t cb_params = {0};
    ...
    case BTC_BLE_MESH_EVT_HEALTH_CLIENT_GET_STATE:
        act = XR_BLE_MESH_HEALTH_CLIENT_GET_STATE_EVT;
        break;
    case BTC_BLE_MESH_EVT_HEALTH_CLIENT_SET_STATE:
        act = XR_BLE_MESH_HEALTH_CLIENT_SET_STATE_EVT;
        break;
    case BTC_BLE_MESH_EVT_HEALTH_CLIENT_PUBLISH:
        act = XR_BLE_MESH_HEALTH_CLIENT_PUBLISH_EVT;
        break;
    case BTC_BLE_MESH_EVT_HEALTH_CLIENT_TIMEOUT:
        act = XR_BLE_MESH_HEALTH_CLIENT_TIMEOUT_EVT;
        break;
    default:
...
static int btc_ble_mesh_health_client_get_state(xr_ble_mesh_client_common_param_t *params,
                                                xr_ble_mesh_health_client_get_state_t *get)
{
    ...
    if (params->opcode == XR_BLE_MESH_MODEL_OP_HEALTH_FAULT_GET && get == NULL) {
    ...
    switch (param.opcode) {
    case XR_BLE_MESH_MODEL_OP_ATTENTION_GET:
        return bt_mesh_health_attention_get(&param);
    case XR_BLE_MESH_MODEL_OP_HEALTH_PERIOD_GET:
        return bt_mesh_health_period_get(&param);
    case XR_BLE_MESH_MODEL_OP_HEALTH_FAULT_GET:
...
static int btc_ble_mesh_health_client_set_state(xr_ble_mesh_client_common_param_t *params,
                                                xr_ble_mesh_health_client_set_state_t *set)
{
    bt_mesh_client_common_param_t param = {0};
...
{
    btc_ble_mesh_health_client_args_t *arg = NULL;
    xr_ble_mesh_health_client_cb_param_t cb = {0};
    ...
    case BTC_BLE_MESH_ACT_HEALTH_CLIENT_GET_STATE: {
        ...
        if (cb.error_code) {
            ...
            btc_ble_mesh_health_client_callback(&cb, XR_BLE_MESH_HEALTH_CLIENT_GET_STATE_EVT);
        }
        break;
    }
    case BTC_BLE_MESH_ACT_HEALTH_CLIENT_SET_STATE: {
        ...
        if (cb.error_code) {
            /* If send failed, callback error_code to app layer immediately */
            btc_ble_mesh_health_client_callback(&cb, XR_BLE_MESH_HEALTH_CLIENT_SET_STATE_EVT);
        }
        break;
    }
    default:
...
{
    xr_ble_mesh_health_client_cb_param_t *param = NULL;
    ...
    param = (xr_ble_mesh_health_client_cb_param_t *)(msg->arg);

    if (msg->act < XR_BLE_MESH_HEALTH_CLIENT_EVT_MAX) {
...
static inline void btc_ble_mesh_health_server_cb_to_app(xr_ble_mesh_health_server_cb_event_t event,
                                                        xr_ble_mesh_health_server_cb_param_t *param)
{
    xr_ble_mesh_health_server_cb_t btc_ble_mesh_cb =
        (xr_ble_mesh_health_server_cb_t)btc_profile_cb_get(BTC_PID_HEALTH_SERVER);
    if (btc_ble_mesh_cb) {
        btc_ble_mesh_cb(event, param);
    }
}
...
static void btc_ble_mesh_health_server_copy_req_data(btc_msg_t *msg, void *p_dest, void *p_src)
{
    ...
    switch (msg->act) {
    case XR_BLE_MESH_HEALTH_SERVER_FAULT_UPDATE_COMP_EVT:
        break;
    default:
        break;
    }
...
static void btc_ble_mesh_health_server_callback(xr_ble_mesh_health_server_cb_param_t *cb_params, uint8_t act)
{
    btc_msg_t msg = {0};
    ...
    btc_transfer_context(&msg, cb_params, sizeof(xr_ble_mesh_health_server_cb_param_t),
                         btc_ble_mesh_health_server_copy_req_data);
}

void btc_ble_mesh_health_server_call_handler(btc_msg_t *msg)
{
    xr_ble_mesh_health_server_cb_param_t param = {0};
    ...
    case BTC_BLE_MESH_ACT_HEALTH_SERVER_FAULT_UPDATE:
        ...
        btc_ble_mesh_health_server_callback(&param, XR_BLE_MESH_HEALTH_SERVER_FAULT_UPDATE_COMP_EVT);
        break;
    default:
        break;
    }
...
{
    xr_ble_mesh_health_server_cb_param_t *param = NULL;
    ...
    param = (xr_ble_mesh_health_server_cb_param_t *)(msg->arg);

    if (msg->act < XR_BLE_MESH_HEALTH_SERVER_EVT_MAX) {
...
void btc_ble_mesh_health_server_fault_clear(struct bt_mesh_model *model, uint16_t company_id)
{
    ...
    param.fault_clear.model = (xr_ble_mesh_model_t *)model;
    param.fault_clear.company_id = company_id;

    btc_ble_mesh_health_server_callback(&param, XR_BLE_MESH_HEALTH_SERVER_FAULT_CLEAR_EVT);
}
                                           ...
                                           uint8_t test_id, uint16_t company_id)
{
    ...
    param.fault_test.model = (xr_ble_mesh_model_t *)model;
    param.fault_test.test_id = test_id;
    param.fault_test.company_id = company_id;

    btc_ble_mesh_health_server_callback(&param, XR_BLE_MESH_HEALTH_SERVER_FAULT_TEST_EVT);
}
...
void btc_ble_mesh_health_server_attention_on(struct bt_mesh_model *model, uint8_t time)
{
    ...
    param.attention_on.model = (xr_ble_mesh_model_t *)model;
    param.attention_on.time = time;

    btc_ble_mesh_health_server_callback(&param, XR_BLE_MESH_HEALTH_SERVER_ATTENTION_ON_EVT);
}
...
void btc_ble_mesh_health_server_attention_off(struct bt_mesh_model *model)
{
    ...
    param.attention_off.model = (xr_ble_mesh_model_t *)model;

    btc_ble_mesh_health_server_callback(&param, XR_BLE_MESH_HEALTH_SERVER_ATTENTION_OFF_EVT);
}
