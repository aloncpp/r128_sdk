...
typedef struct alarm_t {
    ...
    // xr_timer_handle_t alarm_hdl; //todo might problem
...
int64_t k_uptime_get(void)
{
     ...
     * but xr_timer_get_time is in microseconds
     */
    // return (xr_timer_get_time() / 1000);
	struct timeval tm_val_now;
	gettimeofday(&tm_val_now, NULL);
    return (int64_t)(tm_val_now.tv_sec * 1000 + tm_val_now.tv_usec / 1000); //todo might problem
}
...
uint32_t k_uptime_get_32(void)
{
	...
	struct timeval tm_val_now;
	gettimeofday(&tm_val_now, NULL);
    return (uint32_t)(tm_val_now.tv_sec * 1000 * 1000 + tm_val_now.tv_usec); //todo might problem
}
