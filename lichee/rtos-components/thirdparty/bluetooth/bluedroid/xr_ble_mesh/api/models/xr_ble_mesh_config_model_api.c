...
xr_err_t xr_ble_mesh_register_config_client_callback(xr_ble_mesh_cfg_client_cb_t callback)
{
    ...
    return (btc_profile_cb_set(BTC_PID_CONFIG_CLIENT, callback) == 0 ? XR_OK : XR_FAIL);
}

static bool config_client_get_need_param(xr_ble_mesh_opcode_t opcode)
{
    switch (opcode) {
                                               ...
                                               xr_ble_mesh_cfg_client_get_state_t *get_state)
{
    ...
    if (params == NULL || params->model == NULL ||
        ...
        !XR_BLE_MESH_ADDR_IS_UNICAST(params->ctx.addr) ||
                                               ...
                                               xr_ble_mesh_cfg_client_set_state_t *set_state)
{
    ...
    if (params == NULL || params->model == NULL ||
        params->ctx.net_idx == XR_BLE_MESH_KEY_UNUSED ||
        !XR_BLE_MESH_ADDR_IS_UNICAST(params->ctx.addr) ||
        (params->opcode != XR_BLE_MESH_MODEL_OP_NODE_RESET && set_state == NULL)) {
        return XR_ERR_INVALID_ARG;
    }
...
xr_err_t xr_ble_mesh_register_config_server_callback(xr_ble_mesh_cfg_server_cb_t callback)
{
    ...
    return (btc_profile_cb_set(BTC_PID_CONFIG_SERVER, callback) == 0 ? XR_OK : XR_FAIL);
}
