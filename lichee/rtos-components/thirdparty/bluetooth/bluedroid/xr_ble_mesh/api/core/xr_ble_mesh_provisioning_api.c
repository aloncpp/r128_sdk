...
xr_err_t xr_ble_mesh_register_prov_callback(xr_ble_mesh_prov_cb_t callback)
{
    ...
    return (btc_profile_cb_set(BTC_PID_PROV, callback) == 0 ? XR_OK : XR_FAIL);
}
...
static bool prov_bearers_valid(xr_ble_mesh_prov_bearer_t bearers)
{
    if ((!(bearers & (XR_BLE_MESH_PROV_ADV | XR_BLE_MESH_PROV_GATT))) ||
        (IS_ENABLED(CONFIG_BLE_MESH_PB_ADV) &&
            !IS_ENABLED(CONFIG_BLE_MESH_PB_GATT) &&
            !(bearers & XR_BLE_MESH_PROV_ADV)) ||
        (!IS_ENABLED(CONFIG_BLE_MESH_PB_ADV) &&
            IS_ENABLED(CONFIG_BLE_MESH_PB_GATT) &&
            !(bearers & XR_BLE_MESH_PROV_GATT))) {
        return false;
    }
    return true;
}
...
xr_err_t xr_ble_mesh_node_input_string(const char *string)
{
    ...
    if (!string || strlen(string) > XR_BLE_MESH_PROV_INPUT_OOB_MAX_LEN) {
        return XR_ERR_INVALID_ARG;
    }
...
xr_err_t xr_ble_mesh_set_unprovisioned_device_name(const char *name)
{
    ...
    if (!name || strlen(name) > XR_BLE_MESH_DEVICE_NAME_MAX_LEN) {
        return XR_ERR_INVALID_ARG;
    }
    ...
    strncpy(arg.set_device_name.name, name, XR_BLE_MESH_DEVICE_NAME_MAX_LEN);
...
xr_err_t xr_ble_mesh_provisioner_input_string(const char *string, uint8_t link_idx)
{
    ...
    if (!string || strlen(string) > XR_BLE_MESH_PROV_OUTPUT_OOB_MAX_LEN ||
                                                  ...
                                                  xr_ble_mesh_dev_add_flag_t flags)
{
    ...
    memcpy(arg.provisioner_dev_add.add_dev.addr, add_dev->addr, sizeof(xr_ble_mesh_bd_addr_t));
                                                         ...
                                                         uint16_t oob_info, uint16_t unicast_addr)
{
    ...
    if (uuid == NULL || (bearer == XR_BLE_MESH_PROV_GATT && (addr == NULL ||
        addr_type > XR_BLE_MESH_ADDR_TYPE_RANDOM)) ||
        (bearer != XR_BLE_MESH_PROV_ADV && bearer != XR_BLE_MESH_PROV_GATT) ||
        !XR_BLE_MESH_ADDR_IS_UNICAST(unicast_addr)) {
        return XR_ERR_INVALID_ARG;
    }
...
xr_err_t xr_ble_mesh_provisioner_delete_dev(xr_ble_mesh_device_delete_t *del_dev)
{
    ...
    if (del_dev->flag & DEL_DEV_ADDR_FLAG) {
        ...
        memcpy(arg.provisioner_dev_del.del_dev.addr, del_dev->addr, sizeof(xr_ble_mesh_bd_addr_t));
                                                      ...
                                                      uint8_t offset, bool prov_after_match)
{
    ...
    if (match_len + offset > XR_BLE_MESH_OCTET16_LEN) {
        return XR_ERR_INVALID_ARG;
    }
...
xr_err_t xr_ble_mesh_provisioner_set_primary_elem_addr(uint16_t addr)
{
    ...
    if (!XR_BLE_MESH_ADDR_IS_UNICAST(addr)) {
        return XR_ERR_INVALID_ARG;
    }
...
xr_err_t xr_ble_mesh_set_fast_prov_info(xr_ble_mesh_fast_prov_info_t *fast_prov_info)
{
    ...
    if (fast_prov_info == NULL || (fast_prov_info->offset +
            fast_prov_info->match_len > XR_BLE_MESH_OCTET16_LEN)) {
        return XR_ERR_INVALID_ARG;
    }
