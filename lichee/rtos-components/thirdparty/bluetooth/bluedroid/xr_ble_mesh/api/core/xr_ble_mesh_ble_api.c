...
xr_err_t xr_ble_mesh_register_ble_callback(xr_ble_mesh_ble_cb_t callback)
{
    ...
    return (btc_profile_cb_set(BTC_PID_BLE_MESH_BLE_COEX, callback) == 0 ? XR_OK : XR_FAIL);
}
                                             ...
                                             const xr_ble_mesh_ble_adv_data_t *data)
{
    ...
    memcpy(&arg.start_ble_adv.param, param, sizeof(xr_ble_mesh_ble_adv_param_t));
    if (data) {
        memcpy(&arg.start_ble_adv.data, data, sizeof(xr_ble_mesh_ble_adv_data_t));
    }
