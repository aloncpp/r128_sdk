...
#ifndef _XR_BLE_MESH_NETWORKING_API_H_
#define _XR_BLE_MESH_NETWORKING_API_H_
...
typedef void (* xr_ble_mesh_model_cb_t)(xr_ble_mesh_model_cb_event_t event,
 ...
 *                  application layer;
 *                - If users call xr_ble_mesh_server/client_model_send, this callback notifies the
 ...
 *                For example, xr_ble_mesh_register_config_client_callback is the register
 ...
 *                      the user_data(defined using xr_ble_mesh_client_t_) of the client model
 ...
 *
 * @note          This function shall be invoked before xr_ble_mesh_deinit() is called.
 *
 ...
 *                publication message (@ref xr_ble_mesh_model_pub_t.msg) contains a valid
 *                message to be sent. And if users want to update the publishing message,
 *                this API should be called in XR_BLE_MESH_MODEL_PUBLISH_UPDATE_EVT
 ...
 * @param[in]    length: Message length (exclude the message opcode).
 * @param[in]    data: Parameters of the Access Payload (exclude the message opcode) to be sent.
 * @param[in]    device_role: Role of the device (node/provisioner) publishing the message of the type xr_ble_mesh_dev_role_t.
 *
 * @return       XR_OK on success or error code otherwise.
 *
 */
 ...
 *                needs to be reprovisioned. The API function xr_ble_mesh_node_prov_enable()
 ...
 *
 * @note         index is obtained from the parameters of XR_BLE_MESH_PROVISIONER_PROV_COMPLETE_EVT.
 *
 * @return       XR_OK on success or error code otherwise.
 *
 */
 ...
 *                if the node exists, i.e. if the *(xr_ble_mesh_node_t **node) is NULL.
 *                For example:
 *                ```
 *                const xr_ble_mesh_node_t **entry = xr_ble_mesh_provisioner_get_node_table_entry();
 *                for (int i = 0; i < CONFIG_BLE_MESH_MAX_PROV_NODES; i++) {
 *                    const xr_ble_mesh_node_t *node = entry[i];
 *                    if (node) {
 *                        ......
 *                    }
 *                }
...
#endif /* _XR_BLE_MESH_NETWORKING_API_H_ */
