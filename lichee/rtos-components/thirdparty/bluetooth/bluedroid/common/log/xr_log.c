...
#include <string.h>
#include "xr_log.h"
//#include "soc/soc_memory_layout.h"  // for xr_ptr_byte_accessible


//print number of bytes per line for xr_log_buffer_char and xr_log_buffer_hex
#define BYTES_PER_LINE 16

void xr_log_buffer_hex_internal(const char *tag, const void *buffer, uint16_t buff_len,
                                 xr_log_level_t log_level)
{
    if (buff_len == 0) {
        return;
    }
    ...
    do {
        ...
        //if (!xr_ptr_byte_accessible(buffer)) {test for lwh
        if(0){
        ...
        XR_LOG_LEVEL(log_level, tag, "%s", hex_buffer);
        buffer += bytes_cur_line;
        buff_len -= bytes_cur_line;
    } while (buff_len);
}

void xr_log_buffer_char_internal(const char *tag, const void *buffer, uint16_t buff_len,
                                  xr_log_level_t log_level)
{
    if (buff_len == 0) {
        return;
    ...
    do {
        ...
        XR_LOG_LEVEL(log_level, tag, "%s", char_buffer);
        buffer += bytes_cur_line;
        buff_len -= bytes_cur_line;
    } while (buff_len);
}
...
void xr_log_buffer_hexdump_internal(const char *tag, const void *buffer, uint16_t buff_len, xr_log_level_t log_level)
{
    ...
    do {
        ...
        XR_LOG_LEVEL(log_level, tag, "%s", hd_buffer);
        buffer += bytes_cur_line;
        buff_len -= bytes_cur_line;
    } while (buff_len);
}
