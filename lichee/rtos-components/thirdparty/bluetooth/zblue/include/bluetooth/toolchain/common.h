...
#define Z_STRUCT_SECTION_FOREACH(struct_type, iterator) \
	...
	for (struct struct_type *iterator = \
		...
		iterator < _CONCAT(_##struct_type, _list_end); }); \
	     iterator++)
#else
#define Z_STRUCT_SECTION_LIST(struct_type) \
	static sys_slist_t struct_type##_list
 ...
 */
#define Z_STRUCT_SECTION_FOREACH(struct_type, iterator) \
	struct struct_type *iterator;	\
	SYS_SLIST_FOR_EACH_CONTAINER(&struct_type##_list, iterator, node)
