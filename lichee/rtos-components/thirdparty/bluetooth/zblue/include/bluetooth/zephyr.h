...
********************************************************************************
* @file    zephyr.h
* @brief   This file contain the delcarations of zephyr kernel function.
******************************************************************************
*/
...
typedef struct _wait_q{
	XR_OS_Semaphore_t waitq;
} _wait_q_t;

#define _WAIT_Q_INIT(wait_q) { {NULL} }
...
/**
 * @brief Get thread ID(!but the thread handle from os in fact by XRadio
 * Realization) of the current thread.
 *
 * @return ID of current thread.
 *
 * @req K-THREAD-013
 */
#define k_current_get() XR_OS_ThreadGetCurrentHandle()
 ...
 */
static inline s32_t k_ms_to_ticks_ceil32(s32_t ms)
{
	return (s32_t)XR_OS_MSecsToTicks(ms);
}
...
{
	return (uint64_t)XR_OS_TicksToMSecs(t) * (NSEC_PER_USEC) * (USEC_PER_MSEC);
}
 ...
 * TODO: Time comparing should use XR_OS_TimeAfter in zephyr host!
 */
...
{
	return XR_OS_SemaphoreCreate(&w->waitq, 0, XR_OS_SEMAPHORE_MAX_COUNT);
}

static inline int z_waitq_init_n(_wait_q_t *w, uint32_t n)
{
	return XR_OS_SemaphoreCreate(&w->waitq, n, XR_OS_SEMAPHORE_MAX_COUNT);
}

#if defined(CONFIG_BT_DEINIT)
static inline int z_waitq_deinit_n(_wait_q_t *w)
{
	return XR_OS_SemaphoreDelete(&w->waitq);
}
#endif
...
static inline int z_pend_curr(void *lock, u32_t key,
	       _wait_q_t *wait_q, s32_t timeout)
{
	irq_unlock(key);
	return XR_OS_SemaphoreWait(&wait_q->waitq, (XR_OS_Time_t)timeout);
}

static inline int z_pend_curr_irqlock(u32_t key, _wait_q_t *wait_q, s32_t timeout)
{
	irq_unlock(key);
	return XR_OS_SemaphoreWait(&wait_q->waitq, (XR_OS_Time_t)timeout);
}
...
{
	XR_OS_SemaphoreRelease(&wait_q->waitq);
	return (void *)!0;
}
...
#define k_cycle_get_32() XR_OS_GetTicks()

#define SYS_CLOCK_HW_CYCLES_TO_NS(c) (XR_OS_TicksToMSecs(c) * 1000U * 1000U)
...
#if defined(CONFIG_BT_DEINIT)
#define K_POLL_STATE_DEINIT _POLL_STATE_BIT(_POLL_STATE_DEINIT)
#endif
...
#define BLE_HOST_VERSION "XRadio BLE HOST V2.3.0.1"
 ...
 */
#ifdef CONFIG_BT_IRQ_LOCK_BY_MUTEX
