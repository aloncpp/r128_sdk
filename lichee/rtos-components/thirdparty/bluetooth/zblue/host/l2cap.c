...
#else
static struct bt_l2cap *bt_l2cap_pool;
#endif
...
void bt_l2cap_encrypt_change(struct bt_conn *conn, uint8_t hci_status)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&conn->channels, chan, node) {//todo not safe
#if defined(CONFIG_BT_L2CAP_DYNAMIC_CHANNEL)
...
int bt_l2cap_server_unregister(uint16_t psm)
{
	struct bt_l2cap_server *server;
	...
	sys_slist_find_and_remove(&servers, &server->node);
				   ...
				   struct net_buf *buf, uint16_t seg)
{
	...
	BT_DBG("chan %p len %u", chan, net_buf_frags_len(buf));
				   ...
				   struct net_buf *buf)
{
	...
	BT_DBG("chan %p seg %d len %u", chan, seg, net_buf_frags_len(buf));
...
#if defined(CONFIG_BT_DEINIT)
#if !defined(CONFIG_BLEHOST_Z_ITERABLE_SECTION)
void bt_l2cap_le_fixed_chan_unregister(uint16_t cid)
{
	// struct bt_gatt_service *tmp;
	struct bt_l2cap_chan *chan;

	BT_DBG("CID 0x%04x", cid);

	SYS_SLIST_FOR_EACH_CONTAINER(&le_channels, chan, node) { //todo48
		sys_slist_find_and_remove(&le_channels, &chan->node); //todo48
	}
}
#endif
#endif
...
#if defined(CONFIG_BT_VAR_MEM_DYNC_ALLOC)
	bt_l2cap_pool = (struct bt_l2cap *)k_malloc(sizeof(struct bt_l2cap) * CONFIG_BT_MAX_CONN);
	if (bt_l2cap_pool == NULL) {
		BT_ERR("Alloc bt l2cap pool error");
		return ;
	}
...
#if !defined(CONFIG_BLEHOST_Z_ITERABLE_SECTION)
	bt_l2cap_le_fixed_chan_unregister(BT_L2CAP_CID_LE_SIG);
#endif
}

void bt_l2cap_mem_deinit(void)
{
	uint32_t i;

#if defined(CONFIG_BT_L2CAP_DYNAMIC_CHANNEL)
	NET_BUF_POOL_FIXED_DEINIT(&disc_pool, 1,
			  BT_L2CAP_BUF_SIZE(CONFIG_BT_L2CAP_TX_MTU), NULL);
#endif
...
#endif
	{
		memset(&bt_l2cap_pool[i], 0, sizeof(struct bt_l2cap));
	}
}
#endif
...
int bt_l2cap_chan_send(struct bt_l2cap_chan *chan, struct net_buf *buf)
{
	...
	BT_DBG("chan %p buf %p len %u", chan, buf, net_buf_frags_len(buf));
