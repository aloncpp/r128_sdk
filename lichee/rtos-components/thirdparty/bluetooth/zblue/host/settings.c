...
struct settings_handler bt_handler = {
		.name = "bt",
		.h_get = NULL,
		.h_set = set,
		.h_commit = commit,
...
extern struct settings_handler bt_ccc_handler;
extern struct settings_handler bt_sc_handler;
#if defined(CONFIG_BT_GATT_CACHING)
extern struct settings_handler bt_cf_handler;
extern struct settings_handler bt_hash_handler;
#endif
extern struct settings_handler bt_keys_handler;
#endif
...
#if defined(CONFIG_SETTINGS_DYNAMIC_HANDLERS) && !defined(CONFIG_BLEHOST_Z_ITERABLE_SECTION)
	settings_register(&bt_handler);
	settings_register(&bt_ccc_handler);
	settings_register(&bt_sc_handler);
#if defined(CONFIG_BT_GATT_CACHING)
	settings_register(&bt_cf_handler);
	settings_register(&bt_hash_handler);
#endif
	settings_register(&bt_keys_handler);
#endif
...
int bt_settings_deinit(void)
{
	int err;

#if defined(CONFIG_SETTINGS_DYNAMIC_HANDLERS) && !defined(CONFIG_BLEHOST_Z_ITERABLE_SECTION)
	settings_unregister(&bt_handler);
	settings_unregister(&bt_ccc_handler);
	settings_unregister(&bt_sc_handler);
#if defined(CONFIG_BT_GATT_CACHING)
	settings_unregister(&bt_cf_handler);
	settings_unregister(&bt_hash_handler);
#endif
	settings_unregister(&bt_keys_handler);
#endif

	err = settings_subsys_deinit();
	if (err) {
		BT_ERR("settings_subsys_deinit failed (err %d)", err);
		return err;
	}
