...
static int chan_req_send(struct bt_att_chan *chan, struct bt_att_req *req)
{
	...
	BT_DBG("chan %p req %p len %u", chan, req,
...
struct net_buf *bt_att_create_pdu(struct bt_conn *conn, uint8_t op, size_t len)
{
	...
	BT_WARN("No ATT channel for MTU %u", len + sizeof(op));
...
void bt_att_init(void)
{
	...
	k_mem_slab_init(&req_slab, _k_mem_slab_buf_req_slab,
					sizeof(struct bt_att_req), CONFIG_BT_ATT_TX_MAX);
	k_mem_slab_init(&att_slab, _k_mem_slab_buf_att_slab, sizeof(struct bt_att),
					CONFIG_BT_MAX_CONN);
	k_mem_slab_init(&chan_slab, _k_mem_slab_buf_chan_slab,
					sizeof(struct bt_att_chan),
...
static int bt_eatt_deinit(void)
{
	int err;

	BT_DBG("");

	err = bt_l2cap_server_unregister(BT_EATT_PSM);
	if (err) {
		BT_ERR("Remove EATT Failed (ERR %d)", err);
		return err;
	}
...
int bt_att_deinit(void)
{
	...
	if (IS_ENABLED(CONFIG_BT_EATT)) {
		err = bt_eatt_deinit();
		if (err) {
			BT_ERR("EATT deinit failed (ERR %d)", err);
			return err;
		}
	}
	...
	err = bt_gatt_deinit();
	if (err) {
		BT_ERR("GATT deinit failed (ERR) %d", err);
		return err;
	}

#if !defined(CONFIG_BLEHOST_Z_ITERABLE_SECTION)

		bt_l2cap_le_fixed_chan_unregister(BT_L2CAP_CID_ATT);

		k_mem_slab_deinit(&req_slab);
		k_mem_slab_deinit(&att_slab);
		k_mem_slab_deinit(&chan_slab);
#endif /* !CONFIG_BLEHOST_Z_ITERABLE_SECTION */

	return 0;
}

void bt_att_mem_deinit(void)
{
#if CONFIG_BT_ATT_PREPARE_COUNT > 0
	NET_BUF_POOL_FIXED_DEINIT(&prep_pool,
		CONFIG_BT_ATT_PREPARE_COUNT, BT_ATT_MTU, sizeof(struct bt_attr_data), NULL);
#endif

#if defined(CONFIG_BLEHOST_Z_ITERABLE_SECTION)
#if defined(CONFIG_BT_VAR_MEM_DYNC_ALLOC)
	mem_slab_var_deinit(&req_slab, sizeof(struct bt_att_req), CONFIG_BT_L2CAP_TX_BUF_COUNT);
	mem_slab_var_deinit(&att_slab, sizeof(struct bt_att), CONFIG_BT_MAX_CONN);
	mem_slab_var_deinit(&chan_slab, sizeof(struct bt_att_chan), CONFIG_BT_MAX_CONN * ATT_CHAN_MAX);
#else
	req_slab = (struct k_mem_slab)K_MEM_SLAB_DEINIT(req_slab, sizeof(struct bt_att_req),
		  CONFIG_BT_L2CAP_TX_BUF_COUNT, 16);
	att_slab = (struct k_mem_slab)K_MEM_SLAB_DEINIT(att_slab, sizeof(struct bt_att),
		  CONFIG_BT_MAX_CONN, 16);
	chan_slab = (struct k_mem_slab)K_MEM_SLAB_DEINIT(chan_slab, sizeof(struct bt_att_chan),
		  CONFIG_BT_MAX_CONN * ATT_CHAN_MAX, 16);
#endif
#endif
}
