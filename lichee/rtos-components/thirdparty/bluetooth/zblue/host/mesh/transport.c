				 ...
				 struct net_buf_simple *sdu)
{
	...
	if (node == NULL) {
		BT_WARN("No node found for addr 0x%04x", rx->ctx.addr);
		return -EINVAL;
	}
...
#if 0 //Wewisetek : disable for not used
...
void bt_mesh_trans_init(void)
{
	...
	k_mem_slab_init(&segs, _k_mem_slab_buf_segs,
					BT_MESH_APP_SEG_SDU_MAX, CONFIG_BT_MESH_SEG_BUFS);
#endif
