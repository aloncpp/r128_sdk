...
static int node_id_adv(struct bt_mesh_subnet *sub)
{
	...
	err = bt_le_adv_start(&temp_fast_adv_param, node_id_ad,
...
static size_t gatt_prov_adv_create(struct bt_data prov_sd[2])
{
	...
	if (prov->uri) {
		...
		if (uri_len > 29) {
			...
			prov_sd[0].data = (u8_t *)prov->uri; //Wewisetek : modify for compiler error
			sd_space -= 2 + uri_len;
			prov_sd_len++;
		}
	}
	...
	if (sd_space > 2 && name_len > 0) {
		...
		prov_sd[prov_sd_len].data = (u8_t *)name; //Wewisetek : modify for compiler error
		prov_sd_len++;
	}
