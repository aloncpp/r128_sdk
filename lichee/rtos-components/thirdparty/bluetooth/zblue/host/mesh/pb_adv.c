...
{
    /* During the test with R328, we found that there was a problem with the 
       expected id in the provisioning packet sent by R328, and it was not 
       changed as required. In order to continue the test, the expected id 
       check was temporarity blocked. Add by liuzhang, 20200824  */
#if (BT_MESH_FIXED_TRANSACTION_ID_CHECK)
