...
#ifdef ENABLE_STACK //Wewisetek Modified
static K_THREAD_STACK_DEFINE(rx_thread_stack, CONFIG_BT_RX_STACK_SIZE);
#else
static k_thread_stack_t *rx_thread_stack = NULL;
#endif
#endif
...
#else
static k_thread_stack_t *tx_thread_stack = NULL;
#endif
...
#if defined(CONFIG_BT_USER_DATA_LEN_UPDATE) //Wewisetek Modified
			...
			struct net_buf *buf, uint8_t len)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&scan_cbs, listener, node) { //safe
		net_buf_simple_save(&buf->b, &state);
...
int bt_enable(bt_ready_cb_t cb)
{
	...
	/* TX thread */
	k_thread_create(&tx_thread_data, tx_thread_stack,"zbt_hci_tx",
			/*K_THREAD_STACK_SIZEOF(tx_thread_stack)+1020*/CONFIG_BT_HCI_TX_STACK_SIZE,
...
#if !defined(CONFIG_BT_RECV_IS_RX_THREAD)
	/* RX thread */
	k_thread_create(&rx_thread_data, rx_thread_stack,"zbt_hci_rx",
			/*K_THREAD_STACK_SIZEOF(rx_thread_stack)*/CONFIG_BT_RX_STACK_SIZE,
...
int common_deinit(void)
{
	...
	bt_dev.init = (struct k_work)_K_WORK_INITIALIZER(init_work);
#if !defined(CONFIG_BT_WAIT_NOP)
	bt_dev.ncmd_sem 	 = (struct k_sem)_K_SEM_INITIALIZER(bt_dev.ncmd_sem, 1, 1);
#else
	bt_dev.ncmd_sem 	 = (struct k_sem)_K_SEM_INITIALIZER(bt_dev.ncmd_sem, 0, 1);
#endif
	bt_dev.cmd_tx_queue  = (struct k_fifo)_K_FIFO_INITIALIZER(bt_dev.cmd_tx_queue);
#if !defined(CONFIG_BT_RECV_IS_RX_THREAD)
	bt_dev.rx_queue 	 = (struct k_fifo)_K_FIFO_INITIALIZER(bt_dev.rx_queue);
#endif
