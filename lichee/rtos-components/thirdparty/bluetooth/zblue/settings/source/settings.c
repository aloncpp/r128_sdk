...
int settings_deinit(void)
{
	...
	XR_OS_RecursiveMutexLock(&settings_lock, K_FOREVER);
	...
	XR_OS_RecursiveMutexUnlock(&settings_lock);
	err = XR_OS_RecursiveMutexDelete(&settings_lock);
	if (err) {
		return err;
	}
