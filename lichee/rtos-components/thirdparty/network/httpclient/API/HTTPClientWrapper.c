...
void HTTPWrapperGetRespHeader(const char *response)
{
        char *pos = strstr(response, "HTTP/");
        if (pos)
                sscanf(pos, "%*s %d", &http_resp.status_code);

        if ((pos = strstr(response, "Content-Type:")) ||
            (pos = strstr(response, "content-type:")))
                sscanf(pos, "%*s %10s", http_resp.content_type);

        if ((pos = strstr(response, "Content-Length:")) ||
            (pos = strstr(response, "content-length:")))
                sscanf(pos, "%*s %ld", &http_resp.content_length);

       //printf("GetRespHeader resp.content_length:%ld\n", http_resp.content_length);
        return;
}

long HTTPWrapperGetRespContentLength()
{
       //printf("GetRespContentLength resp.content_length:%ld\n", http_resp.content_length);
        return http_resp.content_length;
}
...
long HTTPWrapperGetUpTime()
{
        ...
        return tp.tv_sec;
#else
        return XR_OS_TicksToSecs(XR_OS_GetTicks());
#endif
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
