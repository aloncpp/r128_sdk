...
//	fcntl(iSockID, F_SETFL, O_NONBLOCK);  /* set noblocking */
				...
				if (reply_size >= (int)(sizeof(struct ip_hdr)+sizeof(struct icmp_echo_hdr))) {
					...
					if ((pecho->id == PING_IDs) && (pecho->seqno == htons(ping_seq_num))) {
						...
						printf("%d bytes from %s: icmp_seq=%d	 time=%d ms\n",
						       ...
						       htons(pecho->seqno), OS_TicksToMSecs(TimeElapse));
						max_rtt = TimeElapse > max_rtt ? TimeElapse : max_rtt;
						min_rtt = TimeElapse < min_rtt ? TimeElapse : min_rtt;
						time_counter += TimeElapse;
						ping_pass++;
						break;
					}
				}
	...
	printf("%d packets transmitted, %d received, %d packet loss\n",
			data->count,
			ping_pass,
			data->count - ping_pass);
	printf("rtt min/avg/max = %d/%d/%dms\n\n",
			OS_TicksToMSecs(min_rtt),
			OS_TicksToMSecs(time_counter/data->count),
			OS_TicksToMSecs(max_rtt));
