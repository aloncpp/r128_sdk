...
decide_what_to_do(struct conn *c)
{
...
#if !defined(SHTTPD_FS)
	    	if ((c->loc.chan.fh =(unsigned int)(unsigned long)_shttpd_open(path, 0, 0)) == 0) {
#else
	...
	else if ((c->loc.chan.fh = (unsigned int)(unsigned long)_shttpd_open(path, 0, 0)) != 0) {
		_shttpd_get_file(c, &st);
#endif
	} else {
