...
#ifdef XR_OS_GetTicks
#undef XR_OS_GetTicks
#endif /* XR_OS_GetTicks */
#define XR_OS_GetTicks() (tick)
...
{
	timer->end_time = XR_OS_TicksToMSecs(XR_OS_GetTicks()) + timeout_ms;
}
...
{
	int diff = (int)(timer->end_time) - (int)(XR_OS_TicksToMSecs(XR_OS_GetTicks()));
...
{
	return 0 <= (int)XR_OS_TicksToMSecs(XR_OS_GetTicks()) - (int)(timer->end_time); /* is time_now over than end time */
}
...
int mqtt_random()
{
    return XR_OS_Rand32();//(((unsigned int)rand() << 16) + rand());
}
