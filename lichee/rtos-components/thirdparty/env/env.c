...
{
#ifdef CONFIG_COMPONENTS_ENV_FILE_SUPPORT
	int fd, len;
	if (dev_current == 0) {
		fd = open("/dev/env", O_RDONLY);
		if (fd == -1) {
			printf("open part env fail\n");
			return -1;
		}
	} else if (dev_current == 1) {
		fd = open("/dev/env-redund", O_RDONLY);
		if (fd == -1) {
	...
	//printf("dev_current:%d, read len:%d, size:%d\n", dev_current, len, size);
	close(fd);
	if (len != size) {
...
#ifdef CONFIG_COMPONENTS_ENV_PRINTENV
int cmd_fw_printenv(int argc, char ** argv)
{
	int i, rc = 0;
	int value_only = 0;
...
FINSH_FUNCTION_EXPORT_CMD(cmd_fw_printenv, fw_printenv, Print env);
#endif
...
FINSH_FUNCTION_EXPORT_CMD(cmd_fw_setenv, fw_setenv, Set env);
#endif
