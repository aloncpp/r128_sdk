...
time_t os_time(time_t *timer)
{
	if (!timer)
		return xTaskGetTickCount() / CONFIG_HZ;
	*timer = (time_t)xTaskGetTickCount() / CONFIG_HZ;
	return *timer;
}
...
int cmd_whetstone(int argc, char ** argv)
{
	return whetstone_main();
}

FINSH_FUNCTION_EXPORT_CMD(cmd_whetstone, whetstone, benchmark);
