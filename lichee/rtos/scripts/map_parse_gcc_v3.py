...
with open(map_file, 'r') as f:
    ...
    mem_config_text = '\n'
    mem_config_text_res = re.findall('Memory Configuration\n\nName             Origin             Length             Attributes\n([\s\S]+)\nLinker script and memory map', s)
    if (len(mem_config_text_res)) == 0:
        mem_config_text += re.findall('内存配置\n\n名称           来源             长度             属性\n([\s\S]+)\n链结器命令稿和内存映射', s)[0]
    else:
    ...
    mem_map_res = re.findall('Linker script and memory map([\s\S]+?)OUTPUT\(', s)
    if (len(mem_map_res)) == 0:
    ...
    modules = list(set(item[0] for item in re.findall('0x\w+[\t ]+0x\w+[\t ]+.+?([\w\-\/]+\.[ao])(\(.+\.o\))?\n', mem_map)))
    ...
    for module in modules:
        ...
        map_lines.append('| %-40s | %-8d | %-8d | %-8d | %s |'%(module.split('/')[-1],ram_size, flash_size, psram_size, module))
