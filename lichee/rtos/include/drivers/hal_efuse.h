...
/* breif: hal_get_chipid: unify api to get chipid.
* In param: buffer: array to store the chipid. You must take note of no overflow.
*                   The buffer length MUST > 16Byte(128bit).
* return value: (>0) means the real lenth of read data;
*/
int hal_get_chipid(unsigned char *buffer);

/* breif: hal_get_chipid: unify api to get chipid.
* In param: buffer: array to store the chipid. You must take note of no overflow.
*                   The buffer length MUST > 8Byte(64bit).
* return value: (>0) means the real lenth of read data;
*/
