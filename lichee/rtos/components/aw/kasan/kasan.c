...
{
    uint32_t *shadow_addr = (uint32_t *)kasan_mem_to_shadow((void *)addr);
...
static __always_inline unsigned long bytes_is_zero(const uint8_t *start,
        size_t size)
{
    while (size)
    {
...
void kasan_free_pages(void *page, unsigned int npages)
{
    ...
    shadow_byte = *(int8_t *)kasan_mem_to_shadow(page);
...
void kasan_double_free_check(void *ptr)
{
    ...
    shadow_byte = *(int8_t *)kasan_mem_to_shadow(ptr);
    if (shadow_byte < 0 || shadow_byte >= KASAN_SHADOW_SCALE_SIZE)
    {
