 ...
 *
 * Porting the file for Allwinnerte RTOS by
 *        Zeng Zhijin <zhijinzeng@allwinnertech.com>
 *
...
typedef struct _kasan_lock
{
    freert_spinlock_t lock;
} kasan_lock_t;

static kasan_lock_t _kasan_lock;
...
static unsigned long kasan_lock(kasan_lock_t *lock)
{
    return freert_spin_lock_irqsave(&lock->lock);
}

static int kasan_lock_init(kasan_lock_t *lock)
{
    freert_spin_init(&lock->lock);
}

static void kasan_unlock(kasan_lock_t *lock, unsigned long flags)
{
    freert_spin_unlock_irqrestore(&lock->lock, flags);
}

static const void *find_first_bad_addr(const void *addr, size_t size)
{
    uint8_t shadow_val = *(uint8_t *)kasan_mem_to_shadow(addr);
    const void *first_bad_addr = addr;

    while (!shadow_val && first_bad_addr < addr + size)
    {
        first_bad_addr += KASAN_SHADOW_SCALE_SIZE;
        shadow_val = *(uint8_t *)kasan_mem_to_shadow(first_bad_addr);
    }
...
static const char *get_shadow_bug_type(struct kasan_access_info *info)
{
    ...
    shadow_addr = (uint8_t *)kasan_mem_to_shadow(info->first_bad_addr);
...
static void print_error_description(struct kasan_access_info *info)
{
    ...
    printf("BUG: KASAN: %s in %p\n",
           bug_type, (void *)info->ip);
    printf("%s of size %u at addr %p by task %s/%d\n",
...
static void kasan_end_report(unsigned long *flags)
{
    printf("==================================================================\n");
    kasan_enable_report();
    kasan_unlock(&_kasan_lock, *flags);
}
                              ...
                              void *ip)
{
    ...
    printf("BUG: KASAN: double-free or invalid-free in %p\n", ip);
    printf("\n");
