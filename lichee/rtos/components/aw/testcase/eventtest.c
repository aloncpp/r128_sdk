...
static freert_spinlock_t event_lock;
...
#define ebSET_BIT_TASK_PRIORITY		( tskIDLE_PRIORITY + 0 )
...
BaseType_t xError = pdFALSE;
	...
	for( ;; )
	{
		...
		***********************************************************************
		...
		cpu_sr = freert_spin_lock_irqsave(&event_lock);
		SMP_DBG("pass.\n");
		freert_spin_unlock_irqrestore(&event_lock, cpu_sr);
	}
...
TaskHandle_t xTestSlaveTaskHandle = ( TaskHandle_t ) pvParameters;
	...
	for( ;; )
	{
		...
		uint32_t cpusr;
		cpusr = freert_spin_lock_irqsave(&event_lock);
		SMP_DBG("pass.\n");
		freert_spin_unlock_irqrestore(&event_lock, cpusr);
	}
...
EventBits_t uxSynchronisationBit, uxReturned;
	...
	for( ;; )
	{
		...
		if(!(( uxReturned & ebALL_SYNC_BITS ) != ebALL_SYNC_BITS))
		{
			SMP_DBG("fatal error.0x%08x, 0x%08x.\n", uxReturned, ebALL_SYNC_BITS);
			soft_break();
		}
...
void cpu0_app_main(void)
{
	...
	vTaskDelete(NULL);
	return;
        xTaskCreateOnCore(prvTestSlaveTask, "WaitO", STACK_SIZE, NULL,ebWAIT_BIT_TASK_PRIORITY , &xTestSlaveTaskHandle, 1);
        xTaskCreateOnCore(prvTestMasterTask, "SetB", STACK_SIZE, ( void * ) xTestSlaveTaskHandle, ebSET_BIT_TASK_PRIORITY, NULL, 1);
        xTaskCreateOnCore(prvSyncTask, "Rndv", STACK_SIZE, ( void * ) ebRENDESVOUS_TASK_1_SYNC_BIT, ebWAIT_BIT_TASK_PRIORITY, &xSyncTask1, 1);
        xTaskCreateOnCore(prvSyncTask, "Rndv", STACK_SIZE, ( void * ) ebRENDESVOUS_TASK_2_SYNC_BIT, ebWAIT_BIT_TASK_PRIORITY, &xSyncTask2, 1);
