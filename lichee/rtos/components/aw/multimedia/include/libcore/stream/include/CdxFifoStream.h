...
struct CdxFifoStreamOpsS {
	...
	cdx_int32 (*reset_out)(CdxFifoStreamT *); /*set fifo.out = 0*/
    cdx_int32 (*cleareos)(CdxFifoStreamT *);   /* mean go looping */
};
...
static inline cdx_int32 CdxFifoStreamResetOut(struct CdxFifoStreamS *stream)
{
	CDX_CHECK(stream);
	CDX_CHECK(stream->ops);
	CDX_CHECK(stream->ops->reset_out);
	return stream->ops->reset_out(stream);
}

static inline cdx_int32 CdxFifoStreamCleareos(struct CdxFifoStreamS *stream)
{
    CDX_CHECK(stream);
    CDX_CHECK(stream->ops);
    CDX_CHECK(stream->ops->seteos);
    return stream->ops->cleareos(stream);
}
#ifdef __cplusplus
}
#endif
