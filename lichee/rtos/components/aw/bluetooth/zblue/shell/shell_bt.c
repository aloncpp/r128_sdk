...
static void cmd_ble_exec(int argc,char **argv)
{
	static int i = 0;
	const int cmd_size = sizeof(shell_bt_cmds) / sizeof(struct shell_static_entry) - 1;
	...
	if (argc < 2)
	{
		...
		{
			printf("%s\n",shell_bt_cmds[i].syntax);
		}
		return;
	}
	for (i = 0; i < cmd_size; i++) {
		if (!strcmp(argv[1], shell_bt_cmds[i].syntax)) {
			if ((argc-1 >= shell_bt_cmds[i].args.mandatory) &&
				(argc-1 <= shell_bt_cmds[i].args.mandatory +
				shell_bt_cmds[i].args.optional)) {
				shell_bt_cmds[i].handler(_shell, argc-1, argv+1);
				return;
			}else {
				printf("usage: %s\n",shell_bt_cmds[i].help);
				return;
			}
		}
	}
...
FINSH_FUNCTION_EXPORT_CMD(cmd_ble_exec, ble, Console ble Command);
