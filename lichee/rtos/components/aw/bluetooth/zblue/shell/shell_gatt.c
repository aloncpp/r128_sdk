				  ...
				  size_t argc, char *argv[])
{
	...
	shell_print(shell, "register gatt service 0");
	bt_gatt_service_register(&vnd1_svc);
	shell_print(shell, "register gatt service 1");
				    ...
				    size_t argc, char *argv[])
{
	...
	shell_print(shell, "unregister gatt service 0");
	bt_gatt_service_unregister(&vnd1_svc);
	shell_print(shell, "unregister gatt service 1");
...
static void cmd_gatt_exec(int argc,char **argv)
{
	static int i = 0;
	const int cmd_size = sizeof(shell_gatt_cmds) / sizeof(struct shell_static_entry) - 1;
	const struct shell *_shell;
	if (argc < 2) {
		printf("usage:\n");
		printf("gatt + \n");
		for (i = 0; i < cmd_size; i++)
		{
			printf("%s\n",shell_gatt_cmds[i].syntax);
		}
		return;
	}
	for (i = 0; i < cmd_size; i++) {
		if (!strcmp(argv[1], shell_gatt_cmds[i].syntax)) {
			if ((argc-1 >= shell_gatt_cmds[i].args.mandatory) &&
				(argc-1 <= shell_gatt_cmds[i].args.mandatory +
				shell_gatt_cmds[i].args.optional)) {
				shell_gatt_cmds[i].handler(_shell, argc-1, argv+1);
				return;
			}else {
				printf("usage: %s\n",shell_gatt_cmds[i].help);
				return;
			}
		}
	}
...
FINSH_FUNCTION_EXPORT_CMD(cmd_gatt_exec, gatt, Console gatt Command);