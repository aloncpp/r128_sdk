...
static int scan_recv_num;
static uint32_t scan_recv_num_print_time;
#endif
		      ...
		      struct net_buf_simple *buf)
{
	...
	if (scan_silent == 1) {
		...
		uint32_t delta = k_uptime_ticks() - scan_recv_num_print_time;
		if (delta > (5000)) {
			shell_print(ctx_shell, "scan recv %d pkt/s",
						scan_recv_num / (delta / 1000));
			scan_recv_num_print_time = k_uptime_ticks();
			scan_recv_num = 0;
		}
		return;
	}
...
static int cmd_init(const struct shell *shell, size_t argc, char *argv[])
{
	...
	if (initialized) {
		shell_print(shell, "Bluetooth init already");
		return 0;
	}
	initialized = 1;
	...
	err = bt_ctrl_enable();
	if (err) {
		shell_error(shell, "Bluetooth Controller init failed (err %d)", err);
		return err;
	}
...
static int cmd_deinit(const struct shell *shell, size_t argc, char *argv[])
{
	...
	if (!initialized) {
		shell_print(shell, "Bluetooth not init");
		return 0;
	}
	initialized = 0;
	...
	/* unregister should be executed after bt_disable() or before?
	   it may be done in bt_disable callback, when bt_disable exist callback param.
	   TODO...
	*/
#if defined(CONFIG_BT_CONN)
	default_conn = NULL;
	...
	if (err) {
		...
		shell_print(ctx_shell, "Bluetooth deinitialized");
	}
	bt_zephyr_adapter_unregister();

	err = bt_ctrl_disable();
	if (err) {
		shell_error(shell, "Bluetooth controller deinit failed (err %d)", err);
	} else {
		shell_print(ctx_shell, "Bluetooth controller deinitialized");
	}
...
static int cmd_id_show(const struct shell *shell, size_t argc, char *argv[])
{
	...
	for (i = 0; i < count; i++) {
		...
		shell_print(shell, "%s%u: %s", i == selected_id ? "*" : " ", i,
		      addr_str);
	}
...
static int cmd_scan(const struct shell *shell, size_t argc, char *argv[])
{
	...
	while (++i < argc) {
		...
		} else if (!cmd_strcmp(argv[i], "coded")) {
			param.options |= BT_LE_SCAN_OPT_CODED;
		} else if (!cmd_strcmp(argv[i], "no-1m")) {
		...
		} else if (!cmd_strcmp(argv[i], "hex")) {
			//scan_cb = cmd_scan_cb_hex;
		} else if (!cmd_strcmp(argv[i], "silent")) {
			scan_silent = 1;
		} else if (!cmd_strcmp(argv[i], "pps")) {
			scan_silent = 2;
	...
	err = bt_le_scan_start(&param, NULL);
	if (err) {
		shell_error(shell, "Bluetooth set scan failed "
			    "(err %d)", err);
		return err;
	} else {
		shell_print(shell, "Bluetooth scan enabled");
	}
...
static int cmd_advertise(const struct shell *shell, size_t argc, char *argv[])
{
	...
	while (++i < argc) {
		if (!cmd_strcmp(argv[i], "no-name")) {
			param.options &=~ BT_LE_ADV_OPT_USE_NAME;
		} else if (!cmd_strcmp(argv[i], "nconn")) {
			param.options &=~ BT_LE_ADV_OPT_CONNECTABLE;
		} else if (!cmd_strcmp(argv[i], "1shot")
		           || !cmd_strcmp(argv[i], "one-time")) {
			param.options |= BT_LE_ADV_OPT_ONE_TIME;
//		} else if (!cmd_strcmp(argv[i], "id")) {
//			param.options |= BT_LE_ADV_OPT_USE_IDENTITY;
		} else if (!cmd_strcmp(argv[i], "dir-low")) {
			//param.options |= BT_LE_ADV_OPT_DIR_MODE_LOW_DUTY;
		} else if (!cmd_strcmp(argv[i], "dir-rpa")) {
			//param.options |= BT_LE_ADV_OPT_DIR_ADDR_RPA;
		} else if (!cmd_strncmp(argv[i], "min=0x", 4)) {
			uint32_t num;
			int cnt = cmd_sscanf(argv[i] + 4, "%x", &num);
			if (cnt != 1) {
				CMD_ERR("invalid param %s\n", argv[i] + 4);
			...
			param.interval_min = (uint16_t)(num);
			CMD_DBG("adv interval_min 0x%x\n", param.interval_min);
		} else if (!cmd_strncmp(argv[i], "max=0x", 4)) {
			uint32_t num;
			int cnt = cmd_sscanf(argv[i] + 4, "%x", &num);
			if (cnt != 1) {
				CMD_ERR("invalid param %s\n", argv[i] + 4);
			...
			param.interval_max = (uint16_t)(num);
			CMD_DBG("adv interval_max 0x%x\n", param.interval_max);
		} else if (!cmd_strcmp(argv[i], "discov")) {
			/* Default */
		} else if (!cmd_strcmp(argv[i], "non_discov")) {
			ad = NULL;
			ad_len = 0;;
		} else if (!strcmp(argv[i], "wl")) {
			param.options |= BT_LE_ADV_OPT_FILTER_SCAN_REQ;
			param.options |= BT_LE_ADV_OPT_FILTER_CONN;
		} else if (!strcmp(argv[i], "wl-scan")) {
			param.options |= BT_LE_ADV_OPT_FILTER_SCAN_REQ;
		} else if (!strcmp(argv[i], "wl-conn")) {
			param.options |= BT_LE_ADV_OPT_FILTER_CONN;
		} else if (!strcmp(argv[i], "identity")) {
			param.options |= BT_LE_ADV_OPT_USE_IDENTITY;
		} else if (!strcmp(argv[i], "disable-37")) {
			param.options |= BT_LE_ADV_OPT_DISABLE_CHAN_37;
		} else if (!strcmp(argv[i], "disable-38")) {
			param.options |= BT_LE_ADV_OPT_DISABLE_CHAN_38;
		} else if (!strcmp(argv[i], "disable-39")) {
			param.options |= BT_LE_ADV_OPT_DISABLE_CHAN_39;
		} else {
			CMD_ERR("invalid param %s\n", argv[i]);
			return ENOEXEC;
...
static int cmd_connect_le(const struct shell *shell, size_t argc, char *argv[])
{
	...
	while (++i < argc) {
		if (!cmd_strncmp(argv[i], "lat=", 4)) {
			...
			conn_param.latency = (uint16_t)(num);
			...
			conn_param.timeout = (uint16_t)(num);
			...
			conn_param.interval_min = (uint16_t)(num);
			...
			conn_param.interval_max = (uint16_t)(num);
...
static int cmd_res_info(const struct shell *shell, size_t argc, char *argv[])
{
	extern void ble_resource_info(void);
	ble_resource_info();
...
static int cmd_addr_show(const struct shell *shell, size_t argc, char *argv[])
{
	bt_addr_le_t addr;

	bt_get_rand_addr(&addr);
	print_le_addr("Local on-air", &addr);
	return 0;
}
...
SHELL_STATIC_SUBCMD_SET_CREATE(bt_cmds,
	SHELL_CMD_ARG(init, NULL, "[no-settings-load], [sync]",
		      cmd_init, 1, 2),
#if defined(CONFIG_BT_DEINIT)
	SHELL_CMD_ARG(deinit, NULL, HELP_NONE, cmd_deinit, 1, 0),
#endif
...
#endif
	SHELL_CMD_ARG(id-create, NULL, "[addr: XX:XX:XX:XX:XX:XX]", cmd_id_create, 1, 1),
	SHELL_CMD_ARG(id-reset, NULL, "<id> [addr: XX:XX:XX:XX:XX:XX]", cmd_id_reset, 2, 1),
	...
	SHELL_CMD_ARG(scan, NULL,
		      "<value: on, passive, off> [dup: dups, nodups] [wl] [int=0x<hex>] [win=0x<hex>]"
		      EXT_ADV_SCAN_OPT,
		      cmd_scan, 2, 4),
#endif /* CONFIG_BT_OBSERVER */
#if defined(CONFIG_BT_BROADCASTER)
	SHELL_CMD_ARG(advertise, NULL,
		      ...
		      "[1shot/one-time] [disable-37] [disable-38] [disable-39] [nconn] [min=0x<hex>] [max=0x<hex>]",
		      cmd_advertise, 2, 8),
#if defined(CONFIG_BT_PERIPHERAL)
	...
	SHELL_CMD_ARG(connect, NULL, HELP_ADDR_LE EXT_ADV_SCAN_OPT
			  " [lat=0x<hex>] [to=0x<hex>] [min=0x<hex>] [max=0x<hex>]",
		      cmd_connect_le, 3, 6),
#if !defined(CONFIG_BT_WHITELIST)
	SHELL_CMD_ARG(auto-conn, NULL, HELP_ADDR_LE " [value:on,off]", cmd_auto_conn, 3, 1),
#endif /* !defined(CONFIG_BT_WHITELIST) */
#endif /* CONFIG_BT_CENTRAL */
...
#if defined(CONFIG_BT_DEBUG_RESOURCES_USE)
	SHELL_CMD_ARG(res_info, NULL, HELP_NONE, cmd_res_info, 1, 0),
#endif
	SHELL_CMD_ARG(addr-show, NULL, HELP_NONE, cmd_addr_show, 1, 0),
...
enum cmd_status cmd_ble_exec(char *cmd)
{
	return cmd_exec_shell(cmd, shell_bt_cmds, cmd_nitems(shell_bt_cmds) - 1);
}
