...
#if (CMD_GATT_VALUE_PRINT)
    if (length) {
        shell_print(ctx_shell, "Read Value:");
        for (i = 0; i < length; i++) {
...
static int cmd_gatt_write(const struct shell *shell, size_t argc, char *argv[])
{
	int err;
...
#if (CMD_GATT_WRITE_DYNAMIC)
	if (argv[4] && !cmd_strcmp(argv[4], "string")) {
		size_t len;
		len = MIN(strlen(argv[3]), sizeof(gatt_write_buf));
		strncpy((char *)gatt_write_buf, argv[3], len);
...
#if CMD_GATT_INDICATION
    if (echo_enabled & 0x2) {
        shell_print(ctx_shell, "Indicate Echo attr len %u", len);
...
static void indicate_cb(struct bt_conn *conn, struct bt_gatt_indicate_params *params,
			uint8_t err)
{
	printk("Indication sent to conn %p\n", conn);
}

static int cmd_indicate(const struct shell *shell, size_t argc, char *argv[])
{
	uint8_t data = 0;

    if (!(echo_enabled & 0x2)) {
        shell_error(shell, "Indication not enabled");
        return -ENOEXEC;
    }
    ...
    indicate_params.uuid = &vnd1_echo_uuid.uuid;
    indicate_params.attr = vnd1_attrs;
    indicate_params.data = &data;
    indicate_params.len = sizeof(data);
...
#if (CMD_GATT_WRITE_DYNAMIC)
    SHELL_CMD_ARG(write, NULL, "<handle> <offset> <data> [string]",
		      cmd_gatt_write, 4, 1),
#else
...
#if CMD_GATT_INDICATION
    SHELL_CMD_ARG(indicate, NULL, "[data]", cmd_indicate, 1, 1),
#endif
#endif /* CONFIG_BT_GATT_DYNAMIC_DB */
...
enum cmd_status cmd_gatt_exec(char *cmd)
{
  return cmd_exec_shell(cmd, shell_gatt_cmds, cmd_nitems(shell_gatt_cmds) - 1);
}
