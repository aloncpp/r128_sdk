...
typedef struct {
    ...
    char base_path[VFS_PATH_MAX];   /* base path in VFS where partition is registered */
    size_t max_files;   /* max number of simultaneously open files; size of files[] array */
    hal_mutex_t lock;       /* guard for access to this structure */
...
static int vfs_fat_statfs(void* ctx, const char * path, struct statfs * buf);
...
int vfs_fat_register(const char* base_path, const char* fat_drive, size_t max_files, FATFS** out_fs)
{
    ...
    int err = vfs_register(base_path, &vfs, fat_ctx);
    if (err != AW_OK) {
...
int vfs_fat_unregister_path(const char* base_path)
{
    ...
    int err = vfs_unregister(fat_ctx->base_path);
    if (err != AW_OK) {
        return err;
    }
    hal_mutex_delete(fat_ctx->lock);
    free(fat_ctx->o_append);
    free(fat_ctx);
    s_fat_ctxs[ctx] = NULL;
    return AW_OK;
...
static int vfs_fat_open(void* ctx, const char * path, int flags, int mode)
{
    pr_debug(TAG, "%s: path=\"%s\", flags=%x, mode=%x", __func__, path, flags, mode);
    vfs_fat_ctx_t* fat_ctx = (vfs_fat_ctx_t*) ctx;
    hal_mutex_lock(fat_ctx->lock);
    prepend_drive_to_path(fat_ctx, &path, NULL);
    int fd = get_next_fd(fat_ctx);
    if (fd < 0) {
        hal_mutex_unlock(fat_ctx->lock);
        pr_err(TAG, "open: no free file descriptors");
        errno = ENFILE;
        return -1;
    }
    ...
    if (res != FR_OK) {
        ...
        hal_mutex_unlock(fat_ctx->lock);
        pr_debug(TAG, "%s: fresult=%d", __func__, res);
    ...
    if(!(fat_mode_conv(flags) & (FA_WRITE))) {
        ...
        if (clmt_mem == NULL) {
            ...
            file_cleanup(fat_ctx, fd);
            hal_mutex_unlock(fat_ctx->lock);
            pr_err(TAG, "open: Failed to pre-allocate CLMT buffer for fast-seek");
            errno = ENOMEM;
            return -1;
        }
        ...
        pr_debug(TAG, "%s: fast-seek has: %s",
                __func__,
...
static ssize_t vfs_fat_pread(void *ctx, int fd, void *dst, size_t size, off_t offset)
{
    ...
    if (f_res != FR_OK) {
        pr_debug(TAG, "%s: fresult=%d", __func__, f_res);
...
static off_t vfs_fat_lseek(void* ctx, int fd, off_t offset, int mode)
{
    ...
    pr_debug(TAG, "%s: offset=%ld, filesize:=%d", __func__, new_pos, f_size(file));
...
static int vfs_fat_statfs(void* ctx, const char * path, struct statfs * buf)
{
    ...
    buf->f_bfree = (__fsblkcnt_t)fre_sect;
    buf->f_bavail = (__fsblkcnt_t)fre_sect;
    buf->f_blocks = (__fsblkcnt_t)tot_sect;
#if _MAX_SS != _MIN_SS
    buf->f_bsize = f->ssize;
#else
    buf->f_bsize = 512;
...
static int vfs_fat_link(void* ctx, const char* n1, const char* n2)
{
    ...
    if (buf == NULL || pf1 == NULL || pf2 == NULL) {
        hal_mutex_unlock(fat_ctx->lock);
        pr_debug(TAG, "alloc failed, pf1=%p, pf2=%p, buf=%p", pf1, pf2, buf);
        free(pf1);
        free(pf2);
        free(buf);
        errno = ENOMEM;
    ...
    while (size_left > 0) {
        ...
        size_t read;
        res = f_read(pf1, buf, will_copy, (void *)&read);
        if (res != FR_OK) {
            goto fail3;
        ...
        size_t written;
        res = f_write(pf2, buf, will_copy, (void *)&written);
        if (res != FR_OK) {
            goto fail3;
...
static void vfs_fat_seekdir(void* ctx, DIR* pdir, long offset)
{
    ...
    if (offset < fat_dir->offset) {
        ...
        if (res != FR_OK) {
            pr_debug(TAG, "%s: rewinddir fresult=%d", __func__, res);
            errno = fresult_to_errno(res);
            return;
        }
        fat_dir->offset = 0;
    ...
    while (fat_dir->offset < offset) {
        ...
        if (res != FR_OK) {
            pr_debug(TAG, "%s: f_readdir fresult=%d", __func__, res);
            errno = fresult_to_errno(res);
            return;
        }
        fat_dir->offset++;
...
static int vfs_fat_truncate(void* ctx, const char *path, off_t length)
{
    ...
    if (file == NULL) {
        hal_mutex_unlock(fat_ctx->lock);
        pr_debug(TAG, "truncate alloc failed");
        errno = ENOMEM;
        ret = -1;
        goto out;
    }
    ...
    if (sz < length) {
        hal_mutex_unlock(fat_ctx->lock);
        pr_debug(TAG, "truncate does not support extending size");
        errno = EPERM;
        ret = -1;
        goto close;
    }
...
close:
    ...
    if (res != FR_OK) {
        pr_err(TAG, "closing file opened for truncate failed");
