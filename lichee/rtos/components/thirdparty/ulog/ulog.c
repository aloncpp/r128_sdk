...
#ifdef CONFIG_ULOG_OUTPUT_FLOAT
#include <stdio.h>
#endif

#ifdef CONFIG_ULOG_TIME_USING_TIMESTAMP
#include <sys/time.h>
#endif
...
#endif /* CONFIG_ULOG_USING_FILTER */
};
...
static char *get_log_buf(void)
{
    ...
    if (rt_interrupt_get_nest() == 0)
    {
        memset(ulog.log_buf_th, 0, sizeof(ulog.log_buf_th));
        return ulog.log_buf_th;
    }
    else
    {
#ifdef CONFIG_ULOG_USING_ISR_LOG
        memset(ulog.log_buf_isr, 0, sizeof(ulog.log_buf_isr));
        return ulog.log_buf_isr;
#else
        rt_kprintf("Error: Current mode not supported run in ISR. Please enable CONFIG_ULOG_USING_ISR_LOG.\n");
        return NULL;
#endif
    }
}
...
#endif /* CONFIG_ULOG_TIME_USING_TIMESTAMP */

        log_len += rt_strlen(log_buf + log_len);
    }
#endif /* CONFIG_ULOG_OUTPUT_TIME */

#ifdef CONFIG_ULOG_OUTPUT_LEVEL
...
#ifdef CONFIG_ULOG_OUTPUT_TAG

#if !defined(CONFIG_ULOG_OUTPUT_LEVEL) && defined(CONFIG_ULOG_OUTPUT_TIME)
    log_len += ulog_strcpy(log_len, log_buf + log_len, " ");
#endif
...
#ifdef CONFIG_ULOG_OUTPUT_THREAD_NAME
    /* add thread info */
    {

#if defined(CONFIG_ULOG_OUTPUT_TIME) || defined(CONFIG_ULOG_OUTPUT_LEVEL) || defined(CONFIG_ULOG_OUTPUT_TAG)
        log_len += ulog_strcpy(log_len, log_buf + log_len, " ");
#endif
        ...
        if (rt_interrupt_get_nest() == 0)
        {
            rt_size_t name_len = rt_strnlen(pcTaskGetName(xTaskGetCurrentTaskHandle()), RT_NAME_MAX);

            rt_strncpy(log_buf + log_len, pcTaskGetName(xTaskGetCurrentTaskHandle()), name_len);
            log_len += name_len;
        }
        else
        {
...
#if !defined(CONFIG_ULOG_USING_COLOR) || defined(CONFIG_ULOG_USING_SYSLOG)
...
#endif /* !defined(CONFIG_ULOG_USING_COLOR) || defined(CONFIG_ULOG_USING_SYSLOG) */
    }
}
...
static void do_output(rt_uint32_t level, const char *tag, rt_bool_t is_raw, const char *log_buf, rt_size_t log_len)
{
    ...
    /* allocate log frame */
    log_blk = rt_rbb_blk_alloc(ulog.async_rbb, RT_ALIGN(sizeof(struct ulog_frame) + log_len + 1, RT_ALIGN_SIZE));
    if (log_blk)
    {
        memset(log_blk->buf, 0, RT_ALIGN(sizeof(struct ulog_frame) + log_len + 1, RT_ALIGN_SIZE));
...
#endif /* CONFIG_ULOG_USING_ASYNC_OUTPUT */
}
...
#ifndef CONFIG_ULOG_USING_SYSLOG
    RT_ASSERT(level <= LOG_LVL_DBG);
#else
    RT_ASSERT(LOG_PRI(level) <= LOG_DEBUG);
#endif /* CONFIG_ULOG_USING_SYSLOG */
...
int ulog_init(void)
{
    ...
    ulog.async_notice = xSemaphoreCreateCounting(10, 0);
    /* async output thread */
    ret = xTaskCreate(async_output_thread_entry, (signed portCHAR *) "ulog_async", ULOG_ASYNC_OUTPUT_THREAD_STACK, NULL, configAPPLICATION_NORMAL_PRIORITY, &(ulog.async_th));

    if (ulog.async_th == NULL)
    {
        rt_kprintf("Error: ulog init failed! No memory for async output thread.\n");
        vSemaphoreDelete(ulog.output_locker);
        vSemaphoreDelete(ulog.async_notice);
