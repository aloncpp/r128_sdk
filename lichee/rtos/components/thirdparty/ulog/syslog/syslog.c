...
void openlog(const char *ident, int option, int facility)
{
    ...
    {
        rt_strncpy(local_ident, "tina-rt", ULOG_SYSLOG_IDENT_MAX_LEN);
    }
...
void vsyslog(int priority, const char *format, va_list args)
{
    ...
    ulog_voutput(priority, local_ident, ULOG_NEWLINE_FLAG, format, args);
}
...
RT_WEAK rt_size_t syslog_formater(char *log_buf, int level, const char *tag, rt_bool_t newline, const char *format, va_list args)
{
    ...
    /* add thread info */
    {
        ...
        {
            log_len += ulog_strcpy(log_len, log_buf + log_len, pcTaskGetName(xTaskGetCurrentTaskHandle()));
        }
        else
        {
...
#if defined(CONFIG_ULOG_TIME_USING_TIMESTAMP) \
	|| defined(CONFIG_ULOG_OUTPUT_TAG) \
	|| defined(CONFIG_ULOG_OUTPUT_THREAD_NAME)
    log_len += ulog_strcpy(log_len, log_buf + log_len, ": ");
#endif
