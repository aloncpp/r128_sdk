...
#if defined(CONFIG_COMPONENT_VFS) && defined(CONFIG_USING_WORKDIR)
...
static int finsh_getchar(void)
{
    ...
    {
        hal_uart_receive_polling(CONFIG_CLI_UART_PORT, (uint8_t *)&buf, 1);
        return buf;
    }
    else
    {
...
static void finsh_wait_auth(void)
{
    ...
    while (1)
    {
#ifdef FINSH_ALLOW_SKIP_AUTH
        extern int finsh_is_auth_required(void);
        if(!finsh_is_auth_required()){
            rt_kprintf("skip auth...\n");
            usleep(500 * 1000);
            return;
        }
#endif
...
extern int cli_console_init(cli_console * console);
void finsh_thread_entry(void *parameter)
{
    ...
    cli_task_set_console(cli_current_task(), (cli_console *)parameter);
#endif
...
int finsh_system_init(void)
{
    ...
    tid = xTaskCreate(finsh_thread_entry, FINSH_THREAD_NAME, FINSH_THREAD_STACK_SIZE, NULL, FINSH_THREAD_PRIORITY, &xHandle);
#else
    shell = &_shell;
    tid = &finsh_thread;
...
//     esKRNL_SemCreate(1);
//    xSemaphoreCreateCountingStatic( SEM_VALUE_MAX, 0, &(shell->rx_sem));
