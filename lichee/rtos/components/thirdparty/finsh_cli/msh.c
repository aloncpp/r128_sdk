...
FINSH_FUNCTION_EXPORT_ALIAS(msh_exit, __cmd_exit, return to shell mode.);
...
int msh_help(int argc, char **argv)
{
    rt_kprintf("FreeRTOS shell commands:\n");
    {
        ...
        for (index = (struct finsh_syscall *)&_syscall_table_begin;
                (uint32_t)index < (uint32_t)&_syscall_table_end;
...
FINSH_FUNCTION_EXPORT_ALIAS(msh_help, __cmd_help, finsh_cli help);
...
void msh_auto_complete(char *prefix)
{
    ...
    {
        for (index = (struct finsh_syscall *)&_syscall_table_begin; (uint32_t)index < (uint32_t)&_syscall_table_end; FINSH_NEXT_SYSCALL(index))
        {
