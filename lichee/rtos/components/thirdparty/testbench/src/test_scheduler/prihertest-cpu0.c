...
void cpu0_app_main(void)
{
	...
	ret = xTaskCreateOnCore( cpu0_task_A, "task_A", 1024, "low",configTIMER_TASK_PRIORITY - 5 , NULL, 0);
	if(ret != pdPASS)
	{
	...
	ret = xTaskCreateOnCore( cpu0_task_B, "task_B", 1024, "mid",configTIMER_TASK_PRIORITY -2 , NULL, 0);
	if(ret != pdPASS)
	{
		SMP_DBG("Create Rx Task Failure.\n");
		soft_break();
	...
	ret = xTaskCreateOnCore( cpu0_task_C, "task_C", 1024, "hih",configTIMER_TASK_PRIORITY , NULL, 0);
	if(ret != pdPASS)
	{
		SMP_DBG("Create Rx Task Failure.\n");
		soft_break();
