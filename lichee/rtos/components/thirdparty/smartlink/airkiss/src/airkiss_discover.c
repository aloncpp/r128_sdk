...
static void airkiss_lan_discover_task(void *arg)
{
	...
	char *buffer;
	int times;
	int time_next = XR_OS_JiffiesToMSecs(XR_OS_GetJiffies()) +
	...
	while (!(priv->lan_thread_run & AK_TASK_STOP)) {
		...
		times = XR_OS_JiffiesToMSecs(XR_OS_GetJiffies());
		if (XR_OS_TimeAfterEqual(times, time_next)) {
			time_next = XR_OS_JiffiesToMSecs(XR_OS_GetJiffies()) +
				priv->ack_info.ack_period_ms;
...
malloc_faild:
	...
	XR_OS_ThreadResumeScheduler();
	XR_OS_ThreadDelete(&priv->lan_thread);
}
...
int wlan_airkiss_lan_discover_start(char *app_id, char *dev_id, uint32_t period_ms)
{
	...
	if (XR_OS_ThreadCreate(&priv->lan_thread,
	                    "ak_lan",
	                    airkiss_lan_discover_task,
	                    (void *)priv,
	                    XR_OS_THREAD_PRIO_APP,
	                    AIRKISS_ONLINE_DIALOG_THREAD_STACK_SIZE) != XR_OS_OK) {
