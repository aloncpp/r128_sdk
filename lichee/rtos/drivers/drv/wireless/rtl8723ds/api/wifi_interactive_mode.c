...
int cmd_wifi_connect(int argc, char **argv)
{
	...
	printf("\r\n\nGot IP after %dms.\n", (tick3-tick1) * 1000 / CONFIG_HZ);
    return ret;
}

FINSH_FUNCTION_EXPORT_CMD(cmd_wifi_connect, wc, Console WifiConnect Command);

int cmd_dhcp_start(int argc,char **argv)
{
	struct netif *net_if = NULL;
	net_if = get_netif(MODE_STA);
	if(net_if == NULL) {
	...
	return dhcp_start(net_if);
}

FINSH_FUNCTION_EXPORT_CMD(cmd_dhcp_start, dhcp, Console dhcp_start Command);
...
FINSH_FUNCTION_EXPORT_CMD(cmd_wifi_disconnect, wr, Console WifiDisconnect Command);
...
FINSH_FUNCTION_EXPORT_CMD(cmd_wifi_info, wfinfo, Console WifiInfo Command);
...
FINSH_FUNCTION_EXPORT_CMD(cmd_wifi_on, wo, Console WifiOn Command);
...
FINSH_FUNCTION_EXPORT_CMD(cmd_wifi_off, wf, Console WifiOff Command);

void cmd_wifi_long_time_test(int argc,char **argv)
{
	int ret;
	int i = 0;
	while(i <100) {
		cmd_wifi_on(0,NULL);
		sleep(1);
		ret = cmd_wifi_connect(argc,argv);
		if(ret != RTW_SUCCESS) {
			printf("\n====== test end,test times: %d\n",i);
			break;
		}
		sleep(1);
...
FINSH_FUNCTION_EXPORT_CMD(cmd_wifi_long_time_test, wl, Console Wifi long time test Command);
...
FINSH_FUNCTION_EXPORT_CMD(cmd_wifi_scan, ws, Console WifiScan Command);
...
static void cmd_wifi_iwpriv(int argc, char **argv)
{
	...
	if(argv[1]) {
		...
		wext_private_command(WLAN0_NAME, str, 1);
	}
	else {
...
FINSH_FUNCTION_EXPORT_CMD(cmd_wifi_iwpriv, iwpriv, Console Wifi iwpriv Command);
...
FINSH_FUNCTION_EXPORT_CMD(cmd_debug, wd, Console WifiDebug Command);
