...
static valid_address_t valid_addr[] = {
	{ .start = 0x40000000, .end = 0x41000000 },
};
#elif defined(CONFIG_ARCH_SUN8IW20P1)
static valid_address_t valid_addr[] = {
	{ .start = 0x40000000, .end = 0x41000000 },
};
#else
...
static void *find_task(char *name, TaskStatus_t *get_temp)
{
	...
	uint32_t cpsr_flag;
#if defined(CONFIG_ARCH_SUN8IW18P1) || defined(CONFIG_ARCH_SUN8IW20P1)
	...
	for (x = 0; x < uxArraySize; x++) {
		...
		if (strcmp(name, temp->pcTaskName) && (xTaskNumber != temp->xTaskNumber)) {
			...
			memcpy(get_temp, temp, sizeof(*temp));
			ret = get_temp;
			break;
		}
	}
...
static int check_task_is_running(void *task)
{
	...
	if (thread == xTaskGetCurrentTaskHandle() && uGetInterruptNest() == 0) {
		return TASK_RUNNING;
...
static int thumb_find_jump_offset(char *LR)
{
	char *LR_fixed;
...
static void get_register_from_task_stack(void *context, char **PC, char **LR,
					 int **SP,print_function print_func)
{
	...
	backtrace_debug("context addr = 0x%8x\r\n",(uint32_t)context);
	backtrace_debug("context size = %d\r\n",sizeof(switch_ctx_regs_t));
	backtrace_debug("context->psplim = 0x%8x\r\n",task_ctx->psplim);
	backtrace_debug("context->lr = 0x%8x\r\n",task_ctx->lr);
	backtrace_debug("context->retAddr = 0x%8x\r\n",task_ctx->retAddr);
	backtrace_debug("context->xPSR = 0x%8x\r\n",task_ctx->xPSR);
	*SP = (int *)((unsigned long)(context) + sizeof(switch_ctx_regs_t));
	*LR = 0;
	offest = thumb_find_jump_offset((char *)task_ctx->lr);
	if(offest < 0) {
		*PC = (char *)0xffffffff;
		backtrace_debug(print_func, "find lr offest failed\r\n");
	}else if(offest == 0){
		*PC = (char *)0;
	}else
		*PC = (char *)((long)task_ctx->lr - offest);
#else
	*SP = (int *)((unsigned long)(context) + sizeof(switch_ctx_regs_t) +
	      4 * 2);
	*LR = 0;
	*PC = (char *)((long)task_ctx->r14 - 4 * 3);
#endif
...
static int find_lr_offset(char *LR, print_function print_func, bool *state)
{
	...
	if (LR_fixed == PC2ADDR(&_interrupt_return_address)) {
		long2str((long)&_interrupt_return_address,
			 &backtrace_output_buf[14]);
	...
	/* meet rt_thread_exit, it is task exit function */
	if (LR_fixed == PC2ADDR(get_task_stack_bottom())) {
		long2str((long)get_task_stack_bottom(),
			 &backtrace_output_buf[14]);
		PRINT_CALL(print_func, backtrace_output_buf);
		return 0;
	}
...
static int thumb_get_push_ins_framesize(unsigned int inst, unsigned char *jump,
					int thumb32bit)
{
	int framesize = -1;
	if (thumb32bit) {
				    ...
				    print_function print_func)
{
	...
	backtrace_debug(
		"i = %d, parse_addr = %p, PC = %p, offset = %d, framesize = %d\n",
	...
	*pPC = LR - offset;
	...
	backtrace_debug(
		"*pSP = %p, offset = %d, *pPC = %p, framesize = %d, state = %d\n",
...
int thumb_get_next_inst(print_function print_func, int *error, int *offset,
			char *ins16_h_addr, char *ins16_l_addr, int lsb,
			int *thumb32bit)
{
				      ...
				      print_function print_func)
{
	...
	for (i = 2; i < BT_SCAN_MAX_LIMIT; i += 2) {
		...
		int error = 0;
		int thumb32bit = 0;
		ins32 = thumb_get_next_inst(print_func, &error, &i, parse_addr,
					    parse_addr - 2, 0, &thumb32bit);
		if (error) {
			return -1;
		}
	...
	for (i = 2; parse_addr + i <= PC; i += 2) {
		int error = 0;
		int thumb32bit = 0;
		ins32 = thumb_get_next_inst(print_func, &error, &i,
					    parse_addr + i + 2, parse_addr + i,
					    1, &thumb32bit);
		if (error) {
			return -1;
		}
...
static int thumb_backtrace_stack_pop(unsigned int inst,
				     unsigned char *sp_change, int thumb32bit)
{
	int framesize = -1;
	if (!thumb32bit) {
				   ...
				   print_function print_func)
{
	...
	for (i = 2; i < BT_SCAN_MAX_LIMIT; i += 2) {
		...
		int error = 0;
		int thumb32bit = 0;
		ins32 = thumb_get_next_inst(print_func, &error, &i, PC + i + 2,
					    PC + i, 1, &thumb32bit);
		if (error) {
			return -1;
		}
	...
	if (sp_change) {
		for (i = 2; i < BT_SCAN_MAX_LIMIT; i += 2) {
			...
			int thumb32bit = 0;
			ins32 = thumb_get_next_inst(print_func, &error, &i,
						    parse_addr, parse_addr - 2,
						    0, &thumb32bit);
			if (error) {
				return -1;
			}
			temp = thumb_get_push_ins_framesize(ins32, &jump,
							    thumb32bit);
			if (temp >= 0) {
				framesize += temp;
			}
 ...
 */
static int _backtrace(char *taskname, void *output[], int size, int offset,
		      print_function print_func, unsigned long arg_cpsr,
		      unsigned long arg_sp, unsigned long arg_pc,
		      unsigned long arg_lr, unsigned long exception_mode)
{
	char *PC = NULL;
	int *SP = NULL;
	char *saved_pc;
	...
	if (taskname) {
		task = find_task(taskname, &status);
		if (task == NULL) {
			PRINT_CALL(print_func, "Task not found : %s\r\n",
				   taskname);
			return 0;
		}
		get_register_from_task_stack((void *)task->pxTopOfStack, &PC,
					     &LR, &SP,print_func);
	...
	if ((check_self == 0 || exception_mode == 1) && level == 1) {
		...
		if (ret == 0) {
			for (; level < BT_LEVEL_LIMIT; level++) {
				ret = backtrace_from_stack(&SP, &PC, &LR,
							   print_func);
				if (ret != 0) {
					break;
				}
...
int arch_backtrace(char *taskname, void *trace[], int size, int offset,
		   print_function print_func)
{
	return _backtrace(taskname, trace, size, offset, print_func, 0, 0, 0, 0,
			  0);
}

int arch_backtrace_exception(print_function print_func, unsigned long arg_cpsr,
			     unsigned long arg_sp, unsigned long arg_pc,
			     unsigned long arg_lr)
{
	return _backtrace(NULL, NULL, 0, 0, print_func, arg_cpsr, arg_sp,
			  arg_pc, arg_lr, 1);
}
