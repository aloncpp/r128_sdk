...
#define SPINOR_LOGICAL_OFFSET CONFIG_SUNXI_MELIS_LOGICAL_OFFSET
#else
...
int sunxi_sprite_download_mbr(void *buffer, uint buffer_size)
{
	...
	if (get_boot_storage_type() == STORAGE_EMMC0) {
		mbr_num = 1;
	}
#endif
...
int download_secure_boot0(void *buffer, int production_media)
{
	...
	int ret = 0;
	printf("storage type = %d\n", production_media);
	int boot0_max_size = 64*1024;
	ret = sunxi_sprite_download_spl(buffer, boot0_max_size, production_media);
...
int download_normal_boot0(void *buffer, int production_media)
{
	...
	extern int sunxi_set_boot_select(void);
	if (!ret) {
		printf("burn boot0 ok, set boot select bit\n");
		ret = sunxi_set_boot_select();
	}
#endif
...
#if defined(CONFIG_SUNXI_RTOS) || defined(CONFIG_SUNXI_MELIS)
		*logic_offset = SPINOR_LOGICAL_OFFSET;
#endif
