...
static int pll_lock_is_new_mode(void)
{
	__u32 reg_val;
...
static void enable_pll_lock_bit(__u32 lock_bit)
{
	__u32 reg_val;
	reg_val = readl(CCMU_PLL_LOCK_CTRL_REG);
	reg_val |= lock_bit;
...
static void disable_pll_lock_bit(__u32 lock_bit)
{
	__u32 reg_val;
	reg_val = readl(CCMU_PLL_LOCK_CTRL_REG);
	reg_val &= (~lock_bit);
...
int clock_set_corepll(int frequency)
{
	...
	reg_val &= ~((1 << 31) | (0x03 << 16) | (0x1f << 8) | (0x03 << 4) |
		     (0x03 << 0));
	reg_val |= (pll_factor.FactorP << 16) | (pll_factor.FactorN << 8) |
		   (pll_factor.FactorK << 4) | (pll_factor.FactorM << 0);
	writel(reg_val, &ccm->pll1_cfg);
	enable_pll_lock_bit(LOCK_EN_PLL_CPUX);
	writel((1 << 31) | readl(&ccm->pll1_cfg), &ccm->pll1_cfg);
