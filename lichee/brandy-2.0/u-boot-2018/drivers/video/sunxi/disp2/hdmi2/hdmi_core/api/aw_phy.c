...
static struct aw_phy_config aw_phy[] = {
	...
	{594000, PIXEL_REPETITION_OFF, COLOR_DEPTH_8,  0x0, 0x7770, 0xAAAFFFF, 0x131F1F1F, 0x803000, 580000},
...
static int phy_enable(void)
{
	...
	for (i = 0; i < AW_PHY_TIMEOUT; i++) {
		...
		if (status & 0x1) {
			HDMI_INFO_MSG("[%s]:phy_rcalend2d_status\n", __func__);
			break;
		}
	}
	...
	for (i = 0; i < AW_PHY_TIMEOUT; i++) {
		...
		if (status & 0x1) {
			HDMI_INFO_MSG("[%s]:pll_lock_status\n", __func__);
			break;
		}
	}
	if ((i == AW_PHY_TIMEOUT) && !status) {
	...
	for (i = 0; i < AW_PHY_TIMEOUT; i++) {
		...
		if (status & 0x1) {
			HDMI_INFO_MSG("[%s]:tx_ready_status\n", __func__);
			break;
		}
	}
	if ((i == AW_PHY_TIMEOUT) && !status) {
						...
						encoding_t EncodingOut)
{
	...
	if (pClk == 594000) {
		phy_base->pll_ctl0.bits.n_cntrl = 0;
		phy_base->modulator_cfg1.bits.fnpll_mash_fnv = 4;
		phy_base->phy_ctl6.bits.switch_clkch_data_corresponding = 1;
	}
	...
	for (i = 0; i < LOCK_TIMEOUT; i++) {
		...
		if (lock & 0x1) {
			HDMI_INFO_MSG("[%s]:PHY PLL locked\n", __func__);
			return TRUE;
		}
	}
