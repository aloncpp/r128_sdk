...
void sunxi_dma_init(void)
{
	...
	setbits_le32(&ccm->dev_rst_ctrl1, 1 << 6);
	setbits_le32(&ccm->bus_clk_gating_ctrl1, 1 << 6);
	setbits_le32(&ccm->mbus_clk_gating_ctrl, 1 << 5);
#else
...
void sunxi_dma_exit(void)
{
	...
	/* sunxi_dma_init function in rtos does not enable dma clock.
	 * So we do not close dma clock here.
	 */
#ifndef CONFIG_SUNXI_RTOS
#if defined(CONFIG_SUNXI_VERSION1)
