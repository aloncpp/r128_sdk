		...
		struct aw_spinand_chip_request *super)
{
	...
	/* mark the 1st block */
	ret1 = aw_spinand_chip_markbad_single_block(chip, &phy);

	/* check the 2st block */
	phy.block++;
	ret2 = aw_spinand_chip_markbad_single_block(chip, &phy);

	/* return failed only when both of two blocks mark bad failed */
