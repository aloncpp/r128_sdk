...
#if defined(CONFIG_MACH_SUN20IW2)
	/* reset */
	reg_val = get_wvalue(SUNXI_CCM_BASE + 0xc);
	reg_val |= 0x1U << 0;
	put_wvalue(SUNXI_CCM_BASE + 0xc, reg_val);
	/* open bus gating */
	reg_val = get_wvalue(SUNXI_CCM_BASE + 0x4);
	reg_val |= 0x1U << 0;
	put_wvalue(SUNXI_CCM_BASE + 0x4, reg_val);
#endif
	/* open clock */
	reg_val = get_wvalue(sclk0_reg_adr);
	reg_val |= 0x1U << 31;
