...
int sunxi_image_header_check(sunxi_image_header_t *ih, void *pbuffer)
{
	uint16_t chksum = 0;
	...
	if (chksum != 0xffff) {
		printf("header chksum error: 0x%x!\n", chksum);
		return -1;
	}
	...
	if (pbuffer == NULL)
		pbuffer = (void *)((uint8_t *)ih + ih->ih_hsize);
	chksum += image_get_checksum16(pbuffer, ih->ih_psize);
	if (chksum != 0xffff) {
		printf("data chksum error: 0x%x!\n", chksum);
		return -1;
	}
...
int sunxi_image_header_magic_check(sunxi_image_header_t *ih)
{
	if (ih->ih_magic != SUNXI_IH_MAGIC) {
		return -1;
	}
...
int sunxi_image_header_dump(sunxi_image_header_t *ih)
{
	printf("magic:0x%x\n", ih->ih_magic);
	printf("hversion:0x%x\n", ih->ih_hversion);
	printf("pversion:0x%x\n", ih->ih_pversion);
	printf("hsize:0x%x\n", ih->ih_hsize);
	printf("psize:0x%x\n", ih->ih_psize);
	printf("tsize:0x%x\n", ih->ih_tsize);
	printf("load:0x%x\n", ih->ih_load);
