...
int sunxi_verify_rtos(struct rtos_img_hdr *rtos_hdr)
{
	bool rtos_with_cert = 0;
	if (!strcmp(rtos_hdr->cert_magic, AW_CERT_MAGIC) &&
		rtos_hdr->cert_offset != 0 && rtos_hdr->cert_size != 0)
		rtos_with_cert = 1;
	else
		rtos_with_cert = 0;

	printf("rtos with embedded signature: %d\n", rtos_with_cert);

	if (rtos_with_cert)
		return sunxi_verify_embed_signature(
				(void *)((unsigned char *)rtos_hdr + rtos_hdr->rtos_offset),
				rtos_hdr->rtos_size,
				"freertos-gz",
				(void *)((unsigned char *)rtos_hdr + rtos_hdr->cert_offset),
				rtos_hdr->cert_size);
	else
		return sunxi_verify_signature(
				(void *)((unsigned char *)rtos_hdr + rtos_hdr->rtos_offset),
				rtos_hdr->rtos_size,
				"freertos-gz");
}
...
#else
	if (strcmp(name, "freertos-gz") == 0) {
		if (memcmp(key_hash, uboot_spare_head.freertos, 32) == 0) {
			printf("pubkey %s valid\n", name);
			return 0;
		} else {
			printf("pubkey %s invalid\n", name);
			return -1;
		}
	} else {
		printf("pubkey %s not found\n", name);
...
#else
	if (strcmp(cert_name, "freertos-gz") == 0) {
		if (memcmp(hash_of_file, uboot_spare_head.freertos, 32) == 0) {
			printf("image %s hash valid\n", cert_name);
			return 0;
		} else {
			printf("image %s hash invalid\n", cert_name);
			return -1;
		}
	} else {
...
static int sunxi_rsa_pkey_check(uint8_t *key_n, uint32_t n_len,
			uint8_t *key_e, uint32_t e_len, char *key_name)
{
	int ret = 0;
	uint8_t pk[2048 / 8 * 2 + 256 + 32] = {0}; // for sha padding
	uint8_t pkey_hash[256 / 8] = {0};
	uint8_t *align = (uint8_t *)(((uint32_t)pk + 31) & (~31));

	memset(pk, 0x91, sizeof(pk));
	memcpy(align, key_n, n_len);
	memcpy(align + n_len, key_e, e_len);
	ret = sunxi_sha_calc(pkey_hash, 32, align, 2048 / 8 * 2);
	if (ret) {
		printf("sunxi_sha_calc: calc rsa pubkey sha256 with hardware err\n");
		return ret;
	}

	ret = tfm_check_hash(key_name, pkey_hash);

	if (ret) {
		printf("sunxi_rsa_pkey_check failed\n");
		return ret;
	}
...
static int sunxi_ecc_pkey_check(uint8_t *ecc_pkey, uint32_t ecc_pkey_len, char *key_name)
{
	int ret = 0;
	uint8_t pk[256 / 8 * 2 + 256 + 32] = {0}; // for sha padding
	uint8_t pkey_hash[256 / 8] = {0};
	uint8_t *align = (uint8_t *)(((uint32_t)pk + 31) & (~31));

	memcpy(align, ecc_pkey, ecc_pkey_len);
	ret = sunxi_sha_calc(pkey_hash, 32, align, ecc_pkey_len);
	if (ret) {
		printf("sunxi_sha_calc: calc ecc pubkey sha256 with hardware err\n");
		return ret;
	}
	...
	if (ret) {
		printf("sunxi_ecc_pkey_check failed\n");
		return ret;
	}
...
{
	int ret = 0;
	//uint32_t payload_addr = (uint32_t)payload;
	//uint32_t hpsram_size = 0;
	uint8_t key_n[256] = {0};
	uint8_t key_e[256] = {0};
	uint8_t ecc_pkey[64] = {0};
	uint8_t sign[256] = {0};
	...
	sunxi_ss_open();
	// check roptk
	// 1. sunxi_sha_calc will padding at the end of src, and write the pkey and signature, so we backup the pkey and sign.
	// 2. key and sign buffer address align
	if (tlv->th_pkey_type == 0) {
		memcpy(key_n, (uint8_t *)tlv + tlv->th_size, 256);
		memcpy(key_e, (uint8_t *)tlv + tlv->th_size + 256, 256);
		memcpy(sign, (uint8_t *)tlv + tlv->th_size + tlv->th_pkey_size, 256);
		ret = sunxi_rsa_pkey_check(key_n, 256, key_e, 3, (char *)tlv->th_priv);
		if (ret) {
			return ret;
		}
	} else if (tlv->th_pkey_type == 1) {
		memcpy(ecc_pkey, (uint8_t *)tlv + tlv->th_size, 64);
		memcpy(sign, (uint8_t *)tlv + tlv->th_size + tlv->th_pkey_size, 64);
		ret = sunxi_ecc_pkey_check(ecc_pkey, 64, (char *)tlv->th_priv);
		if (ret) {
			return ret;
		}
	}
	// calc payload hash
	memset(hash_of_file, 0, sizeof(hash_of_file));
			memset(payload - ih->ih_hsize, 0, ih->ih_hsize);
			memcpy(payload - ih->ih_hsize, ih, ih->ih_hsize);
			memcpy(payload + ih->ih_psize, tlv, tlv->th_size + tlv->th_pkey_size);
			ret = sunxi_sha_calc(hash_of_file, sizeof(hash_of_file), payload - ih->ih_hsize,
									ih->ih_hsize + ih->ih_psize + tlv->th_size + tlv->th_pkey_size);
	if (ret) {
		printf("calc file sha256 with hardware err\n");
		return -1;
	}
	...
	if (tlv->th_pkey_type == 0) {
		...
		if (memcmp(hash_of_file, sign_of_hash, 32)) {
			printf("hash compare is not correct\n");
			printf(">>>>>>>hash of file<<<<<<<<<<\n");
			ndump((uint8_t *)hash_of_file, 32);
			printf(">>>>>>>sign of hash<<<<<<<<<<\n");
			ndump((uint8_t *)sign_of_hash, 32);

			return -1;
		}
	} else if (tlv->th_pkey_type == 1) {
		if (!uECC_verify(ecc_pkey, hash_of_file, TC_SHA256_DIGEST_SIZE, sign, &curve_secp256r1)) {
			printf("uECC_vierfy error\n");
			printf(">>>>>>>hash of file<<<<<<<<<<\n");
			ndump((uint8_t *)hash_of_file, 32);
			return -1;
		}
