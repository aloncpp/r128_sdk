					...
					bool prot)
{
	...
	if (prot) {
		// printf("%s %d ofs:0x%x len:0x%x lock\n",__func__, __LINE__, ofs, len);
		return flash->flash_lock(flash, (loff_t )ofs, (u64)len);
	} else {
		// printf("%s %d ofs:0x%x len:0x%x unlock\n",__func__,__LINE__, ofs, len);
		return flash->flash_unlock(flash, (loff_t )ofs, (u64)len);
	}
}

static inline int spi_flash_is_protected(struct spi_flash *flash, u32 ofs, u32 len)
{
